...
class Publisher:
    ...
    def publish(self, message: dict, routing_key: str, context: Context = None):
        """
        ...
        self._channel.publish(bytes(message_str, encoding), routing_key, context.properties)
...
class Service(Publisher):
    ...
    def _message_received(self, message: AmqpMessage):
        try:
            ...
            message_dict = json.loads(str(message.body, encoding=encoding))
            ...
            logger_manager.get_logger().error("Service - Error al procesar el mensaje", ex)
