...
from ialib.PredictionModels import predictionEnsembleModelClassification, hiveModelClassification
...
class Tester:
    ...
    def train(self, sequence):
        ...
        self.bottle.observeClassification(sequence[-1])  ## Now, we want all the query nodes to get the classification.
    ...
    def test(self, sequence):
        ...
        for node_ensemble in answers:
            for node, ensemble in node_ensemble.items():
                ...
                predicted_value = predictionEnsembleModelClassification(ensemble)
                record['node_predictions'][node] = predicted_value
                hive_prediction.append(predicted_value)
        
        hive_prediction = hiveModelClassification(hive_prediction)
