...
def validateData(data):
    ...
    if "vectors" not in list(data.keys()) or "determinants" not in list(data.keys()) or "strings" not in list(data.keys()):
...
class DataRecords:
    ...
    def __init__(self, original_dataset, DR, DF, shuffle, folder=True):
        ...
        '''
        if folder:
            original_dataset = [original_dataset + '/' + f for f in os.listdir(original_dataset) if not f.startswith('.') and not f.startswith('_')]
        ...
        try:
            ...
            print("  DataRecords BROKE by %s", e.args)
            raise
        return
...
class Data():
    ...
    def prep(self, percent_of_dataset_chosen, percent_reserved_for_training, shuffle=False):
        if self.data_directories:
            data = [DataRecords(d, percent_of_dataset_chosen, percent_reserved_for_training, shuffle, folder=True) for d in self.data_directories] ## It's a list because the user may pick several data file directories.
        elif self.dataset:
            data = [DataRecords(self.dataset, percent_of_dataset_chosen, percent_reserved_for_training, shuffle, folder=False)]
