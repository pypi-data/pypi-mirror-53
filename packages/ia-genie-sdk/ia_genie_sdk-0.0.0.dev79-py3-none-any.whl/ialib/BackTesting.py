...
class BackTest:
    ...
    def __init__(self, **kwargs):
        ...
        self.mongo_client.backtesting = self.mongo_client['%s-%s-%s' %(self.name, self.bottle.genome.agent, self.bottle.name)]  ## Collection storing the backtesting results is the bottle's name.
        ...
        if self.test_type == "utility":
            self.TEST = Utility.Tester(**kwargs)
        elif self.test_type == 'classification':
            self.TEST = Classification.Tester(**kwargs)
        ...
        headers = ["Test Run", "Trial", "Phase", "Filename", "Historical"] + [node["name"] for node in self.bottle.query_nodes] + ["hive"]
        self.backtesting_log.insert_one({"headers": headers})
        self.f = FloatProgress(min=0, max=self.number_of_things_to_do, description="Starting...", bar_style="info")
    ...
    def run(self):
        ...
        while self.current_test_count < self.total_test_counts:
            ...
            for sequence in self.data.train_sequences:
                ...
                self.f.value = self.number_of_things_done 
                self.f.description = '%0.2f%%' %(100*self.number_of_things_done /self.number_of_things_to_do)
    ...
    def train(self, s):
        ...
        if self.data_directories:
            self.backtesting_log.insert_one({"timestamp_utc": datetime.datetime.utcnow(), "status": "training", "file": os.path.basename(s)})
            with open(s) as f:
        ...
        ## Train the sequence:
        result_log_record = self.TEST.train(sequence)
    ...
    def test(self, s):
        ## get a sequence either from a file, or directly as a list:
        if self.data_directories:
            self.backtesting_log.insert_one({"timestamp_utc": datetime.datetime.utcnow(), "status": "testing", "file": os.path.basename(s)})
            with open(s) as f:
                sequence = [json.loads(data.strip()) for data in f if data]
        elif self.data_source:
            sequence = s
        ...
        result_log_record = self.TEST.test(sequence)
        result_log_record['trial'] = self.number_of_things_done
        result_log_record['run'] = self.current_test_count
