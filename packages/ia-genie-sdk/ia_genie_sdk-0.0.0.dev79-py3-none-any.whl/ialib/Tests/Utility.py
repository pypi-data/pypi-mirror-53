...
from ialib.PredictionModels import predictionEnsembleModelUtility, hiveModelUtility
...
class Tester:
    ...
    def nextTestPrep(self):
        "Anything to reset in between multiple test runs."
        return
    
    def train(self, sequence):
        record = {'phase': 'training'}
        record['historical_expecting'] = sum([sum(data['determinants']) for data in sequence])
    ...
    def test(self, sequence):
        ...
        for answer in answers:
            for node, ensemble in answer.items():
                predicted_value = predictionEnsembleModelUtility(ensemble)
                hive_prediction.append(predicted_value) ## Add to hive model's list for later
                record['node_predictions'][node] = predicted_value
        
        hive_prediction = hiveModelUtility(hive_prediction)
        record['node_predictions']['hive'] = hive_prediction
        if hive_prediction == historical_expecting:
            predicton_error = False
        
        if self.learning_strategy == 'continuous' or (self.learning_strategy == 'on-error' and predicton_error):  ### NOTE!: Unlike regular symbols for the classification tests, utilities pass through all nodes, so we don't need to send directly to egress nodes.  Simply send through ingress nodes.
