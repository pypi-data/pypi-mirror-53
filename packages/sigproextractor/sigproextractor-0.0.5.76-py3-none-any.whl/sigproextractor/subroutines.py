...
def pnmf(seed=None, genomes=1, totalProcesses=1, resample=True, init="random", normalization_cutoff=10000000, gpu=False):
    ...
    if resample == True:
        ...
        genomes = normalize_samples(genomes, normalize=True, all_samples=False, number=normalization_cutoff)
    ...
    # denormalize H
    H = denormalize_samples(H, totalMutations, normalization_value=normalization_cutoff) 
    print ("process " +str(totalProcesses)+" continues please wait... ")
    print ("execution time: {} seconds \n".format(round(time.time()-tic), 2))
...
def parallel_runs(genomes=1, totalProcesses=1, iterations=1,  n_cpu=-1, verbose = False, resample=True, seeds = None, init="random", normalization_cutoff=1, gpu=False):
    if verbose:
...
def stabVsRError(csvfile, output, title, all_similarities_list, mtype= ""):
    ...
    probable_solutions = probable_solutions[probable_solutions["Minimum Stability"]>=0.5]
