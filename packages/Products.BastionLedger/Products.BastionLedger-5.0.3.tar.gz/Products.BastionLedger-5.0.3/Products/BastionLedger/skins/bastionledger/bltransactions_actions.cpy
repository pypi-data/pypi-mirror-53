...
elif REQUEST.has_key('form.button.Merge'):
    context.manage_mergeTransactions(ids)
    context.plone_utils.addPortalMessage('Merged %i transaction(s) into %s' % (len(ids), ids[0]))
else:
