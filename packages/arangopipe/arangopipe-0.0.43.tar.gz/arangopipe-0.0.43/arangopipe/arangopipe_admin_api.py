...
class ArangoPipeAdmin:
    ...
    def add_edge_definition_to_arangopipe(self, edge_name, from_vertex, to_vertex):
        ...
        if not self.emlg.has_vertex_collection(from_vertex):
            logger.error("Source vertex, " + from_vertex +\
                         " does not exist, aborting edge creation!")
            return
        elif not self.emlg.has_vertex_collection(to_vertex):
            logger.error("Destination vertex, " + to_vertex +\
                         " does not exist, aborting edge creation!")
            return
