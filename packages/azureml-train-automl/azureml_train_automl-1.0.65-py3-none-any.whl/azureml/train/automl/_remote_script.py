...
from automl.client.core.common.exceptions import AutoMLException, ClientException
...
from .jasmine_client_proxy import JasmineClientProxy
...
def _get_data_from_serialized_dataflow(dataprep_json: str,
    ...
    if automl_settings_obj.enable_streaming:
        ...
        fit_iteration_parameters_dict['x_raw_column_names'] = training_data.head(1).columns.values
        ...
        fit_iteration_parameters_dict['X'] = training_data.drop_columns(
            automl_settings_obj.label_column_name)
        fit_iteration_parameters_dict['y'] = training_data.keep_columns(
            automl_settings_obj.label_column_name)
        fit_iteration_parameters_dict['X_valid'] = validation_data.drop_columns(
            automl_settings_obj.label_column_name)
...
def _transform_and_validate_input_data(
    ...
    jasmine_client_proxy = None  # If this remains none, we default to blob store config for feature_sweeping.
    if experiment is not None:
        ...
        jasmine_client_proxy = JasmineClientProxy(jasmine_client=jasmine_client)
    ...
    transformed_data_context = data_transformation.transform_data(
        ...
        jasmine_client=jasmine_client_proxy,
...
def _post_parent_run_error(parent_run: Run, error_details: BaseException, error_code: str) -> None:
    try:
