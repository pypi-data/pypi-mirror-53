...
def load_model_file(eslib, id, inference_type=constants.ESTIMATOR):
        ...
        if not ok:
            ...
            if inference_type == constants.ESTIMATOR:
                ...
                return None, None, pred, None, None
        ...
        if (not _.is_empty(res)):
            try:
                ...
                try:
                    filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], storage_folder, constants.ONEHOTENCODER_EXTENSION)    
                    onehotencoder = joblib.load(filename)
                except Exception as ex:
                    onehotencoder = None
                    _logger.error('One hot encoder not found')
                ...
                if inference_type == constants.ESTIMATOR:
                    return None, None, pred, onehotencoder
                else:
                    return None, None, pred, onehotencoder, None

        else:
            if inference_type == constants.ESTIMATOR:
                return None, None, pred, onehotencoder
            else:
                return None, None, pred, None, onehotencoder
        if inference_type == constants.ESTIMATOR:
            return model, scaler, pred, onehotencoder
        else:
            return model, scaler, pred, labelencoder, onehotencoder
...
def save_model(eslib, id, model, scaler, metadata, labelencoder=None, onehotencoder=None):
    try:
        ...
        if onehotencoder:
            joblib.dump(onehotencoder, filename + '.' + constants.ONEHOTENCODER_EXTENSION)
            uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.ONEHOTENCODER_EXTENSION, id, config['ML']['algorithm'])
            if not uploaded:
                    _logger.error('Error on uploading file to min.io filesystem(' + filename + '.' + constants.ONEHOTENCODER_EXTENSION + ') - ')
                    return ''
