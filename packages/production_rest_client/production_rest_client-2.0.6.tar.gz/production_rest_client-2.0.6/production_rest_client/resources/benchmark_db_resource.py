...
class BenchmarkDBResource(object):
    ...
    def get_search_match_string(self, index=None, project_name=None, begin_time=None, end_time=None, test_name=None):
        ...
        if index is not None:
            match_string = self.get_index_match_string(index)
            search_str = self.generate_filter_string(search_str, match_string, is_first_filter)

        return search_str

    @staticmethod
    def get_index_match_string(index):
        match_str = "`index`='{}'".format(index)
        return match_str
    ...
    def get_real_time_results(self, index):
        real_time_table_name = self.get_spec_index(index, "real_time_index")
