...
def disable_cgal_cpp_support():
    ...
    if sys.maxsize <= 2**32:
        print("You are using either a 32bit operating system or a 32bit Python",
              "environment. For full functionallity, 64bit is needed.",
...
# for windows, we have to add the directory of our module to the PATH
# (otherwise, the dependent libs will not be found)
def __change_path_on_windows(os_name, add_path):
    if os_name == "Windows":
        if add_path not in os.environ["PATH"]:
            os.environ["PATH"] = "{};{}".format(os.environ["PATH"], add_path)
    ...
    for path in paths[os_name]:
        try:
            if os.path.exists(path):
                __change_path_on_windows(os_name, os.path.dirname(path))
