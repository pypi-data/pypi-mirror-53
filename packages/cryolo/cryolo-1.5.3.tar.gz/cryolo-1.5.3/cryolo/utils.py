...
def check_for_updates():
    ...
    try:
        ...
        with urllib.request.urlopen(
            ...
            print(current_version,lastest_version)
            for ver in data["releases"].keys():
                vers = V(ver)
                if vers > V(lastest_version) and len(vers.version) == 3:
                    lastest_version = ver
            if V(lastest_version) > V(current_version):
                print("###############################################")
                print("New version of crYOLO available")
                print("Local version:\t\t", current_version)
                print("Latest version:\t\t", lastest_version)
