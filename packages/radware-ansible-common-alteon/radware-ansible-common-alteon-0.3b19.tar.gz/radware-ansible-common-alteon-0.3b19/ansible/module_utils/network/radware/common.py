...
try:
    from radware.sdk.common import RadwareParametersStruct, RadwareParametersExtension, \
        PasswordArgument, get_annotation_class, is_annotation_type_optional, is_annotation_type_list, is_optional_type_list
...
radware_adc_spec = {}
radware_adc_spec.update(radware_provider_spec)
radware_adc_spec.update(ssh_server_spec)
radware_adc_spec.update(https_server_spec)
...
def build_specs_from_annotation(parameter_class):
    instance = parameter_class()
    specs = dict()
    annotations = parameter_class.__annotations__
