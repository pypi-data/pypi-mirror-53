...
class TaskClient(BaseClient):
    ...
    def ackTask(self, taskId, workerid):
        ...
        params = {}
        params['workerid'] = workerid
        value = self.post(url, params, None)
        return value == 'true'
...
class WorkflowClient(BaseClient):
    ...
    def getWorkflows(self, wfName, version=1, startTime=None, endTime=None):
        url = self.makeUrl('search')
        params = {}
        params['size'] = 1000 
        params['freeText'] = "workflowType={}".format(wfName)
        return self.get(url, params)
    ...
    def startWorkflowWithDomain(self, wfName, inputjson, domainMap, version=1, correlationId=None):
        url = self.makeUrl('{}', wfName)
        params = {}
        params['version'] = version
        params['correlationId'] = correlationId
        params['taskToDomain'] = domainMap
        headers = {'Accept': 'text/plain'}
        return self.post(url, params, inputjson, headers)

    def clearRunningWorkflow(self, workflowName, workflowVersion):
        running_workflows = self.getRunningWorkflows(workflowName, workflowVersion)
        for workflow in running_workflows:
            self.terminateWorkflow(workflow)
            url = self.makeUrl('{}/remove', workflow) + '?archiveWorkflow=false'
            self.delete(url)
            print('delete {} '.format(workflow))
        return

    def clearWorkflow(self, workflowName, workflowVersion):
        workflows = self.getWorkflows(workflowName, workflowVersion)
        if "results" not in workflows.keys():
            print("No workflows")
            return
        workflowIDs = map(lambda x: x["workflowId"], workflows["results"])
    ...
    def getExcutedTask(self, wfId, taskName):
        workflowObj = self.getWorkflow(wfId, True)
        for task in workflowObj['tasks']:
            if task['referenceTaskName'] == taskName:
                return task
        return None
    def decideWorkflow(self, wfId):
        url = self.makeUrl('decide/{}', wfId)
        self.put(url)
    # -----------suchu.gao add end---------------------
