...
try:
    import quilt3
    quilt3._DEV_MODE = True
except ModuleNotFoundError:  # pragma: no cover
    quilt3 = None  # pragma: no cover
import re
import requests
...
class QuiltManager(object):
    ...
    def __init__(self, path, package, owner=None, token=None, verbose=None):
        ...
        if not quilt3:
            raise ModuleNotFoundError('Quilt must be installed. Run `pip install quilt3`')  # pragma: no cover
    ...
    def upload(self):
        ...
        with abduct.captured(abduct.out(tee=self.verbose)):
            quilt3.build(self.get_owner_package(), config_filename)
            quilt3.login_with_token(self.token)
            quilt3.push(self.get_owner_package(), is_public=True, is_team=False)
    ...
    def download(self, system_path=None, sym_links=False):
        ...
        with abduct.captured(abduct.out(tee=self.verbose)):
            quilt3.login_with_token(self.token)
            quilt3.install(pkg_name_path, force=True, meta_only=False)
            quilt3.export(pkg_name_path, output_path=self.path,
    ...
    def get_package_path(self, system_path):
        ...
        with abduct.captured(abduct.out(tee=self.verbose)):
            quilt3.install(pkg_name, force=True, meta_only=True)

        pkg = importlib.import_module('quilt3.data.' + pkg_name.replace('/', '.'))

        nodes_to_visit = [((), pkg)]
        while nodes_to_visit:
            parent_pkg_path, parent = nodes_to_visit.pop()
            for child_name, child in parent._items():
                if isinstance(child, quilt3.nodes.DataNode):
                ...
                elif isinstance(child, quilt3.nodes.GroupNode):
