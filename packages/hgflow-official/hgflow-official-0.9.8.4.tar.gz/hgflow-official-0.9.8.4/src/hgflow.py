...
class Flow( object ) :
    ...
    def _shelve_name( self, bname ) :
        """
        Given a branch's name, returns a proper path to the file of the corresponding shelved patch.

        @type  bname: C{str}
        @param bname: Branch name
        """
        # Replaces '#' with '_SHaRP_' because hg forbids '#' in patch-file names. '_SHaRP_' is arbitrarily chosen just to
        # have a low chance of name collisions.
        bname = bname.replace( "#", "_SHaRP_" )
        return "flow/" + bname + ".pch"
    ...
    def _is_shelved( self, branch ) :
        ...
        """
        patch_fname = self.repo.vfs.join( "patches/" + self._shelve_name( branch.fullname() ) )
    ...
    def _shelve( self, *arg, **kwarg ) :
        ...
        if (self.autoshelve or kwarg.get( "force" )) :
            if (self._has_uncommitted_changes()) :
                shelve_name = self._shelve_name( self.curr_workspace.fullname() )
                if (commands.dryrun()) :
    ...
    def _unshelve( self, basename = None, **kwarg ) :
        ...
        if (self.autoshelve or kwarg.get( "force" )) :
            ...
            shelve_name = self._shelve_name( basename )
...
"@tips" : """
- To just know what a hgflow action/command would exactly do, use the
  {{{--dry-run}}} option. Hgflow will print out the exact hg commands it would
  execute in sequence without actually executing them. For example:
    {{{> hg flow feature finish --dry-run}}}
    flow: This is a dry run.
    flow: note: Hg command history:
    flow: note:   hg commit --message "flow: Closed <hotfix> '0.9.8.4'." --close-branch
    flow: note:   hg update default
    flow: note:   hg merge hotfix/0.9.8.4
    flow: note:   hg commit --message "flow: Merged <hotfix> '0.9.8.4' to <master> ('default')."
    flow: note:   hg tag --force v0.9.8.4
    flow: note:   hg update develop
    flow: note:   hg merge hotfix/0.9.8.4
    flow: note:   hg commit --message "flow: Merged <hotfix> '0.9.8.4' to <develop> ('develop')."

- To move uncommitted changes in the workspace into a different branch, create a
  new branch with the {{{--dirty}}} option. For example, say I am currently in
  the 'hotfix/0.9.8.4' branch, I made some changes that I now think are better
  committed into a <feature> branch, I can move the (uncommitted) changes to
  temporary branch called 'feature/tmp' using the following commands:
    {{{> hg flow feature start tmp --dirty}}}
  then use {{{hg commit}}} to commit the change into the new 'feature/tmp'
  branch, afterwards I can use {{{hg graft}}} to move the changes to whichever
  branch, and at the end to clean up the temporary branch, use
  {{{hg flow feature tmp}}} and {{{hg flow feature abort -e}}}.
""",
...
- flow action
  ...
  'start' is an action. Use {{{hg flow -h}}} to see all available actions and
  the options of each action.
