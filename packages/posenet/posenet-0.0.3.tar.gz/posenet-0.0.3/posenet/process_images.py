...
import numpy as np
import cv2, time, argparse, os, posenet, glob
...
def process_images(*args):
  ...
  args['image_glob'] = args.get('image_glob', './images/*')
  ...
  with tf.Session() as sess:
    ...
    filenames = glob.glob(args['image_glob'])

    start = time.time()
    for idx, f in enumerate(filenames):
      print(' * processing image', idx+1, 'of', len(filenames), ':', f)
      ...
      if args['log']: print(input_image, pose_scores, keypoint_coords)
      ...
      if args['output_dir'] and not args['noimg']:
        ...
        cv2.imwrite(os.path.join(args['output_dir'], os.path.basename(f)), draw_image)

      data = []
      if not args['notxt']:
        if args['log']: print()
        if args['log']: print('Results for image: %s' % f)
        for pi in range(len(pose_scores)):
          pose_data = []
          if pose_scores[pi] == 0.:
            break
          if args['log']: print('Pose #%d, score = %f' % (pi, pose_scores[pi]))

          for ki, (s, c) in enumerate(zip(keypoint_scores[pi, :], keypoint_coords[pi, :, :])):
            if args['log']: print('Keypoint %s, score = %f, coord = %s' % (posenet.constants.PART_NAMES[ki], s, c))
    ...
    if args['log']: print('Average FPS:', len(filenames) / (time.time() - start))
