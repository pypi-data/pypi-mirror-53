...
class BTC(Coin):
    ...
    def __aenter__(self) -> "BTC":
        return self

    def __aexit__(self, exc_type: Any, exc_value: Any, tb: Any) -> None:
        self.close()
    ...
    def pay_to_many(
        ...
        if not fee_arg:
            ...
            try:
                ...
                resulting_fee = None
            if resulting_fee:
                tx_data = self.server.paytomany(outputs,fee=resulting_fee,feerate=feerate)
        if broadcast:
            return self.server.broadcast(tx_data) # type: ignore
        else:
            return tx_data  # type: ignore
