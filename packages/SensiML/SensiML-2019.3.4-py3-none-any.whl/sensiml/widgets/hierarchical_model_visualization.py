...
class HierarchicalModelVisualizationWidget():
    ...
    def score_nodes(self, acc_dict, kb_description):
        ...
        while [label for label, value in acc_dict.items() if value is None]:
            ...
            for node in [label for label, value in acc_dict.items() if value is None]:
                both_node = sum([label in kb_description.keys(
                ) for _, label in kb_description[node]['results'].items()]) == 2
                corrected_score = [acc_dict[label]
                ...
                if both_node and None not in corrected_score:
                    acc_dict[node] = mean(
                        [acc_dict[label] for _, label in kb_description[node]['results'].items()])

                # this is a node has 1 child node and one leaf with corrected stat
                if sum([label in label_list for _, label in kb_description[node]['results'].items()]) == 1:
                    acc_list = []
                    for _, label in kb_description[node]['results'].items():
    ...
    def compute_hm_graph_paramenter(self, kp):
        ...
        for i, node in enumerate(kb_description_list):
            ...
            if node == 'Parent':
                hm_confusionmatrix = temp_kp.model_results['metrics']['hierarchical']['ConfusionMatrix']
            ...
            for cn_index, cn_node in child_nodes.items():
                if cn_node != 'Report':
                    ...
                    class_acc = round(100 * hm_confusionmatrix[class_label][class_label] / sum(
                        list(hm_confusionmatrix[class_label].values())), 2)
                    leaf_acc_list.append(class_acc)
                    acc_dict[class_label] = {
                        'Accuracy': class_acc, 'Predictions': hm_confusionmatrix[class_label]}
                ...
                link_data.append(
                    {'source': link_map[node], 'target': num, 'value': 1})
                num = num + 1
        ...
        for node_name in node_names:
            node_data_dict = {'label': node_name,
                              'shape': 'rect',
        ...
        over_all_accuracy = round(mean([acc_dict[node]['Accuracy'] for node in class_labels ]), 2)

        return node_names, link_data, node_data, y_axis_loc, x_axis_loc, colors, over_all_accuracy
    ...
    def plot_tree(self, kp):
        ...
        graph = Graph(node_data=node_data,
                      ...
                      x=x_axis_loc, y=y_axis_loc)

        tooltip = Tooltip(fields=['Model', 'Accuracy'], formats=['', ''])
        graph.tooltip = tooltip

        display(Figure(marks=[graph], layout=fig_layout,
