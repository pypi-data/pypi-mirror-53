...
class Query(object):
    ...
    def update(self):
        ...
        if (len(query_info['columns']) > 0 or len(query_info['metadata_columns']) > 0):
            filters = self._metadata_filter.split(
                'AND') if len(self._metadata_filter) > 0 else []
            for i, filter in enumerate([filter for filter in filters if len(filters)]):
                match = re.search(
                    "\[(?P<key>[0-9A-Za-z_\-\. \)\(]+)\]([ ]*(?P<symbol>[\>\=|\<\=|\=|\<\>|\>|\<|in|IN|!=]+)[ ]*\[(?P<value>.+)\])?", str(filter))
                if not match:
                    self._checked = 1
    ...
    def statistics(self):
        ...
        if err is False:
            ...
            data = df.groupby(['Capture']+self.metadata_columns._columns +
                              [self.label_column])[self.label_column].size().unstack(fill_value=0)
