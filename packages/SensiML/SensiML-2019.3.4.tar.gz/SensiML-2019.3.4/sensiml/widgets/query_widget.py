...
class QueryWidget():
    ...
    def update_statistics(self, query=None, plot_type='segments'):
        ...
        if query is not None:
            try:
                ...
                self._stats = query.statistics_segments()
                self.renderer.render("")
        ...
        for tmp_df in g:
            ...
            data.append(
                sorted(list(tmp_df[1]['Segment Length']), reverse=True))
        ...
        for j in range(max_segments):
            stacked_data.append([get_item(data[i], j, 0)
        ...
        ax_y = Axis(scale=y_lin, orientation='vertical',
                    label=plot_type, label_offset='60px')

        if plot_type == 'samples':
            bar = Bars(x=Labels, y=counts, color_mode='group', type='stacked',
                       scales={'x': x_ord, 'y': y_lin})

        elif plot_type == 'segments':
            bar = Bars(x=Labels, y=segments, color_mode='group', type='stacked',
                       scales={'x': x_ord, 'y': y_lin})

        elif plot_type == 'samples_stacked':
            bar = Bars(x=Labels, y=stacked_data, color_mode='group', type='stacked',
                       scales={'x': x_ord, 'y': y_lin})
    ...
    def create_widget(self, name=''):
        ...
        self._plot_type = Dropdown(description='Plot', options=[
                                   'segments', 'samples', 'samples_stacked'],
                                   value='segments')
        ...
        self._w_project_explorer = qgrid.show_grid(DataFrame({'capture_name': ['']}),
