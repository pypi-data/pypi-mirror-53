...
class ISAContext:
    ...
    def __init__(self,spark,debug=False):
        ...
        """
        spark.sql(f"""CREATE DATABASE IF NOT EXISTS {ADHOC_DB} LOCATION 's3a://{DORA_BUCKET}/{ADHOC_DB}/'""")
        spark.sql(f"""CREATE DATABASE IF NOT EXISTS {CACHE_DB} LOCATION 's3a://{DORA_BUCKET}/{CACHE_DB}/'""")
        spark.sql(f"""CREATE DATABASE IF NOT EXISTS {DELTA_DB} LOCATION 's3a://{DORA_BUCKET}/{DELTA_DB}/'""")
        self.spark = spark
        self.debug = debug
    ...
    def creat_version_of(self, table, version, alias):
        ...
        try:
            ...
            r = spark.sql(f"""SELECT * FROM(SELECT * FROM (SELECT max(timestamp)`dt`,max(version)`v` FROM {table_history} WHERE date_trunc('DD',timestamp)<='{dt}' 
