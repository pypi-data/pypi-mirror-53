...
class SchemaTransformer(object):
    ...
    * Properties: `type`, `description`, `required`, `enum`
    ...
    could call it with the shorthand JSON like so::
        ...
        $ altus foo bar --baz arg1=Value1,arg2=5 arg1=Value2
    ...
    def transform(self, schema):
        """Convert JSON schema to the format used internally by the Altus CLI.
        ...
        :return: The transformed model in a form that can be consumed
            internally by the Altus CLI.  The dictionary returned will
    ...
    def _transform(self, schema, shapes, shape_name):
        ...
        if schema['type'] == 'object':
            shapes[shape_name] = self._transform_object(schema, shapes)
        elif schema['type'] == 'array':
            shapes[shape_name] = self._transform_array(schema, shapes)
        else:
    ...
    def _transform_object(self, schema, shapes):
        # Transforming a object involves:
        # 1. Generating the shape definition for the object
        # 2. Generating the shape definitions for its members
        object_shape = self._populate_initial_shape(schema)
        members = {}
        required_members = []

        for key, value in schema['properties'].items():
            current_type_name = self._get_schema_type(value)
        ...
        object_shape['members'] = members
        if required_members:
            object_shape['required'] = required_members
        return object_shape

    def _transform_array(self, schema, shapes):
        # Transforming an array involves:
        # 1. Generating the shape definition for the array
        # 2. Generating the shape definitions for its 'items' member
        list_shape = self._populate_initial_shape(schema)
        member_type = self._get_schema_type(schema['items'])
    ...
    def _populate_initial_shape(self, schema):
        shape = {'type': self._get_schema_type(schema)}
