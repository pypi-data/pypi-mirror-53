...
class MqttTopicManager(object):
    ...
    def subscribe(self, event, callback):
        self.events.setdefault(event.event_name, []).append(callback)
        self.client.subscribe(event.event_name)

    def publish(self, event, qos=0, retain=False):
        self.client.publish(event.event_name, event.payload, qos, retain)
