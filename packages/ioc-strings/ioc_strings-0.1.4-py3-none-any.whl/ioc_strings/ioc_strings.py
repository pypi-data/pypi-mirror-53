...
class IOC:
    ...
    def __init__(self, ioc="", args={}, filepath="", tlds=None):
        self.ioc = ioc.strip()
        self.ioc_types = []
...
def handle_filepath(args, filepath, tlds):
    strings = run_strings(args, filepath)
    for s in strings:
        s = s.strip()
        ioc = IOC(s, args, filepath=filepath, tlds=tlds)
...
def main():
    ...
    tlds = IOC(args=args).init_tld()

    if not args["singleprocess"]:
        file_count = 0
        filepaths_amount = len(filepaths)
        while True:
            # Sleep to prevent one core running 100%
            time.sleep(0.05)
            ...
            if len(multiprocessing.active_children()) < CPU_COUNT and file_count != filepaths_amount:
                ...
                p = multiprocessing.Process(target=handle_filepath, args=(args, filepath, tlds))
                p.daemon = True
                p.start()
                file_count += 1
    else:
        for filepath in filepaths:
            handle_filepath(args, filepath, tlds)
