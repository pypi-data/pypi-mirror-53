...
class IOC:
    ...
    def init_tld(self):
        # Get all tlds
        if os.path.exists(self.tld_path):
            # Read from file if it exists
            with open(self.tld_path, "r") as f:
                tld_data = f.read()
        else:
            # Get data from iana if it does not exist
            try:
                resp = requests.get(self.tld_url)
            except:
                return False
            with open(self.tld_path, "w") as f:
                f.write(resp.text)
    ...
    def check_domain(self, ioc):
        ...
        if self.ioc_tld not in self.tlds:
            return False
