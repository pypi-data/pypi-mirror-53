...
class Price(GraphenePrice):
    """ This class deals with all sorts of prices of any pair of assets to
        ...
        each being an instance of :class:`iddex.amount.Amount`. The
        ...
        Way to obtain a proper instance:
            ...
            * ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`iddex.asset.Asset`
            ...
            * ``args`` being a list of ``[quote, base]`` both being instances of :class:`iddex.amount.Amount`
            * ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
            * ``base`` and ``quote`` being instances of :class:`iddex.asset.Amount`

        This allows instanciations like:

        * ``Price("0.315 USD/IDX")``
        * ``Price(0.315, base="USD", quote="IDX")``
        * ``Price(0.315, base=Asset("USD"), quote=Asset("IDX"))``
        ...
        * ``Price(1.0, "USD/GOLD")``
        ...
        .. code-block:: python
            ...
            >>> from iddex.price import Price
            >>> Price("0.3314 USD/IDX") * 2
            0.662600000 USD/IDX
    ...
    def market(self):
        """ Open the corresponding market
            ...
            :returns: Instance of :class:`iddex.market.Market` for the
...
class Order(Price):
    """ This class inherits :class:`iddex.price.Price` but has the ``base``
