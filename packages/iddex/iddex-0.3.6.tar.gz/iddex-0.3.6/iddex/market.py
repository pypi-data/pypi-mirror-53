...
from iddexbase import operations
...
class Market(dict):
    """ This class allows to easily access Markets on the blockchain for trading, etc.
        ...
        :param iddex.iddex.IDDEX blockchain_instance: IDDEX instance
        :param iddex.asset.Asset base: Base asset
        :param iddex.asset.Asset quote: Quote asset
        ...
        * ``base`` and ``quote`` are valid assets (according to :class:`iddex.asset.Asset`)
        * ``base:quote`` separated with ``:``
        * ``base/quote`` separated with ``/``
        * ``base-quote`` separated with ``-``

        .. note:: Throughout this library, the ``quote`` symbol will be
                  presented first (e.g. ``USD:IDX`` with ``USD`` being the
                  quote), while the ``base`` only refers to a secondary asset
                  for a trade. This means, if you call
                  :func:`iddex.market.Market.sell` or
                  :func:`iddex.market.Market.buy`, you will sell/buy **only
    ...
    def get_string(self, separator=":"):
        """ Return a formated string that identifies the market, e.g. ``USD:IDX``
    ...
    def ticker(self):
        """ Returns the ticker for all markets.
            ...
            * ``core_exchange_rate``: Core exchange rate for payment of fee in non-IDX asset
    ...
    def orderbook(self, limit=25):
        """ Returns the order book for a given market. You may also
            ...
            .. code-block:: js
                ...
                {'bids': [0.003679 USD/IDX (1.9103 USD|519.29602 IDX),
                0.003676 USD/IDX (299.9997 USD|81606.16394 IDX),
                0.003665 USD/IDX (288.4618 USD|78706.21881 IDX),
                0.003665 USD/IDX (3.5285 USD|962.74409 IDX),
                0.003665 USD/IDX (72.5474 USD|19794.41299 IDX)],
                'asks': [0.003738 USD/IDX (36.4715 USD|9756.17339 IDX),
                0.003738 USD/IDX (18.6915 USD|5000.00000 IDX),
                0.003742 USD/IDX (182.6881 USD|48820.22081 IDX),
                0.003772 USD/IDX (4.5200 USD|1198.14798 IDX),
                0.003799 USD/IDX (148.4975 USD|39086.59741 IDX)]}


            .. note:: Each bid is an instance of
                class:`iddex.price.Order` and thus carries the keys
    ...
    def accountopenorders(self, account=None):
        """ Returns open Orders
            ...
            :param iddex.account.Account account: Account name or instance of Account to show orders for in this market
        """
        if not account:
            if "default_account" in self.blockchain.config:
                account = self.blockchain.config["default_account"]
    ...
    def buy(
        ...
        **kwargs
        ...
        """ Places a buy order in a given market
            ...
            Prices/Rates are denoted in 'base', i.e. the USD_IDX market
            is priced in IDX per USD.

            **Example:** in the USD_IDX market, a price of 300 means
            a USD is worth 300 IDX

            .. note::

                All prices returned are in the **reversed** orientation as the
                market. I.e. in the BTC/IDX market, prices are IDX per BTC.
            ...
            .. warning::
                ...
                for. Example:
                    ...
                    * You place and order to buy 10 USD for 100 IDX/USD
                    * This means that you actually place a sell order for 1000 IDX in order to obtain **at least** 10 USD
