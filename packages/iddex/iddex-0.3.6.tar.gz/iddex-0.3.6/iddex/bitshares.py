...
from iddexapi.iddexnoderpc import IDDEXNodeRPC
from iddexbase import operations
from iddexbase.account import PublicKey
...
class IDDEX(AbstractGrapheneChain):
    """ Connect to the IDDEX network.
        ...
        * **Wallet Database**: Here, the iddexlibs load the keys from the
          locally stored wallet SQLite database (see ``storage.py``).
          To use this mode, simply call ``IDDEX()`` without the
          ``keys`` parameter
        * **Providing Keys**: Here, you can provide the keys for
          your accounts manually. All you need to do is add the wif
          keys for the accounts you want to use as a simple array
          using the ``keys`` parameter to ``IDDEX()``.
    ...
    def define_classes(self):
        ...
        self.rpc_class = IDDEXNodeRPC
        self.default_key_store_app_name = "iddex"
    ...
    def create_account(
        ...
        **kwargs
        ...
        """ Create new account on IDDEX

            The brainkey/password can be used to recover all generated keys
            (see `iddexbase.account` for more details.
        ...
        from iddexbase.account import PasswordKey, PublicKey
    ...
    def set_proxy(self, proxy_account, account=None, **kwargs):
        """ Set a specific proxy for account
            ...
            :param iddex.account.Account proxy_account: Account to be
                    proxied
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
    ...
    def vesting_balance_withdraw(self, vesting_id, amount=None, account=None, **kwargs):
        """ Withdraw vesting balance
            ...
            :param iddex.amount.Amount Amount: to withdraw ("all" if not
                provided")
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
    ...
    def publish_price_feed(
        ...
        """ Publish a price feed for a market-pegged asset
            ...
            :param iddex.price.Price settlement_price: Price for settlement
            :param iddex.price.Price cer: Core exchange Rate (default
                ``settlement_price + 5%``)
        ...
        ), "settlement_price needs to be instance of `iddex.price.Price`!"
        assert cer is None or isinstance(
            cer, Price
        ), "cer needs to be instance of `iddex.price.Price`!"
        if not account:
            if "default_account" in self.config:
                account = self.config["default_account"]
        if not account:
    ...
    def update_cer(self, symbol, cer, account=None):
        """ Update the Core Exchange Rate (CER) of an asset
            ...
            :param str symbol: Symbol of the asset to publish feed for
            :param iddex.price.Price cer: Core exchange Rate
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
    ...
    def reserve(self, amount, account=None, **kwargs):
        """ Reserve/Burn an amount of this shares
            ...
            :param iddex.amount.Amount amount: The amount to be burned.
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        ...
        **kwargs
        ...
        """ Create a worker
            ...
            **Required**
            ...
            :param iddex.amount.Amount daily_pay: The amount to be paid
                daily
            ...
            **Optional**
            ...
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        from iddexbase.transactions import timeformat
