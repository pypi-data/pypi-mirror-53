...
class XMLAttribute(XMLNode):
    ...
    def __hash__(self):
        textual_content_type = self.textual_content_type
        if textual_content_type is not None:
            textual_content_type = tuple(sorted(textual_content_type))
        textual_content_values = self.textual_content_values
        if textual_content_values is not None:
            textual_content_values = tuple(sorted(textual_content_values))
        return hash((
            self.namespace, self.textual_content_is_list,
            textual_content_type, textual_content_values,
        ))
