...
def find_cover_exons(read_block, exons, flank_len=20, small_exon_size=100):
    ...
    def merge_check(row, ith_intron, intron_start, intron_end):
        ...
        end_check = (not 0 in row.exon_ids) and (not row.exon_num-1 in row.exon_ids)
        if not end_check:
            return EMPTY_SS
        ...
        if not size_check:
            return EMPTY_SS
        left_tx_exon = exons.loc[(row.tx_i, leftmost_exon_id-1), ["exon_start", "exon_end"]]
        left_read_exon = [read_block.exons["exon_starts"][ith_intron], read_block.exons["exon_ends"][ith_intron]]
        left_check = interval_overlap(left_tx_exon, left_read_exon)
        right_tx_exon = exons.loc[(row.tx_i, rightmost_exon_id+1), ["exon_start", "exon_end"]]
        right_read_exon = [read_block.exons["exon_starts"][ith_intron+1], read_block.exons["exon_ends"][ith_intron+1]]
        right_check = interval_overlap(right_tx_exon, right_read_exon)
        overlap_check = left_check & right_check
        if not overlap_check:
            return EMPTY_SS
        realign_start = max(min(left_tx_exon[1], left_read_exon[1]) - flank_len, left_tx_exon[0], left_read_exon[0])
        realign_end = min(max(right_tx_exon[0], right_read_exon[0]) + flank_len, right_tx_exon[1], right_read_exon[1])
        realign_s = pd.Series({
        ...
        realign_s["delta_ratio"] = (realign_s["margin_len"] - realign_s["sum_exon_size"]) / realign_s["sum_exon_size"] 
        return realign_s
    #
    realign_list = []
    ...
    read_block.introns["intron_starts"], read_block.introns["intron_ends"]):
        ...
        if not cover_exon.empty:
            cond_cover_exon = cover_exon.groupby(level="tx_i").apply(condense_exon)
            for rt in cond_cover_exon.itertuples():
                realign_ss = merge_check(rt, ith_intron, intron_start, intron_end)
...
def find_missed_exons(read_block, bed_df, exons, ignore_strand=True, flank_len=20, small_exon_size=100):
    ...
    realign_list = find_cover_exons(read_block, exons, flank_len=flank_len, small_exon_size=small_exon_size)
    return realign_list
