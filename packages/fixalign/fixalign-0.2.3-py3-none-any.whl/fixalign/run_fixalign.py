...
def main():
    ...
    parser.add_argument("genomeFasta", help="Reference genome fasta file, with fai index under the same directory.")
    ...
    if not args.onlyRegion:
        if args.outBam is None:
            raise Exception("Require -o or --outBam if --onlyRegion flag is not set!")
    run_multiprocess(
