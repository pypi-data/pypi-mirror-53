...
def cal_alignment_score(cigar_list, read_seq, ref_seq, cm, mm, go, ge):
    ...
    for n, c in cigar_list:
        ...
        if c == 'M':
            ...
            raise Exception(
                "Error: cigar character exceed assumption {'M', '=', 'X', 'I', 'D', 'S', 'H', 'P', 'N'}!", c)
...
def find_fix(proc_region, ori_bam_path, out_bam_path, ref_fa_path, annot_bed, annot_exon,
    ...
    start_read_id = proc_region["start_read_id"]  # 0 based, left close right open
    end_read_id = proc_region["end_read_id"]
    if not only_region:
        out_bam_path_proc = out_bam_path + ".tmp{0}".format(proc_id)
        realign_bam = pysam.AlignmentFile(out_bam_path_proc, "wb", template=ori_bam)
    filter_list = []
    read_i = 0  # 0 based
    total_intron = 0
