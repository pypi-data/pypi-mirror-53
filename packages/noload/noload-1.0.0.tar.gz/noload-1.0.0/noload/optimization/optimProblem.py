...
def SLSQP(wrapper: Wrapper, x0, options):
    ...
    if (not result.success):  # TODO gérer une exception
        print("Optimization doesn't converge")  # TODO donner plus de détails sur les spec de cette optim
...
def LSSQ(wrapper: Wrapper, x0, options):
    ...
    result = scipyOpt.least_squares(fun=wrapper.f_val, x0=x0, bounds=bounds, jac=wrapper.f_grad_using_make_jvp, ftol = options['ftol'], max_nfev=options['maxiter'])
...
def optimizeParam(model: Callable[..., Dict], parameters:Dict, specifications:Spec, variable, range, outputs, ftol=0.001, disp=False, maxiter=50):
