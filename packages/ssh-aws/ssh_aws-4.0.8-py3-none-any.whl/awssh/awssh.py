...
def _connect(user, instance, args):
    ...
    command = 'ssh -i {key_path} {tunnel} {host} -o ConnectTimeout={timeout} -tt'.format(**config)

    if args.command:
        command = "{} '{}'".format(command, args.command)
...
def _jump_connect(user, ssh_instance, jump_server, args):
    ...
    command = 'ssh-add {key_path}; ' \
              ...
              '-J {jump_host} {target_host} -tt'.format(**command_tuple)
...
def create_parser():
    ...
    SSH, 'ProxyJump', create tunnels, and run commands on your AWS Linux/Unix infrastructure.
    "awssh --profile prod-acc-2 --region us-east-2 'rev-proxy' -c top --users fduran"
    "awssh --profile prod-acc-2" will attempt ssh with default users. The default user list is centos, ubuntu, and 
    ec2-user. Due to the nature of nargs, the option --users needs to be used last. For example: "awssh --users user1
    user2 'rev-proxy'" will not properly filter the results by 'instance-name'; instead try "awssh 'rev-proxy' --users 
    user1 user2". When the instance selected has no external IP, a "JumpHost" will be automatically chosen based on
    instances with external IPs that share the same ssh 'Key Name'. This is particularly useful if your project 
    instances share the same 'SSH Key'. You can also explicitly direct the JumpHost by providing two selections from 
    the list (i.e. Enter server number: <jump> <target>). MFA: when providing the --serial-number and --token-code
    options, awssh creates a profile [mfa] in your AWS credentials file (~/.aws/credentials or the file set by the
    AWS_SHARED_CREDENTIALS_FILE environment variable). You can continue using these temporary tokens until expiration
    with "awssh --profile mfa".
    """)
    ...
    parser.add_argument('--verbose', '-V', help='Prints instance and login details, as well as MFA output.',
                        action='store_true')

    mfa_group = parser.add_argument_group(title='MFA', description='If MFA is required, provide both arguments:')
