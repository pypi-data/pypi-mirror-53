...
def integrity_report(val):
    ...
    ct = val.dtypes.value_counts()
    flcount = ct[0]
    objcount = ct[1]
    intcount = ct[2]
    availablerows = df.shape[0] * len(val.columns)
    missing = (val.shape[0] * len(val.columns)) - sum(rows)

    catnum = pd.DataFrame({'CAT FEATURES': [catfeat], 'NUMERICAL FEATURES': [numfeat]})
    missdata = pd.DataFrame({'USEABLE DATA': [availablerows], 'MISSING VALUES': [missing]})
    piedata = pd.DataFrame({'OBJECTS': [objcount], 'INTEGERS': [intcount], 'FLOATS': [flcount]})

    labels1 = piedata.columns
    labels2 = missdata.columns
    labels3 = catnum.columns
    fig, (ax1, ax2, ax3) = plt.subplots(nrows=1, ncols=3, figsize=(16, 4))

    ax1.pie(piedata, labels=labels1, explode=None, radius=2.65, autopct='%1.1f%%', startangle=45)
    ax2.pie(missdata, labels=labels2, explode=None, radius=2.65, autopct='%1.1f%%', colors=['pink', 'lightcoral'],
            startangle=330)
    ax3.pie(catnum, labels=labels3, explode=None, radius=2.65, autopct='%1.1f%%', startangle=30)
