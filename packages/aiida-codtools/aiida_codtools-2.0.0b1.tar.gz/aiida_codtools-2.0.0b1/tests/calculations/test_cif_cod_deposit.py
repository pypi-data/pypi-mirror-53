...
def test_cif_cod_deposit(
    ...
    generate_cif_data, file_regression
):
    ...
    process, calc_info = generate_calc_job(fixture_sandbox_folder, entry_point_name, inputs)
    ...
    with fixture_sandbox_folder.open(process.inputs.metadata.options.input_filename) as handle:
        input_written = handle.read()

    with fixture_sandbox_folder.open(CifCodDepositCalculation.filename_config) as handle:
    ...
    assert sorted(fixture_sandbox_folder.get_content_list()) == sorted(expected_input_files)
