...
class DatasetBuilder:
    ...
    def __init__(self, location_builder=None, **kwargs):
        ...
        data_preview_type = self._data.get('data_preview_type')

        # XXX clean this up on next release. data_preview_type will be optional
        if sample_data:
            if not data_preview_type:
                data_preview_type = 'STATIC'

            if data_preview_type != 'STATIC':
                raise ValueError(
    ...
    def to_multipart_body(self):
        fields={
            'data': ('data', json.dumps(self.to_json_api()), 'application/json'),
        }
