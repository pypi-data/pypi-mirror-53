...
from azureml.datadrift._logging._telemetry_logger import \
    _TelemetryLogger
...
from azureml.datadrift._restclient.models import (AlertConfiguration,
                                                  DataDriftDto,
                                                  DataDriftRunDto)
...
class DataDriftDetector:
    ...
    def _initialize(self, workspace, model_name, model_version, services, compute_target_name=None, frequency=None,
        ...
        :type alert_config: azureml.datadrift._restclient.models.AlertConfiguration
    ...
    def _create_metric_dict(metric):
        ...
        :type metric: azureml.datadrift._datadiff.Metric
    ...
    def _decode_metric(json):
        def patch_jsonpickle_class_selector():
            ...
            def loadclass(module_and_name, classes=None):
                ...
                if ret is None:
                    if len(classes) == 1:
                        # Use specified class for deserialization
                        return list(classes.values())[0]

                    raise ValueError("Multiple classes specified, only one expected.")
                return ret

            jsonpickle.unpickler.loadclass = loadclass
        patch_jsonpickle_class_selector()

        return jsonpickle.decode(json, classes=Metric)
        ...
        for s in self.services:
            ...
            for f in metrics_list:
                with open(f, 'r') as metrics_json:
                    ...
                    metrics = metrics + DataDriftDetector._decode_metric(data)
