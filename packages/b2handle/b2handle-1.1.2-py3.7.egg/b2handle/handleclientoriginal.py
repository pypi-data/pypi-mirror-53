...
class EUDATHandleClient(object):
    ...
    def exchange_additional_URL(self, handle, old, new):
        ...
        if not self.is_URL_contained_in_10320LOC(handle, old, handlerecord_json):
            ...
            self.__exchange_URL_in_10320loc(old, new, list_of_entries, handle)
    ...
    def __exchange_URL_in_10320loc(self, oldurl, newurl, list_of_entries, handle):
        '''
            ...
            for index in python_indices:
                ...
                for element in all_URL_elements:
                    if element.get('href') == oldurl:
                        LOGGER.debug('__exchange_URL_in_10320loc: Exchanging URL ' + oldurl + ' from 10320/LOC.')
                        num_exchanged += 1
        ...
        if num_exchanged == 0:
            LOGGER.debug('__exchange_URL_in_10320loc: No URLs exchanged.')
        else:
            message = '__exchange_URL_in_10320loc: The URL "' + oldurl + '" was exchanged ' + str(num_exchanged) + \
