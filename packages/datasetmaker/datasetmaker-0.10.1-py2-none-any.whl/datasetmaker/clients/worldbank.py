...
class WorldBank(Client):
    ...
    def _get_wbi(self, code: str, **kwargs: Any) -> pd.DataFrame:
        ...
        kwargs.update({'format': 'json', 'page': 1, 'mrv': self.n_latest_years})
        last_page = -1
        data: list = []
    ...
    def get(self) -> pd.DataFrame:
        ...
        df = df.pivot_table(index=['country', 'year'], values='value', columns='indicator')
        df = df.reset_index()

        self.data = df
        return df

    def save(self, path: Union[Path, str], recursive_concepts: bool = True, **kwargs: str) -> None:
