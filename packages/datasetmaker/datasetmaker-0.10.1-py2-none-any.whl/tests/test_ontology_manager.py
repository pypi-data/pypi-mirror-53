...
class TestOntologyManager(unittest.TestCase):
    ...
    def test_read_concepts_looks_up_recursively(self):
        concepts = manager.read_concepts('nobel_born_country')
        self.assertTrue((concepts.concept == 'country').any())
        self.assertTrue(concepts.shape[0] > 1)
    ...
    def test_sid_to_id(self):
        ...
        self.assertEqual(mapping['bornCity'], 'nobel_born_city')

    def test_id_to_sid(self):
        mapping = manager.id_to_sid(source='nobel')
        self.assertIs(type(mapping), dict)
        self.assertEqual(mapping['nobel_died'], 'died')

    def test_id_to_name(self):
        mapping = manager.id_to_name(source='nobel')
        self.assertIs(type(mapping), dict)
        self.assertEqual(mapping['nobel_died_country'], 'Died in country')

    def test_map(self):
        country_to_iso3 = manager._map('country', ['iso3'], 'name')
