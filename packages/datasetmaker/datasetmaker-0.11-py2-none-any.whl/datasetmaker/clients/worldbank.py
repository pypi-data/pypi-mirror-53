...
class WorldBank(Client):
    ...
    def _get_wbi(self, code: str, **kwargs: Any) -> pd.DataFrame:
        ...
        while last_page != kwargs["page"]:
            ...
            resp_data = resp.json()
            if len(resp_data) == 1 and 'message' in resp_data and resp_data['message'][0]['id'] == '175':
    ...
    def get(self, indicators: list, n_latest_years: Optional[int]) -> pd.DataFrame:
        data = []
        self.indicators = indicators
        if n_latest_years:
            self.n_latest_years = n_latest_years
