...
from datasetmaker.utils import stretch
...
@lru_cache()
def read_entity(name: str) -> pd.DataFrame:
    """
...
def entity_exists(name: str) -> bool:
    return (DDF_DIR / f'ddf--entities--{name}.csv').exists()
...
def _map(entity_name: str, keys: list, value: str) -> MutableMapping:
    """
    ...
    keys : list, entity fields to map from
    ...
    for key in keys:
        if df[key].str.contains(';').any():
            frame = df.pipe(stretch, index_col=value,
                            value_col=key, sep=';')
        else:
