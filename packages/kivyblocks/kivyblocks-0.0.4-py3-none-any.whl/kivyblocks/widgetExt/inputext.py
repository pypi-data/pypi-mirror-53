...
from kivyblocks.kivysize import KivySizes
...
class StrInput(TextInput):
	...
	def __init__(self,**kv):
		...
		a = {
			"allow_copy":True,
			"border":[2,0,0,0],
			"font_size":sp(fntsize),
			"multiline":False,
			"size_hint":[None,None],
			"width":sp(120),
			"halign":"left",
			"hint_text":"",
			"height":sp(int(fntsize*1.8))
		}
		if kv.get('tip'):
			a['hint_text'] = kv['tip']
		a.update(kv)
...
class SelectInput(BoxLayout):
	def __init__(self,**kw):
		super(SelectInput,self).__init__(orientation='horizontal',						size_hint_y=None,height=sp(32))
		self.tinp = StrInput()
