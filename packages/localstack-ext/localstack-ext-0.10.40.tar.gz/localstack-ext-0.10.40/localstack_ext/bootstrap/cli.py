...
def import_auth():
 try:
  ...
  raise KUlwp('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=KUlwe)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=KUlwT)
def get_params(args,names=[],mandatory=KUlwe):
 result=()
 for name in names:
  value=KUlwy
  if name in args:
  ...
  if not value and mandatory:
   raise KUlwp('Please provide %s=... parameter'%name)
  result+=(value)
 if KUlwt(result)==1:
  return result[0]
 return result
