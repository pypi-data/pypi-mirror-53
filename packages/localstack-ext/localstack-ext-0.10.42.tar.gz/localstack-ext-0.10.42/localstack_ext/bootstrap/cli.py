...
def import_auth():
 try:
  ...
  raise xBlcd('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=xBlca)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=xBlco)
def get_params(args,names=[],mandatory=xBlca):
 result=()
 for name in names:
  value=xBlcy
  if name in args:
  ...
  if not value and mandatory:
   raise xBlcd('Please provide %s=... parameter'%name)
  result+=(value)
 if xBlcM(result)==1:
  return result[0]
 return result
