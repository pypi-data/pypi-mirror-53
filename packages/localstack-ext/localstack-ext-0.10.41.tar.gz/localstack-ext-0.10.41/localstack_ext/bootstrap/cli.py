...
def import_auth():
 try:
  ...
  raise tjcQb('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=tjcQJ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=tjcQw)
def get_params(args,names=[],mandatory=tjcQJ):
 result=()
 for name in names:
  value=tjcQB
  if name in args:
  ...
  if not value and mandatory:
   raise tjcQb('Please provide %s=... parameter'%name)
  result+=(value)
 if tjcQK(result)==1:
  return result[0]
 return result
