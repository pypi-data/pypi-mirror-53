...
def import_auth():
 try:
  ...
  raise LJMsE('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=LJMsk)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=LJMsv)
def get_params(args,names=[],mandatory=LJMsk):
 result=()
 for name in names:
  value=LJMsB
  if name in args:
  ...
  if not value and mandatory:
   raise LJMsE('Please provide %s=... parameter'%name)
  result+=(value)
 if LJMsS(result)==1:
  return result[0]
 return result
