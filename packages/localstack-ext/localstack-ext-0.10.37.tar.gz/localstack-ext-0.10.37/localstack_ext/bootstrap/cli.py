...
def import_auth():
 try:
  ...
  raise DJlTb('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=DJlTX)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=DJlTB)
def get_params(args,names=[],mandatory=DJlTX):
 result=()
 for name in names:
  value=DJlTp
  if name in args:
  ...
  if not value and mandatory:
   raise DJlTb('Please provide %s=... parameter'%name)
  result+=(value)
 if DJlTH(result)==1:
  return result[0]
 return result
