...
def import_auth():
 try:
  ...
  raise jPMRg('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=jPMRK)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=jPMRB)
def get_params(args,names=[],mandatory=jPMRK):
 result=()
 for name in names:
  value=jPMRQ
  if name in args:
  ...
  if not value and mandatory:
   raise jPMRg('Please provide %s=... parameter'%name)
  result+=(value)
 if jPMRC(result)==1:
  return result[0]
 return result
