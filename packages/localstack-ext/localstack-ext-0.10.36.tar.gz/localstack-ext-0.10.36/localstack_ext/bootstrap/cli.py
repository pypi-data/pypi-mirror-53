...
def import_auth():
 try:
  ...
  raise RWJNB('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RWJNe)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RWJNV)
def get_params(args,names=[],mandatory=RWJNe):
 result=()
 for name in names:
  value=RWJNU
  if name in args:
  ...
  if not value and mandatory:
   raise RWJNB('Please provide %s=... parameter'%name)
  result+=(value)
 if RWJNx(result)==1:
  return result[0]
 return result
