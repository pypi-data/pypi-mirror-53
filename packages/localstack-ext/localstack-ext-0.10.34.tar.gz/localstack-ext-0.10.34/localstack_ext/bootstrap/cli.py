...
def import_auth():
 try:
  ...
  raise Sgnmw('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=SgnmQ)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=Sgnmo)
def get_params(args,names=[],mandatory=SgnmQ):
 result=()
 for name in names:
  value=SgnmB
  if name in args:
  ...
  if not value and mandatory:
   raise Sgnmw('Please provide %s=... parameter'%name)
  result+=(value)
 if Sgnmq(result)==1:
  return result[0]
 return result
