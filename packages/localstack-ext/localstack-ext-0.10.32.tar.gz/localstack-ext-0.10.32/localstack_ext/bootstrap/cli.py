...
def import_auth():
 try:
  ...
  raise cBgYh('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=cBgYo)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=cBgYF)
def get_params(args,names=[],mandatory=cBgYo):
 result=()
 for name in names:
  value=cBgYU
  if name in args:
  ...
  if not value and mandatory:
   raise cBgYh('Please provide %s=... parameter'%name)
  result+=(value)
 if cBgYN(result)==1:
  return result[0]
 return result
