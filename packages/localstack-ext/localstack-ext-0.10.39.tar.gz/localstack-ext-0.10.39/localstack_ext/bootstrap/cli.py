...
def import_auth():
 try:
  ...
  raise APWUQ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=APWUk)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=APWUX)
def get_params(args,names=[],mandatory=APWUk):
 result=()
 for name in names:
  value=APWUg
  if name in args:
  ...
  if not value and mandatory:
   raise APWUQ('Please provide %s=... parameter'%name)
  result+=(value)
 if APWUD(result)==1:
  return result[0]
 return result
