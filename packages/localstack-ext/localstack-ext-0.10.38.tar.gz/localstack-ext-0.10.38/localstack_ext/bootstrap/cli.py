...
def import_auth():
 try:
  ...
  raise NbspB('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=Nbspc)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=NbspE)
def get_params(args,names=[],mandatory=Nbspc):
 result=()
 for name in names:
  value=NbspM
  if name in args:
  ...
  if not value and mandatory:
   raise NbspB('Please provide %s=... parameter'%name)
  result+=(value)
 if Nbspl(result)==1:
  return result[0]
 return result
