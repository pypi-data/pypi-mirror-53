...
def influx_group_query(idbclient: InfluxDBClient, start: int, end: int):
    ...
    # influxql request :
    query = "select sum(bytes) as bytes into stage2.ringserverstats from autogen.ringserverevents where time>=%ds and time<=%ds group by network,station,location,channel,country,city,client,time(1d) fill(none)"%(start, end)
    ...
    query = "select sum(bytes) as bytes into stage3.ringserverstats from autogen.ringserverevents where time>=%ds and time<=%ds group by network,station,location,channel,country,time(1w) fill(none)"%(start, end)
    logger.debug("Sending grouping query: "+query)
    try:
        result = idbclient.query(query)
        logger.info(("Result: {0}".format(result)))
...
def influxdb_send_data(idbclient: InfluxDBClient, data: List[Dict]) -> bool:
    ...
    """
    try:
        idbclient.write_points(data, time_precision='s', retention_policy='autogen')
    except Exception as e:
    ...
    for f in files:
        ...
        if dbhost != None :
            ...
            try:
                ...
                logger.error(e)
    # Now delete from ringserverevents, its a temporary buffer
    query = "delete from ringserverevents"
    logger.debug("Sending query: "+query)
    try:
        result = client.query(query)
        logger.info(("Result: {0}".format(result)))
    except Exception as e:
        logger.error("Error cleaning ringserverevents")
        logger.error(e)
    client.close()
