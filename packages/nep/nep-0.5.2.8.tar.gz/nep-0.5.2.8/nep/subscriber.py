...
class subscriber:
    ...
    def __network_selection(self):
        ...
        if self.network == "direct":
            ...
            success, port, ip  = nep.masterRegister(self.node, self.topic, master_ip = '127.0.0.1', master_port = 7000, socket = "subscriber", mode = self.mode, pid = self.pid, data_type = self.msg_type)
            if self.topic != "/nep_node":
