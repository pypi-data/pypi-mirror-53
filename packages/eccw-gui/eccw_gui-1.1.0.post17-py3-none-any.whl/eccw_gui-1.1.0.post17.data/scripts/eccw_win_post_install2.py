# postinstall script for eccw
"""Create ECCW start menu and desktop entries"""
...
def install():
    ...
    script = osp.abspath(osp.join(sys.prefix, 'scripts', 'eccw'))
    ...
    ico_dir = osp.join(lib_dir, 'eccw', 'windows')
    ...
    fname = osp.join(start_menu, 'eccw (full).lnk')
    create_shortcut(python, desc, fname, '"%s"' % script, workdir,
                    osp.join(ico_dir, 'eccw.ico'))
    file_created(fname)

    fname = osp.join(start_menu, 'ECCW-Reset all settings.lnk')
    create_shortcut(python, 'Reset ECCW settings to defaults',
