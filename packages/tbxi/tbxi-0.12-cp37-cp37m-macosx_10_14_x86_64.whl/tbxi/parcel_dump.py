...
from .powerpc_dump import is_powerpc, dump_powerpc
...
def dump_parcels(binary, dest_dir, depth=-1):
    os.makedirs(dest_dir, exist_ok=True)
    print('Parcels package into %s' % dest_dir)
    ...
    for tpl, filename in decision.items():
        ...
        if depth != 0:
            if is_powerpc(data):
                dump_powerpc(data, path.join(dest_dir, filename + '.src'), depth=depth-1)
