...
# TODO: Refactoring
# TODO: add suport for simultaneaus inversion of all events
...
def gerr(data, axis=None):
    ...
    lmean = np.nanmean(np.log(data), axis=axis)
    lstd = np.nanstd(np.log(data), axis=axis)
...
def seismic_moment(freq, omM, M0_freq):
    ...
    """
    M0 = [o for f, o in zip(freq, omM) if f < M0_freq and o is not None]
    if len(M0) > 0:
        return gmean(M0)
...
def codamagf(freq_band, streams,
    ...
    for stream in streams:
        ...
        if freqmax > 0.5 * sr:
            msg = '%s: High corner frequency is above Nyquist -> skip pair'
...
#            log.debug(msg, station, son_old - origin_time, son - origin_time)
    ...
    for energy in energies:
        ...
        if dt is None:
            ...
            Nc = Nc // samples
            if Nc == 0:
                msg = '%s: coda window shorter than %d samples -> skip pair'
        ...
        try:
            ...
            msg = '%s: cannot get S-onset window (%s) -> skip pair'
            log.warning(msg, pair, ex)
            remove_pairs.append(pair)
            Eobsc.pop(-1)
            tc.pop(-1)
    ...
    # https://de.wikipedia.org/wiki/Methode_der_kleinsten_Quadrate#Verallgemeinerte_Kleinste-Quadrate-Modelle
...
def codamag(events, inventory, get_waveforms,
    ...
    msg = 'succesfully requested %d streams for %d stations and %d event(s)'
