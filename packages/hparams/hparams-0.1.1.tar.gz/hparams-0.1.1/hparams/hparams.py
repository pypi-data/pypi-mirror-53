...
# TODO: Ensure the example from `Hyperparameter CLI` works. We want to ensure that you can
# `add_config` before `configurable` is set. We could potentially relavalidate later.
# We want to ensure that this works: `--torch.optim.adam.Adam.__init__ HParams(betas=(0.999,0.99))`
# We want to ensure that similar checks run during `configurable` like `add_config`, maybe?
# TODO: Add enough documentation that we don't need READTheDocs
# TODO: Comment on known bugs like the super class in __main__ one.
