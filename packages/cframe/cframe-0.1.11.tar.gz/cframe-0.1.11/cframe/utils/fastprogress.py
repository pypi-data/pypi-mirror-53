...
class NBMasterBar(MasterBar):
    ...
    def update_graph(self, loss_graphs, lr_graphs, lr_bounds, x_bounds=None, y_bounds=None, figsize=(16,4)):
        if self.hide_graph: return
        if not hasattr(self, 'fig'):
            self.fig, self.ax = plt.subplots(nrows=1, ncols=2, figsize=figsize)
            self.out2 = display(self.fig, display_id=True)

        self.ax[0].clear()
        if len(self.names) < len(loss_graphs): self.names += [''] * (len(loss_graphs) - len(self.names))
        for g,n in zip(loss_graphs, self.names): self.ax[0].plot(*g, label=n)
        self.ax[0].legend(loc='upper right')
        if x_bounds is not None: self.ax[0].set_xlim(*x_bounds)
        if y_bounds is not None: self.ax[0].set_ylim(*y_bounds)

        self.ax[1].clear()
        self.ax[1].plot(*lr_graphs, label='lr')
        if x_bounds is not None: self.ax[1].set_xlim(*x_bounds)
        self.ax[1].set_ylim(*lr_bounds)
