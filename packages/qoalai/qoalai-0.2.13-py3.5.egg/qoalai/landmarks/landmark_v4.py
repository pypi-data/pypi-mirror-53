...
class Landmark(object):
    def __init__(self,
        ...
        self.point_position_placeholder = tf.placeholder(tf.float32, shape=(None, self.grid_num, self.grid_num, 2))
        self.point_map_placeholder = tf.placeholder(tf.float32, shape=(None, self.grid_num, self.grid_num, 1))
    ...
    def build_densenet_base(self, input_tensor,
        ...
        self.point_position = tf.nn.sigmoid(out[:, :, :, 1:])
    ...
    def calculate_loss(self):
        ...
        Returns:
            [type] -- [description]
        """
        self.point_map_loss = mse_loss_sum(self.point_map, self.point_map_placeholder)

        self.point_map = tf.math.greater(self.point_map, tf.convert_to_tensor(np.array(self.threshold), tf.float32))
        self.point_map = tf.cast(self.point_map, tf.float32)
        self.point_position = self.point_position * self.point_map
        self.point_position_placeholder = self.point_position_placeholder * self.point_map
    ...
    def read_target(self, file_path):
        ...
        # ----------------------------------- #
        point_map_lable = np.zeros((self.grid_num, self.grid_num, 3))
        ...
        for i in range(self.grid_num):
            for j in range(self.grid_num):
                ...
                point_map_lable[i, j, 1] = (x%self.grid_size_relative) / self.grid_size_relative
                point_map_lable[i, j, 2] = (y%self.grid_size_relative) / self.grid_size_relative
