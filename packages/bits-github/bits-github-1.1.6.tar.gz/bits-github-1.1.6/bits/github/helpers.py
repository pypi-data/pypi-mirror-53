...
class Helpers(object):
    ...
    def get_org_teams_members(self):
        """Return a list of org teams members."""
        teams_members = []
        for t in sorted(self.github.get_org_teams(), key=lambda x: x['slug'].lower()):
            ...
            # firestore.db.collection('github_teams_members').document(str(tid)).set(team)
            if self.verbose:
                print('Updated team members: {}'.format(slug))

        return teams_members

    def get_org_teams_repos(self):
        """Return a list of org teams repos."""
        teams_repos = []
        for t in sorted(self.github.get_org_teams(), key=lambda x: x['name'].lower()):
            tid = t['id']
            slug = t['slug']
            ...
            # firestore.db.collection('github_teams_repos').document(str(tid)).set(team)
            if self.verbose:
                print('Updated team repos: {}'.format(slug))
