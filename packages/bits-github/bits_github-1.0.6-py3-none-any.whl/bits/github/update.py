...
class Update(object):
    ...
    def _get_firestore_github_users(self):
        ...
        for doc in firestore.db.collection('github_users').stream():
            github_id = doc.id
    ...
    def update_user(self, user):
        ...
        last_modified = None
        if self.github_users.get(github_id):
            current = self.github_users[github_id]
            updated_at = dateparser.parse(current['updated_at'])
        ...
        print('Updated user: {}'.format(user['login'].lower()))
    ...
    def update_users(self):
        ...
        for email in sorted(self.users, key=lambda x: self.users[x]['login'].lower()):
            user = self.users[email]
            new = self.update_user(user)
            github_users.append(new)
            if new != user:
                updates.append(new)

        if self.verbose:
            print('Updated {} github_users.'.format(len(updates)))
