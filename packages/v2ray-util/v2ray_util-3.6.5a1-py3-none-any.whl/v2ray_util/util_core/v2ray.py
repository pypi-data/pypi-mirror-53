...
class V2ray:
    ...
    def docker_run(command, keyword):
        try:
            ...
            if subprocess.check_output("tail -n 1 /.run.log |grep started", shell=True) or keyword == "stop":
    ...
    def status():
        if os.path.exists("/.dockerenv"):
            if subprocess.check_output("tail -n 1 /.run.log |grep started", shell=True):
                print(ColorStr.green("v2ray running.."))
            else:
                print(ColorStr.red(bytes.decode(subprocess.check_output("cat /.run.log"))))
        else:
    ...
    def restart(cls):
        if os.path.exists("/.dockerenv"):
            cls.docker_run("killall v2ray 2>/dev/null && /usr/bin/v2ray/v2ray -config=/etc/v2ray/config.json > /.run.log &", "restart")
        else:
    ...
    def start(cls):
        if os.path.exists("/.dockerenv"):
            cls.docker_run("killall v2ray 2>/dev/null && /usr/bin/v2ray/v2ray -config=/etc/v2ray/config.json > /.run.log &", "start")
        else:
    ...
    def stop(cls):
        if os.path.exists("/.dockerenv"):
            cls.docker_run("killall v2ray 2>/dev/null", "stop")
        else:
