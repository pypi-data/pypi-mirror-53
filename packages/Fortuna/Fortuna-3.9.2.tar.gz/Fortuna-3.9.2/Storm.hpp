...
namespace Storm {  // Version 3.2.1
    ...
    namespace GearBox {
        ...
        auto analytic_continuation(Func && func, Integer input, Integer offset) -> Integer {
            if (input < 0) return -func(-std::max(input, -std::numeric_limits<Integer>::max())) + offset;
        ...
        template <typename Func>
        auto approximation_clamp(Func && approximation, Integer target, Integer upper_bound) -> Integer {
    ...
    auto binomial_variate(Integer number_of_trials, double probability) -> Integer {
    ...
    auto negative_binomial_variate(Integer number_of_trials, double probability) -> Integer {
    ...
    auto geometric_variate(double probability) -> Integer {
        std::geometric_distribution<Integer> distribution { std::clamp(probability, 0.0, 1.0) };
        return distribution(Engine::Hurricane);
    }

    auto poisson_variate(double mean) -> Integer {
    ...
    auto plus_or_minus_gauss(Integer number) -> Integer {
        ...
        const auto num { std::abs(number) };
        const auto result { Integer(std::round(Storm::normal_variate(0.0, num / PI))) };
    ...
    auto front_gauss(Integer number) -> Integer {
        if (number > 0) {
            const auto result { Integer(std::floor(Storm::gamma_variate(1.0, number / 10.0))) };
    ...
    auto middle_gauss(Integer number) -> Integer {
        if (number > 0) {
            const auto result { Integer(std::floor(Storm::normal_variate(number / 2.0, number / 10.0))) };
