    ...
    if not in_string_or_comment:#This is just for visual purposes, so I can put the lines in a block of code and document ,after_line)it
      ...
      #region misc tweaks
        ...
        if char=='\n' and after=='' and before_line.endswith(';'):
            ...
            buffer.insert_text('\n')
            return True
        if char!='=' and re.fullmatch(r'.*\=',before_line) and starts_with_any(after_line,']',')','}') and not (len(after_line.strip())==2 and after_line.strip()[1]==':'):
            ...
            buffer.cursor_right()
            buffer.insert_text('=')
            if starts_with_any(after_line,*'})'):
                buffer.insert_text('=')#Can't assign like f(x)=4, but can do f(x)==4
      #endregion
    return False
...
def load_python_bindings(python_input):
    ...
    def _(event):
        ...
        if endswithany(before_line.lstrip(),'.','(','[') and not after_line.strip():# the .=, (=, and [= operators
