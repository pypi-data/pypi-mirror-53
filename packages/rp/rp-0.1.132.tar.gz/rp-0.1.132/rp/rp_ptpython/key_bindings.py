...
def handle_character(buffer,char):
    ...
    if not in_string_or_comment:#This is just for visual purposes, so I can put the lines in a block of code and document ,after_line)it
      ...
      #region pluralize list comprehension (must come before space stoppers)
        if char==' ' and after_line.startswith(']') and re.fullmatch(r'.*\[[^\[\]]* for [a-zA-Z_0-9]+ in ',before_line):
            ...
            # return True 
        if char==' ' and before_line.endswith(' for ') and starts_with_any(after_line,' in]','in)','in}',' in '):
