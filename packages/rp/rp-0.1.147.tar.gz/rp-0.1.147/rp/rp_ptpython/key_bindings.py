...
def handle_character(buffer,char):
    ...
    if not in_string_or_comment:#This is just for visual purposes, so I can put the lines in a block of code and document ,after_line)it
      ...
      #region misc tweaks
        if char in 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_' and before=='5' and not after:
            #For ipython magics, turn 5 into % if at the beginning of a line
            #`5edit` --->  `%edit`
            buffer.delete_before_cursor()
            buffer.insert_text('%')
