...
def healpy2astrotools_angles(phi, theta):
    pass


def astrotools2healpy_angles(phi, theta):
    theta_hp = np.pi / 2. - theta
    phi_hp = phi
    return phi_hp, theta_hp
...
def equatorial_scrambling_lon_lat(v, n=1):
    """
    Performs a scrambling of the input vectors v around the z-axis. In this sense it keeps the
    equatorial declination while assigning new uniform azimuth angles.

    :param v: Input vectors in equatoial coordinates and shape (3, ncrs)
    :param n: Number of scrambled output sets
    :return: scrambled vectors in shape (3, ncrs, nsets)
    """
    lon, lat = vec2ang(v)
    return ang2vec(rand_phi((v.shape[1], n)), lat[:, np.newaxis])


def galactic_scrambling(v, n=1000):
    pass
