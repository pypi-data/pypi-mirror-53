...
from .utils import is_unicode, to_unicode_safe
...
def str_included(haystack, needle, min_needle_length=0):
    ...
    if is_unicode(haystack):
        ...
        if not isinstance(haystack, bytes):
            # raise if we don't find anything (we were not expected this type at first)
