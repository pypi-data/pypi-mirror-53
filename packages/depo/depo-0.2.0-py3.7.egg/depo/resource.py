...
class Requestor:
    def request(self, method, data={}):
        ...
        headers = {'User-Agent': 'depo-python/' + VERSION,
        ...
        try:
            if data:
                ...
                response = requests.get(url=url, headers=headers,
                                        timeout=30, auth=depo.api_credentials)
...
class Order:
    ...
    def create(cls, place, recipient_name, recipient_phone, recipient_email, total_amount, products_amount, reference=None):
        if (isinstance(place, int) or isinstance(place, str) or
                isinstance(place, unicode)):
    ...
    def cancel(cls, order_id):
        client = Requestor()
        response = client.request('packages/cancel', data={
            'number': order_id
        })
        return response.json()
...
class Place:
    def __init__(self, data={}):
        ...
        self.latitude = Decimal(
            data.get('latitude', 0)).quantize(Decimal('.0000001')
        )
        self.longitude = Decimal(
            data.get('longitude', 0)).quantize(Decimal('.0000001')
        )
