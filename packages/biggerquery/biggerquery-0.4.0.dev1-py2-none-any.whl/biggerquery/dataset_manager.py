...
class TemplatedDatasetManager(object):
    ...
    def estimate_costs(self, sql, custom_run_datetime=None):
        return self.dataset_manager.estimate_costs(sql.format(**self.template_variables(custom_run_datetime)))
...
class PartitionedDatasetManager(object):
    ...
    def estimate_costs(self, sql, custom_run_datetime=None):
        return self._dataset_manager.estimate_costs(sql, custom_run_datetime)
...
class DatasetManager(object):
    ...
    def collect(self, sql):
        return self._collect(sql).to_dataframe()

    def estimate_costs(self, sql):
        job_config = bigquery.QueryJobConfig()
        job_config.dry_run = True
        query_job = self._collect(sql, job_config=job_config)
    ...
    def _collect(self, sql, job_config=None):
