def importFromParent(file: str, extension: str = ''):
    ...
    if float(no_slash) == 1.0: # if there's no slash in string "file":
        ...
        if extension == '':
            os.system(f'mkdir "{fst_path}\\__temp__{filename}{extension}"')
        else:
            extension = '.' + extension
        os.chdir('..')
        os.chdir(faf[:len(faf)-len(filename)])
        os.system(f'copy /b "{filename}{extension}" "{fst_path}\\__temp__{filename}{extension}"')
        faf = faf.replace('/','.')
        faf = faf[:faf.find('.')]
        exec(f'import __temp__{filename} as {filename}')
        exec(f'return {filename}')
