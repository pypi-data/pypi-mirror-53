def importFromParent(file: str, extension: str = '', file_dir: str = ''):
    import os
    if file_dir != '':
        os.chdir(file_dir)
    no_slash = 0
    ...
    if float(no_slash) == 1.0: # if there's no slash in string "file":
        ...
        if extension == '':
            os.system(f'mkdir "{fst_path}\\__pycache__\\{fm}"');os.system('cls');
        else:
            os.system(f'mkdir "{fst_path}\\__pycache__"');os.system('cls');
            extension = '.' + extension
        os.chdir('..')
        os.chdir(faf[:len(faf)-len(filename)])
        os.system(f'copy /b "{filename}{extension}" "{fst_path}\\__pycache__\\{fm}{extension}"');os.system('cls');
        faf = faf.replace('/','.')
        faf = faf[:faf.find('.')]
