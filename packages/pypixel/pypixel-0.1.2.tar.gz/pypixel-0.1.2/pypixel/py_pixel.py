...
class PyPixel:
    ...
    class Image:
        ...
        def reshape(self,size):
            img=Image.fromarray(self.__arr).resize(size)
            return PyPixel.Image(np.array(img))

        def save(self,path):
            img=Image.fromarray(self.__arr)
            img.save(path)
        ...
        def gray(self,method="F"):
            ...
            return PyPixel.Image(np.reshape(result,(self.__arr.shape[0],self.__arr.shape[1])))
            ...
            for i in range(len(im)):
                for j in range(len(im[i])):
                    if im[i][j]<threshold:
                        ...
                        im[i][j]=255
            return PyPixel.Image(im)

        def show(self):
            Image.fromarray(self.__arr).show()
