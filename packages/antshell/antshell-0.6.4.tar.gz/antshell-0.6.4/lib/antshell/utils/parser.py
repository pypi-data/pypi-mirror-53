...
def load_argParser():
    ...
    usage = """%(prog)s [ -h | --version ] [-l [-m 2] ]
        [ v | -n 1 | -s 'ip|name' ] [ -A ] [ -B ] 
        [ -e | -d | -a ip [--name tag | --user root | --passwd xx | --port 22 | --sudo root ] ]
        [ -P -c 'cmd1,cmd2,...' ]
    ...
    g2.add_argument("--name",dest="name",action="store",type=str,help=lang["name"],metavar="name")
    g2.add_argument("--user",dest="user",action="store",type=str,help=lang["user"],metavar="root")
    g2.add_argument("--passwd",dest="passwd",action="store",type=str,help=lang["passwd"],metavar="******")
