...
class HostHandle(ParaTools, SShTools):
    '''
    主机交互连接处理
    '''
    ...
    def delList(self, hosts):
        ...
        while not delDone:
            msg = """Confirm [ Y|N ] >> """
            delDone = (input(self.colorMsg(c="green", flag=True).format(msg)).upper() == "Y" )
    ...
    def editList(self, hosts):
        ...
        while not editDone:
            ...
            sudo = self.getInfo(title="Sudo", value=option.sudo, default=hosts["sudo"], isNone=True)
    ...
    def connect(self):
        ...
        if not option.num:
            ...
            while not option.num:
                try:
                    msg = """\nInput choose or [ 'q' | ctrl-c ] to quit!\nServer [ ID | IP | NAME ] >> """
