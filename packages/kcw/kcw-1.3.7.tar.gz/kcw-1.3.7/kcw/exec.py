...
class Exec:
    ...
    def init(self,config=None):
        ...
        plists=['view',conf.app['template_folder'],conf.app['static_folder']] #模块下要执行的文件夹
        for sourcep in plists:
            if not os.path.exists(path+'/'+sourcep):
                os.makedirs(path+"/"+sourcep)
            if sourcep=='view':
                self.__copyview(sourcep)
            elif sourcep=='config' or sourcep=='common' or sourcep==conf.app['template_folder'] or sourcep==conf.app['static_folder']:
