...
class Analytics(object):
    ...
    def sample(self, providers, inputs_dframe, validate=True):
        return track_function(lambda: self.__do_batch(providers, inputs_dframe, validate=validate),
                              self.__config, "API : Python : Sample",
                              { "provider_names": providers })

    def __do_batch(self, providers, inputs_dframe, validate=True):
        print("Sample will only use the first 5 records of your dataframe", file=sys.stderr)
        inputs_dframe = inputs_dframe.head(5)
        inputs_dframe = self.__stringify_dframe(inputs_dframe)

        if (validate):
            if (not self.__validate_providers(providers, inputs_dframe)):
                return None
        ...
        for idx, row in progressbar(inputs_dframe.iterrows()):
            ...
            no_failues = True
            try:
                no_failures = self.__connectors.fetch(providers, inputs)
    ...
    def __enrich_csv(self, providers, csv_file, columns, all_updates, fixed_list_size):
        ...
        input_id = self.__create_batch(csv_file, s3_key, columns, region_id)
    ...
    def __create_batch(self, csv_file, s3_key, columns, region_id):
