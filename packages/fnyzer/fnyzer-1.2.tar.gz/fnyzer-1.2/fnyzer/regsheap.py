...
class regsheap:
    ...
    def mul(X):
        return 2*X[:,1]*X[:,2]
    HXTproregs = {   # Limits of the HXT regions to be created
        'GlCi': [0, 5, 10, 12.5],
        'GlCo': [6, 7, 8]
    }
    HXTregsheap = regsheap(HXTproregs, mul, {'lap': 'z', 'GlCi': 'a', 'GlCo':'b'},
                           maxregs = 16, maxmsr = 0.1, sampoints= 8, pref = 'regHXT')
