...
class SortedSetField(MultiValuesField):
    ...
    def _call_zadd(self, command, *args, **kwargs):
        """
        Normal redis-py 3+ signagure: mapping, nx=False, xx=False, ch=False, incr=False
        So the normal way in redis-py 3+ to pass data to zadd is to pass a mapping.
        But to avoid too much rewritting when using the version of limpyd supporting
        ...
        If unnamed arguments are passed, we pass them as is to the original zadd method.
        Same if mapping is passed as a named argument.
    ...
    @classmethod
    def coerce_zadd_args(cls, *args, **kwargs):
        """
        Take arguments attended by a zadd call, named or not, and return args that can be used
        directly..
