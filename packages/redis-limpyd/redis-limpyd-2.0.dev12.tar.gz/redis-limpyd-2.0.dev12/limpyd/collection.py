...
_ParsedFilterTuple = namedtuple('_ParsedFilterTuple', ['index', 'suffix', 'extra_field_parts', 'value'])


class ParsedFilter(_ParsedFilterTuple):
    def __copy__(self):
...
class CollectionManager(object):
    ...
    def clone(self):
        ...
        new._lazy_collection = deepcopy(self._lazy_collection)
        new._instances = self._instances
        new._lazy_instances = self._lazy_instances
        new._sort = deepcopy(self._sort)
        new._sort_limits = deepcopy(self._sort_limits)
        new._len = self._len
