...
def handleContentType(
    ...
    if filename.endswith("xlsx") or request.META.get("CONTENT_TYPE") == "application/octet-stream":
...
def handleXlsx(
    ...
    df.to_excel(response, index=False, columns=header.all,header=header.translate(),encoding="utf-8-sig")
    response[
        "Content-Disposition"
