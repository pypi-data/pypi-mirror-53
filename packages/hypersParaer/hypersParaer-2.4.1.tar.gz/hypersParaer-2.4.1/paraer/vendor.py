...
def order4List(request, keys):
    ...
    if orderColumn and keys and (orderColumn in keys[0]):
        if isinstance(keys[0][orderColumn], str):
            keys = order_by_name_mixed(keys, key=orderColumn, orderKey=orderKey)
        elif isinstance(keys[0][orderColumn], datetime.date):
            defaultDate = datetime.datetime.strptime("1999-01-01", "%Y-%m-%d").replace(
                tzinfo=datetime.timezone.utc
            )
            if isinstance(keys[0][orderColumn], datetime.datetime):
                pass
            else:
            ...
            # 全为None 时 默认给-1
            keys = sorted(keys, key=lambda x: x[orderColumn] or -1, reverse=orderKey)
    return keys
...
class HTTPPaginator(BasePagination):
    ...
    def paginate_queryset(self, keys, request, sort=True, split=True, **kwargs):
        ...
        if isinstance(keys, pd.DataFrame):
            keys = order4df(request, keys)
        else:
            if not isinstance(keys, list):  # py3兼容 dict_values不是list
                keys = list(keys)
            keys = order4List(request, keys)
