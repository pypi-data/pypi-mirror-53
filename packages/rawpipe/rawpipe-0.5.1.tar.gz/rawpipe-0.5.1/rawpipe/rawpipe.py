...
class Algorithms:
    ...
    def wb(self, frame, wb):
        ...
        if wb is not None:
            ...
            (r, b) = wb
            frame = frame * [r, 1.0, b]
            self._vprint(f"{_elapsed(t0)} - applying WB gains [R={r:.3f}, B={b:.3f}]: range = {_minmax(frame)}")
        return frame
    ...
    def chroma_denoise(self, frame, strength=6, winsize=17):
        ...
        if strength > 0:
            ...
            self._vprint(f"{_elapsed(t0)} - chroma denoising [s={strength}, w={winsize}]: range = {_minmax(frame)}")
        return frame
