...
class APIExperiment(object):
    ...
    def log_output(self, lines, context=None, stderr=False):
        """
        Log output line(s).
        """
        results = self._api._client.log_experiment_output(
            self.id, lines, context, stderr
        )
        if self._check_results(results):
            return results.json()
    ...
    def log_other(self, key, value):
        ...
        """
        results = self._api._client.log_experiment_other(self.id, key, value)
        if self._check_results(results):
            return results.json()

    def log_parameter(self, parameter, value, step=None):
        """
        Set a parameter name/value pair for an experiment.
        """
        results = self._api._client.log_experiment_parameter(
            self.id, parameter, value, step
        )
        if self._check_results(results):
            return results.json()
    ...
    def log_metric(self, metric, value, step=None):
        ...
        """
        results = self._api._client.log_experiment_metric(self.id, metric, value, step)
        if self._check_results(results):
            return results.json()

    def log_html(self, html, overwrite=False):
        """
        Set, or append onto, an experiment's HTML.
        """
        results = self._api._client.log_experiment_html(self.id, html, overwrite)
        if self._check_results(results):
            return results.json()
