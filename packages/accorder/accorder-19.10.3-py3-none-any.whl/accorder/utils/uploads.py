...
class RsyncTunneled:
    ...
    def run_rsync(self):
        ...
        command = f"rsync -zvrith --progress {self.rsync_delete} --inplace --no-whole-file {self.rsync_local_directory} rsync://{self.rsync_user}@localhost:{self.local_port}/{self.rsync_endpoint}/"
        # click.echo(command)
        os.system(command)
