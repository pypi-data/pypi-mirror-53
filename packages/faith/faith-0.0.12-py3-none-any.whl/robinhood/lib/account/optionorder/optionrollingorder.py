...
def derive_option_rolling_order(option_order: 'OptionOrder') -> Optional[OptionRollingOrder]:
    ...
    # Rolling must have one closing leg and one opening leg.
    rolling_strategies = {'short_call', 'short_put'}
    if option_order.opening_strategy not in rolling_strategies:
        return None

    if option_order.closing_strategy not in rolling_strategies:
        return None

    # The closing leg and the opening leg must have the same strategy.
    if option_order.opening_strategy != option_order.closing_strategy:
        return None
