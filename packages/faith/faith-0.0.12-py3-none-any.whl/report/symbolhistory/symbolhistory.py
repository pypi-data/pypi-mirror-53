...
class SymbolHistory(object):
    ...
    def get_current_position_report(self, symbol: str, last_price: float) -> Tuple[str, str]:
        ...
        # Stock
        symbol_with_price = f'{symbol} {format_price(last_price)}'
        symbol_data: OrderedDict[str, Any] = OrderedDict()

        stock_key, stock_details = self._get_current_stock_holding(event=event, last_price=last_price)
        symbol_data[stock_key] = stock_details

        # Option Buy and Sell
        for buy_or_sell in ['Buy', 'Sell']:
            for option_key, option_details in self._get_current_option_buy_or_sell_holding(
                event=event,
                last_price=last_price,
                buy_or_sell=buy_or_sell,
            ):
                symbol_data[option_key] = option_details
    ...
    def _get_current_option_buy_or_sell_holding(
        ...
        last_price: float,
        buy_or_sell: str,
    ) -> 'Iterator[Tuple[str, OrderedDict[str, Any]]]':
        if buy_or_sell == 'Buy':
        ...
        for _, item in holding_dict.items():
            ...
            option_sell_key = (
                '{buy_or_sell} {symbol} ({days_to_expire}): {quantity} x {avg_price} = {base_price}'.format(
                    buy_or_sell=buy_or_sell,
                    symbol=item.option_instrument.symbol,
                    strike_price=format_price(item.option_instrument.strike_price),
                    exp_date=item.option_instrument.exp_date,
                    days_to_expire=format_quantity(item.option_instrument.days_to_expire),
            ...
            current_price_key = f'Current Price: {current_price}'
            ...
            yield (option_sell_key, option_sell_details)
