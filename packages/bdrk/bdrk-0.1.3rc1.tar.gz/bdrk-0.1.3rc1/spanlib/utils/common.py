...
def as_dataclass(obj: Any, dataclass_: Type[T], **kwargs) -> T:
    ...
    try:
        return _cast_object_to_type(obj=obj, type_=dataclass_, **kwargs)
...
def _cast_object_to_type(obj: Any, type_: Type[T], **kwargs) -> T:
    """
    ...
    if type_ is Any or type(type_) is TypeVar:
        ...
        return _cast_object_to_dataclass(obj=obj, type_=type_, **kwargs)
        ...
        return _cast_object_to_generic_class(obj=obj, type_=type_)
    if issubclass(type_, Enum):
        try:
            ...
            raise DataClassConversionError(
                f"_cast_object_to_type: Could not convert object={obj} to enum={type_} "
                f"with kwargs={kwargs}"
            ) from ex
    else:
        # Consider handling other conversions by default, ie. str to float / str to UUID
        raise DataClassConversionError(
            f"_cast_object_to_type: Could not convert object={obj} to type={type_} "
            f"with kwargs={kwargs}"
        )


def _cast_object_to_dataclass(obj: Any, type_: Type, **kwargs):
    temp = {}
    for field in dataclasses.fields(type_):
        field_value = _get_field_value(obj=obj, field_name=field.name, **kwargs)
        temp[field.name] = _cast_object_to_type(obj=field_value, type_=field.type)

    try:
        return type_(**temp)
    except TypeError as ex:
        raise DataClassConversionError(
            f"_cast_object_to_type: Could not convert object={obj} to dataclass={type_} "
            f"with kwargs={kwargs} from_temp={temp}"
        ) from ex
...
def _cast_object_to_generic_class(obj: Any, type_: Type, **kwargs):
    if type_.__origin__ is Union:
        return _cast_object_to_union(obj=obj, type_=type_)
    elif type_.__origin__ is tuple:
        return _cast_object_to_tuple(obj=obj, type_=type_)
    elif type_.__origin__ in uniform_collection:
        return _cast_object_to_collection(obj=obj, type_=type_)
    elif type_.__origin__ in dictionary:
        return _cast_object_to_dict(obj=obj, type_=type_)
    else:
        # Unsupported type conversion
        raise DataClassConversionError(
            f"_cast_object_to_type: Could not convert object={obj} to generic={type_} "
            f"with kwargs={kwargs}"
        )


def _cast_object_to_union(obj: Any, type_: Type, **kwargs):
    for arg in type_.__args__:
        try:
            return _cast_object_to_type(obj=obj, type_=arg)
    ...
    raise DataClassConversionError(
        f"_cast_object_to_type: Could not match any types for object=({obj}, {dict_repr}) "
        f"of type={value_type} to target_type={type_} with kwargs={kwargs}"
    )


def _cast_object_to_collection(obj: Any, type_: Type, **kwargs):
    ...
    temp = [_cast_object_to_type(obj=v, type_=type_.__args__[0]) for v in obj]
    try:
...
def _cast_object_to_tuple(obj: Any, type_: Type, **kwargs):
    if not hasattr(obj, "__len__") or len(obj) != len(type_.__args__):
        raise DataClassConversionError(
            f"_cast_object_to_type: Could not convert object={obj} to tuple={type_} "
            f"with kwargs={kwargs}"
        )
    return type_.__origin__(
        _cast_object_to_type(obj=elem_value, type_=elem_type)
        for elem_value, elem_type in zip(obj, type_.__args__)
    )


def _cast_object_to_dict(obj: Any, type_: Type, **kwargs):
    if not hasattr(obj, "__getitem__"):
        raise DataClassConversionError(
            f"_cast_object_to_type: Could not convert object={obj} to dict={type_} "
            f"with kwargs={kwargs}"
        )
    temp = {
        _cast_object_to_type(obj=k, type_=type_.__args__[0]): _cast_object_to_type(
            obj=v, type_=type_.__args__[1]
        )
