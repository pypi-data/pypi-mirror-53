...
def check_server_folder_para(ssh_host,user,privatekeyfile,filepath_dest):
    try:
        con = paramiko.Transport((ssh_host))
        mykey = paramiko.RSAKey.from_private_key_file(privatekeyfile)
        con.connect(username=user, pkey=mykey)
        sftp = paramiko.SFTPClient.from_transport(con)
    except:
        print("cannot connect to remote server:"+ssh_host+", please check the internet!")
        sys.exit()    
    try:
        sftp.chdir(filepath_dest)
    except:
        sftp.mkdir(filepath_dest)
