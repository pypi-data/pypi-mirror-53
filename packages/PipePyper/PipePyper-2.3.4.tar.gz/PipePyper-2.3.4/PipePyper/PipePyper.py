...
from .mytools import parseDate,timeAJ,logger
...
class reversePipe(object):
    ...
    def map(self,func):
        return reversePipe(map(lambda x:func(x),self.pipe),self.lg)
    def mp_map(self,func,num=10):
        ...
        self.pipe=ps.g_collect()
        return self
    def filter(self,func):
        return reversePipe(filter(lambda x:func(x),self.pipe),self.lg)
