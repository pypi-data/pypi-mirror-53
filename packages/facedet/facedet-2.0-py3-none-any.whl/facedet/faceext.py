...
# imagePath = sys.argv[1]		
 image = cv2.imread("facedet/data/data2.jpg")		# For pip package
# image = cv2.imread(imagePath)
 ...
 if not(condition): 
  raise Exception('High-resoluted images are not accepted. Enter images less than 1024*1024 resolution.')
 return image
...
def area(): 
 ...
 i,j = unravel_index(faces.argmax(), faces.shape)	
 print(i,j)	# gets the position of maximum value from 'face' array.
...
def extract():
 ...
 faces = detection() 
 i,j = area()
 """Most number of loops and conditional statements are avoided to reduce Time complexity"""
 X = faces[i,0]
 Y = faces[i,1]
 W = faces[i,2]
 H = faces[i,3]
 ...
 print("Face(largest) Extracted. Saved locally!")
