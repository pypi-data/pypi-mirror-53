...
class Client:
    def __init__(self, namespace: str = "kube-system", repo_file: str = DEFAULT_REPO_FILE, verbosity: int = 0):
    ...
    def list(self, context) -> List[Chart]:
        ...
        pod_name, port = self.kube.get_pod_name_and_port(self.namespace, DEFAULT_TILLER_LABELS, "tiller")
        with PortForwarder(pod_name, context, dest_port=port, namespace=self.namespace) as port:
    ...
    def update_release(self, name: str, version: str,
                       chart_path: str, kind: str, context: str, dry_run: bool = False, values: dict = None):
        if self.kube is None:
            self.kube = kube.Client(context)
        if values is None:
            values = {}
