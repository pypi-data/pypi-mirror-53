...
class ChartBuilder(object):
    ...
    def get_values(self, *additional_files):
        """
        ...
        if os.path.exists(os.path.join(self.source_directory, 'values.yaml')):
            with open(os.path.join(self.source_directory, 'values.yaml')) as file:
        ...
        for file in additional_files:
            with open(file) as vals:
                file_values.update(yaml.safe_load(vals))

        context = {**raw_values, **file_values}

        return Config(raw=yaml.safe_dump(context))
    ...
    def get_templates(self):
        ...
        for root, _, files in os.walk(os.path.join(self.source_directory,
            ...
            for tpl_file in files:
                ...
                # path separators (/). Thus, sending the template list to Tiller
                # from a Windows machine the lookup will fail.
    ...
    def get_helm_chart(self, **kwargs):
        """
        ...
        for chart in self.chart.get('dependencies', []):
            self._logger.info(f"Building dependency chart {chart.name} for release {self.chart.name}")
        ...
        helm_chart = Chart(
            ...
            values=self.get_values(*kwargs.get("additional_files", [])),
