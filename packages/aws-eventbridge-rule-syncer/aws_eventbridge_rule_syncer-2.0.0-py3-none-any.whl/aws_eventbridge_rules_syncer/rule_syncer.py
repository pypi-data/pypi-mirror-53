...
def sync_rules(rules_path, backup_file, event_bus="default"):
    ...
    for rule in rules:
        ...
        rules_map[name] = (pattern, description, target, state, queue)

    aws_rules = get_rules_from_aws(event_bus)
...
def get_rules_from_aws(event_bus):
    res = event_client.list_rules(EventBusName=event_bus)
