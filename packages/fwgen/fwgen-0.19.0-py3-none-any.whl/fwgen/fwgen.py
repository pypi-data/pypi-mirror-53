...
class Iptables(IptablesCommon):
    ...
    def apply(self, rules):
        ...
        rules_parsed = []
        for rule in rules:
            if rule.startswith('-6 ') or ' -6 ' in rule:
                LOGGER.debug("REMOVED IPV6 RULE: %s", rule)
                continue
            elif rule.startswith('-4 '):
                rule = re.sub(r'^-4\s', '', rule, 1)
            elif ' -4 ' in rule:
                rule = re.sub(r'\s-4\s', ' ', rule, 1)
            rules_parsed.append(rule)
        self._apply(rules_parsed)
...
class Ip6tables(IptablesCommon):
    ...
    def apply(self, rules):
        ...
        rules_parsed = []
        for rule in rules:
            if rule.startswith('-4 ') or ' -4 ' in rule:
                LOGGER.debug("REMOVED IPV4 RULE: %s", rule)
                continue
            elif rule.startswith('-6 '):
                rule = re.sub(r'^-6\s', '', rule, 1)
            elif ' -6 ' in rule:
                rule = re.sub(r'\s-6\s', ' ', rule, 1)
            rules_parsed.append(rule)
        self._apply(rules_parsed)
