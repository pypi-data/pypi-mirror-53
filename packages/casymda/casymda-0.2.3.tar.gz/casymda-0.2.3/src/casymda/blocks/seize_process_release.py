...
from casymda.blocks import (Delay, NamedResource, ResourceRelease,
                            ResourceSeizeQueue)
...
class SeizeProcessRelease(VisualizableBlock):
    ...
    def __init__(self, env, name, xy=None, ways=None,
                 process_time=1.0, resource_capacity=1):
        super().__init__(env, name, xy=xy, ways=ways)

        self.resource = NamedResource(
            self.env, 'resource', capacity=resource_capacity)

        self.receiver = Receiver(self.env)

        self.seizer = ResourceSeizeQueue(
            env, name + "_seizer", resource_to_request=self.resource)
        self.seizer.successors.append(self.receiver)
        self.processor = Delay(
            env,
            name + "_processor",
            process_time=process_time)
        self.processor.successors.append(self.receiver)
        self.releaser = ResourceRelease(
            env, name + "_releaser", resource_to_release=self.resource)
...
class Receiver():
    ...
    def __init__(self, env):
        self.env = env
        self.block_resource = NamedResource(
            env, "receiver_resource", capacity=1)
