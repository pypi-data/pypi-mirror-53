...
COMPONENTS = [
    "sources", "sinks", "tasks", "xGateways"
]


def parse_bpmn(bpmn_path: str, json_path: str,
...
def _generate_model_components(components):
    return "self.model_components = {" + ", ".join(
        ["'" + comp["id_name"] + "': self." + comp["id_name"] for comp in components]) + "}"


def _generate_model_graph_names(elements, components):
    return {
        comp["id_name"]: _get_successor_names_list(comp, elements, components) for comp in
        components if not (_get_successor_names_list(comp, elements, components) is None)
    }
...
def _generate_insertions(bpmn, elements, components):
    ...
    for comp in elements["resources"] + components:
        ...
        if res_name is None:
            insertions.append("self.%s = %s(self.env, '%s', xy=%s" % (id_name,
                                                                      class_name, id_name, x_y))
        else:
        ...
        insertions[-1] += ", ways=" + \
            str(_get_ways_to_successors(comp, bpmn,
                                        elements, components, offset=offset))
...
def _write_model_file(model_path, template_path, insertions,
                      model_components, model_graph_names):
    # read template file
    ...
    loc += 1
    template.insert(loc, "\n\n" + "".join([" " for _ in range(8)])
                    + "self.model_graph_names = " + json.dumps(model_graph_names))
...
def _get_offset(xmlo):
    ...
    for shape in \
            xmlo["bpmn:definitions"]["bpmndi:BPMNDiagram"]["bpmndi:BPMNPlane"]["bpmndi:BPMNShape"]:
    ...
    for edge in \
            xmlo["bpmn:definitions"]["bpmndi:BPMNDiagram"]["bpmndi:BPMNPlane"]["bpmndi:BPMNEdge"]:
...
def _get_shape_xy(elid, xmlo, offset=(0, 0)):
    ...
    for shape in \
        ...
        if shape["@bpmnElement"] == elid:
            x_y = [float(shape["dc:Bounds"]["@x"]) + int(float(shape["dc:Bounds"]["@width"]) / 2),
                   float(shape["dc:Bounds"]["@y"]) + int(float(shape["dc:Bounds"]["@height"]) / 2)]
...
def _get_outgoing_sequence_flows(comp):
    ...
    outgoing_sfs = [outgoing_sfs] if not isinstance(
        outgoing_sfs, list) else outgoing_sfs
    return outgoing_sfs
