...
class ScaledCanvasTk(ScaledCanvas):
    ...
    def load_image_file(self, path: str):
        ...
        image = Image.open(path).convert("RGBA") if platform.system(
        ) != 'Darwin' else Image.open(path).convert("RGB")
    ...
    def create_text(self, x_coord: int, y_coord: int,
                    text: str = "", anchor: str = "c",
                    fill: str = "black", font: str = "Helvetica 10"):
        x_y = self._scale_coords((x_coord, y_coord), self.scale)
        created_text = self.canvas.create_text(*x_y,
                                               font=font, fill=fill,
