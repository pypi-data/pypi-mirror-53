...
class GraylogHandler(DatagramHandler):
    ...
    def __init__(self, app, file):
        ...
        self._app = app
        self._file = file
        self.debugging_fields = False
        self.extra_fields = True
        self.chunk_size = WAN_CHUNK
        self.fqdn = False
        self.localname = None
        self.facility = 'tornado'
        self.level_names = False
        self._get_request_uri = self._closure_get_request_uri()
        DatagramHandler.__init__(self, host, port)

    def _closure_get_request_uri(self):
        try:
            # 可能没有flask？
            import tornado
    ...
    def makePickle(self, record):
        ...
        message_dict['_app'] = self._app
        message_dict['_file'] = self._file

        # 请求路径
        request_uri = self._get_request_uri()
        if request_uri is not None:
            message_dict['request_uri'] = request_uri
        # return message_to_pickle(message_dict)
...
def get_full_message(exc_info):
    return '\n'.join(traceback.format_exception(*exc_info)) if exc_info else None
