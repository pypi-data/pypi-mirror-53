...
from ins._vendor.distlib.scripts import ScriptMaker
from ins._vendor.packaging.utils import canonicalize_name
from ins._vendor.six import StringIO

from ins._internal import pep425tags
from ins._internal.download import path_to_url, unpack_url
from ins._internal.exceptions import (
    InstallationError, InvalidWheelFilename, UnsupportedWheel,
)
from ins._internal.locations import (
    ins_DELETE_MARKER_FILENAME, distutils_scheme,
)
from ins._internal.models.link import Link
from ins._internal.utils.logging import indent_log
...
from ins._internal.utils.setuptools_build import SETUPTOOLS_SHIM
from ins._internal.utils.temp_dir import TempDirectory
from ins._internal.utils.typing import MYPY_CHECK_RUNNING
from ins._internal.utils.ui import open_spinner

if MYPY_CHECK_RUNNING:
    from typing import (
        Dict, List, Optional, Sequence, Mapping, Tuple, IO, Text, Any, Iterable
    )
    from ins._vendor.packaging.requirements import Requirement
    from ins._internal.req.req_install import InstallRequirement
    from ins._internal.download import insSession
    from ins._internal.index import FormatControl, PackageFinder
    from ins._internal.operations.prepare import (
        RequirementPreparer
    )
    from ins._internal.cache import WheelCache
    from ins._internal.pep425tags import Pep425Tag
...
def sorted_outrows(outrows):
    ...
    # https://github.com/pypa/ins/issues/5868
...
def move_wheel_files(
    ...
    # See https://github.com/pypa/ins/issues/1800
    maker.clobber = True
...
"""
    ...
    # Special case ins and setuptools to generate versioned wrappers
    #
    # The issue is that some projects (specifically, ins and setuptools) use
    # code in setup.py to create "versioned" entry points - ins2.7 on Python
    # 2.7, ins3.3 on Python 3.3, etc. But these entry points are baked into
    ...
    #
    # Because setuptools and ins are bundled with _ensureins and virtualenv,
    ...
    # ensureins this code will look for an ``ENSUREins_OPTIONS`` environment
    # variable which will control which version scripts get installed.
    #
    # ENSUREins_OPTIONS=altinstall
    #   - Only insX.Y and easy_install-X.Y will be generated and installed
    # ENSUREins_OPTIONS=install
    #   - insX.Y, insX, easy_install-X.Y will be generated and installed. Note
    #     that this option is technically if ENSUREins_OPTIONS is set and is
    #     not altinstall
    # DEFAULT
    #   - The default behavior is to install ins, insX, insX.Y, easy_install
    #     and easy_install-X.Y.
    ins_script = console.pop('ins', None)
    if ins_script:
        if "ENSUREins_OPTIONS" not in os.environ:
            spec = 'ins = ' + ins_script
            generated.extend(maker.make(spec))

        if os.environ.get("ENSUREins_OPTIONS", "") != "altinstall":
            spec = 'ins%s = %s' % (sys.version[:1], ins_script)
            generated.extend(maker.make(spec))

        spec = 'ins%s = %s' % (sys.version[:3], ins_script)
        generated.extend(maker.make(spec))
        # Delete any other versioned ins entry points
        ins_ep = [k for k in console if re.match(r'ins(\d(\.\d)?)?$', k)]
        for k in ins_ep:
            del console[k]
    ...
    # Record ins as the installer
    installer = os.path.join(info_dir[0], 'INSTALLER')
    temp_installer = os.path.join(info_dir[0], 'INSTALLER.ins')
    with open(temp_installer, 'wb') as installer_file:
        installer_file.write(b'ins\n')
    shutil.move(temp_installer, installer)
    generated.append(installer)

    # Record details of all files installed
    record = os.path.join(info_dir[0], 'RECORD')
    temp_record = os.path.join(info_dir[0], 'RECORD.ins')
...
def check_compatibility(version, name):
    ...
    ins should refuse to install a Wheel-Version that's a major series
    ...
    if version[0] > VERSION_COMPATIBLE[0]:
        raise UnsupportedWheel(
            ...
            "of ins" % (name, '.'.join(map(str, version)))
        )
...
class Wheel(object):
    ...
    def __init__(self, filename):
        ...
        # (https://github.com/pypa/ins/issues/1150)
                ...
                if wheel_file:
                    ...
                    if autobuilding:
                        ...
                        if req.source_dir and not os.path.exists(os.path.join(
                                req.source_dir, ins_DELETE_MARKER_FILENAME)):
