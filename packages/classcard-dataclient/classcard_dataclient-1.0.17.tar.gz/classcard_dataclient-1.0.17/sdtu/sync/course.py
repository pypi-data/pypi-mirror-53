...
class CourseTableSync(BaseSync):
        ...
        for index in range(total_page):
            ...
            for row in rows:
                ...
                if course_name in self.course_map:
                    ...
                    if subject_number not in self.kcdm_list:
                        self.kcdm_list.append("'{}'".format(subject_number))
    ...
    def relate_student(self):
        kcdm_condition = ','.join(self.kcdm_list)

        single_map = {"单": 1, "双": 2}
        count_sql = "SELECT COUNT(*) FROM V_BZKS_XSKB WHERE XN='{}' AND XQ='{}' AND KCDM IN ({})".format(self.xn, self.xq, kcdm_condition)
        self.cur.execute(count_sql)
        try:
            total = self.cur.fetchall()[0][0]
        except (Exception,):
        ...
        for index in range(total_page):
            ...
            sql = "SELECT k.KCDM, k.SKBJ, k.RKJSGH, k.ZC, k.XQJ, k.DSZ, k.JC, k.SKDD, k.XH, k.KCMC, k.r " \
                  ...
                  "WHERE XN='{}' AND XQ='{}' AND KCDM IN ({}) ORDER BY KCDM) " \
                  "k WHERE k.r BETWEEN {} and {} ".format(self.xn, self.xq, kcdm_condition, si, ei)
            self.cur.execute(sql)
            rows = self.cur.fetchall()
            for row in rows:
