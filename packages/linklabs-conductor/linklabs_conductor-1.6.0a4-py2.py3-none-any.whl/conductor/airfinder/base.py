...
class AirfinderUplinkMessage(UplinkMessage):
    ...
    def source_message(self):
        uuid = self.metadata_json.get('sourceMessageId')
        time = self.metadata_json.get('sourceMessageTimestamp')
    ...
    def application_token(self):
        return AppToken(self.session, self.metadata_json.get('app_tok'),
                        self.instance)

    @property
    def network_token(self):
        return NetToken(self.session, self.metadata_json.get('net_tok'),
                        self.instance)
