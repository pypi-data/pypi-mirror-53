...
class Module(UplinkSubject, DownlinkSubject, EventCount):
    ...
    def get_asset_groups(self):
        ...
        url = '{}/assetGroup/node/{}'.format(self.network_asset_url, self.subject_id)
        return [AssetGroup(self.session, x['id'], _data=x) for x in self._get(url)]

    @property
    def downlink_mode(self):
        """ Returns the downlink mode of the module. """
        val = self.metadata_json.get('downlinkMode')
    ...
    def last_mailbox_request_time(self):
        ...
        val = self.metadata_json.get('mailboxRequestTime')
        return parse_time(val) if val else None
    ...
    def registered_by_gateway(self):
        ...
        val = self.metadata_json.get("registeredByGateway")
        try:
...
class LTEmModule(Module):
    ...
    def _post_status_update(self, status):
        ...
        url = '{}/lte/{}/{}'.format(self.network_asset_url, self.subject_id, status)
    ...
    def cell_data_usage(self):
        ...
        val = self.metadata_json.get('cellDataUsage')
    ...
    def last_cell_id(self):
        ...
        through. """
        val = self.metadata_json.get('cellId')
        return int(val) if val else None
    ...
    def last_cell_tac(self):
        ...
        through. """
        val = self.metadata_json.get('cellTac')
        return int(val) if val else None

    @property
    def iccid(self):
        """ Returns the ICCID of the LTEm module. """
        val = self.metadata_json.get('iccid')
        return int(val) if val else None

    @property
    def imei(self):
        """ Returns the IMEI of the LTEm module. """
        val = self.metadata_json.get('imei')
        return int(val) if val else None

    @property
    def ip_address(self):
        """ Returns the IP Address of the LTEm module.  """
        val = self.metadata_json.get('ipAddress')
        return val

    @property
    def version(self):
        """ Returns the software version of the LTEm module. """
        val = self.metadata_json.get('sw_ver')
    ...
    def sqn_versions(self):
        ...
        val1 = self.metadata_json.get('lte_ver1')
        val2 = self.metadata_json.get('lte_ver2')
        return (val1, val2)

    @property
    def provisioned_status(self):
        """ Returns the provisioned status of the LTEm module. """
        val = self.metadata_json.get('lteProvStatus')
        return val

    @property
    def mdn(self):
        """ Returns the mdn of the LTEm module. """
        val = self.metadata_json.get('mdn')
        return int(val) if val else None

    @property
    def min(self):
        """ Returns the min of the LTEm module."""
        val = self.metadata_json.get('min')
        return int(val) if val else None

    @property
    def msisdn(self):
        """ Returns the msisdn of the LTEm module."""
        val = self.metadata_json.get('msisdn')
        return int(val) if val else None

    @property
    def last_slot(self):
        """ Returns the last slot of the LTEm module."""
        val = self.metadata_json.get('slot')
        return int(val) if val else None
