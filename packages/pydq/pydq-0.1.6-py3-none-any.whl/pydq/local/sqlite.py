...
class SQLite(_queue):
    ...
    def __init__(self, name, db_file=os.path.join(os.path.dirname(os.path.realpath(__file__)), FILE_NAME)):
        super().__init__(name)
    ...
    @staticmethod
    def list_all(db_file=os.path.join(os.path.dirname(os.path.realpath(__file__)), FILE_NAME)):
        if not os.path.exists(os.path.dirname(db_file)):
            os.makedirs(os.path.dirname(db_file), exist_ok=True)
