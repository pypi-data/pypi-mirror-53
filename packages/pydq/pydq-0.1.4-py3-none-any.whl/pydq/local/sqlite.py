...
class SQLite(_queue):
    ...
    def __init__(self, name, file_location=os.path.expanduser('~')):
        super().__init__(name)
        self.db_file = os.path.join(file_location, SQLite.TABLE_NAME)
    ...
    @staticmethod
    def list_all(db_file=os.path.join(os.path.expanduser('~'), TABLE_NAME)):
