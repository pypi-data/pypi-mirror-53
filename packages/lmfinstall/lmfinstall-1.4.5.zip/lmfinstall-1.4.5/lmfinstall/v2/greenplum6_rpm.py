...
def data(pin,segs_pernode=3,init_tag=True,superuser="gpadmin"):
...
EOF"""%superuser,user=superuser,encoding='utf8')
...
EOF"""%(superuser,superuser)
    ...
    if init_tag:c.run("""su %s -c "source /home/%s/.bashrc && gpinitsystem -a -c /home/%s/gpinitsystem_config" """%(superuser,superuser,superuser),warn=True,encoding='utf8')
...
def install(pin,gpfile_path,**krg):
    para={"segs_pernode":3,"swap_tag":False,"init_tag":True,"plpython3u":"35","superuser":"gpadmin"}
    para.update(krg)
    pre(pin)
    superuser=para["superuser"]
    soft(pin,gpfile_path,superuser=superuser)
    if  para["swap_tag"]:swap(pin)
    data(pin,para["segs_pernode"],para['init_tag'],superuser=superuser)
...
# ] 
# gprpm_file="E:\\download\\greenplum-db-6.0.0-beta.6-rhel7-x86_64.rpm"
# #install(pin,gprpm_file,swap_tag=False,superuser="gpadmin1")
# #lpython(pin)
