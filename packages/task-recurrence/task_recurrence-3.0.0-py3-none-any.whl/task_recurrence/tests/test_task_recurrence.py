...
class TestWeekdayOfMonth(TestCase):
    ...
    def test_short_string(self):
        wom = WeekdayOfMonth()
        self.assertEqual('', wom.short_string)
        ...
        wom.ordinal = 0
        wom.weekday = 6
        self.assertEqual('1st Sun', wom.short_string)

        wom = WeekdayOfMonth()
        wom.ordinal = 1
        wom.weekday = 6
        self.assertEqual('2nd Sun', wom.short_string)

        wom = WeekdayOfMonth()
        wom.ordinal = 2
        wom.weekday = 6
        self.assertEqual('3rd Sun', wom.short_string)

        wom = WeekdayOfMonth()
        wom.ordinal = 3
        wom.weekday = 6
        self.assertEqual('4th Sun', wom.short_string)

        wom = WeekdayOfMonth()
        wom.ordinal = -1
        wom.weekday = 6
        self.assertEqual('last Sun', wom.short_string)
...
class TestRecurrence(TestCase):
    ...
    def test_string(self):
        ...
        def test_weeks():
            ...
            recurrence_object.weekdays.sunday = True
            self.assertEqual('every week on Sun, Mon, Thu, Sat', recurrence_object.get_string(short_format=True))
            self.assertEqual('every week on Sunday, Monday, Thursday and Saturday', recurrence_object.get_string())
            ...
            recurrence_object.weekdays.saturday = True
            recurrence_object.weekdays.sunday = True
            self.assertEqual('every 2 weeks on Sun, Mon, Thu, Sat', recurrence_object.get_string(short_format=True))
            self.assertEqual('every 2 weeks on Sunday, Monday, Thursday and Saturday', recurrence_object.get_string())

            recurrence_object = Recurrence(enabled=True, interval='week', increment=1)
            recurrence_object.weekdays.monday = True
            recurrence_object.weekdays.sunday = True
            self.assertEqual('every week on Sun, Mon', recurrence_object.get_string(short_format=True))
            self.assertEqual('every week on Sunday and Monday', recurrence_object.get_string())

            recurrence_object = Recurrence(enabled=True, interval='week', increment=1)
            recurrence_object.weekdays.monday = True
            self.assertEqual('every week on Mon', recurrence_object.get_string(short_format=True))
            self.assertEqual('every week on Monday', recurrence_object.get_string())

        def test_months():
            recurrence_object = Recurrence(enabled=True, interval='month', increment=1)
            self.assertEqual('every month', recurrence_object.get_string())

            recurrence_object = Recurrence(enabled=True, interval='month', increment=2)
            self.assertEqual('every 2 months', recurrence_object.get_string())

            recurrence_object = Recurrence(enabled=True, interval='month', increment=1)
            recurrence_object.weekday_of_month.ordinal = 0
            recurrence_object.weekday_of_month.weekday = 5
            self.assertEqual('every month on the first Saturday', recurrence_object.get_string())
            self.assertEqual('every month on 1st Sat', recurrence_object.get_string(short_format=True))

        def test_years():
            recurrence_object = Recurrence(enabled=True, interval='year', increment=1)
            self.assertEqual('every year', recurrence_object.get_string())
            recurrence_object = Recurrence(enabled=True, interval='year', increment=2)
            self.assertEqual('every 2 years', recurrence_object.get_string())
