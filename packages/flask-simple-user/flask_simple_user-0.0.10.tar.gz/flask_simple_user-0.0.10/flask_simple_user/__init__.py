...
class FlaskUser:
    ...
    @classmethod
    def init_db(cls, db, hash_func: Callable, password_length_range: Optional[List[int]]):
        """
        :param db: sqlalchemy mysql db instance.
        :param hash_func: callable object, can hash the password. if salt needed, you can use partical.
        :param password_length_range: It is a range list.
        :return:
        """
        cls.db = db
        cls.hash_func = hash_func
    ...
    def encrypt_password(cls, raw_password: str) -> str:
        return cls.hash_func(raw_password)
    ...
    @classmethod
    def register(cls, name: str, role: str, phone: str, email: str, password: str, unique_field: str = 'name'):
        parameter = {k: v for k, v in locals().items() if k not in ('cls', 'unique_field')}
        if unique_field not in parameter:
            raise ValueError(f'unique_field {unique_field} not in parameter {parameter}')
        if cls.query.filter_by(**{unique_field: parameter[unique_field]}).first():
            raise ValueError(f'the {unique_field} {parameter[unique_field]} has existed.')
