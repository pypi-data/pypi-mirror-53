...
class TestVisitSchedule(TestCase):
    ...
    def test_requisitions(self):
        prn = ["blood_glucose", 'blood_glucose_poc', 'chemistry',
               'fbc', 'hba1c', 'hba1c_poc']
        expected = {
        ...
        for visit_code, visit in schedule.visits.items():
            actual = [
                requisition.name for requisition in visit.requisitions]
            actual.sort()
            self.assertEqual(expected.get(visit_code), actual,
    ...
    def test_crfs(self):
        ...
        for visit_code, visit in schedule.visits.items():
            actual = [
                crf.model for crf in visit.crfs]
            actual.sort()
            self.assertEqual(expected.get(visit_code), actual,
                             msg=f"see CRFs for visit {visit_code}")

            actual = [
                crf.model for crf in visit.crfs_prn]
            actual.sort()
            self.assertEqual(
