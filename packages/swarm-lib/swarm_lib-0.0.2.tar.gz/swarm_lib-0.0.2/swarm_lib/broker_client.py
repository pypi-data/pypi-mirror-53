...
class BrokerClient():
    ...
    def smart_use(contract, method=None, data=None, token=None):
        ...
        if method:
            return BrokerClient.use(provider_description, method, provider_id + entry, data, token)
        else:
            raise "Could not infer method (%s)" % (method)

    def use(service_desc, method, url, data=None, token=None, attrs={}):
        user_attrs = {"swarm:Id": service_desc["@id"], "swarm:ServiceType": service_desc["@type"]}
        user_attrs = {**user_attrs, **attrs}
        if token:
