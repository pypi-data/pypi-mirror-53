...
Variable.processors = [
    Variable.EnvExpander, Variable.PathNormalizer, Variable.DuplicatesRemover
]
...
class Test(unittest.TestCase):
    ...
    def testVariables(self):
        ...
        self.assertRaises(Variable.EnvError, control.declare, 'MY_PATH',
                          'list', True)
        self.assertRaises(Variable.EnvError, control.declare, 'MY_PATH',
                          'scalar', True)
        self.assertRaises(Variable.EnvError, control.declare, 'MY_PATH',
        ...
        control.declare('loc', 'list', True)
        self.assertRaises(Variable.EnvError, control.declare, 'loc', 'list',
                          False)
        self.assertRaises(Variable.EnvError, control.declare, 'loc', 'scalar',
                          True)
        self.assertRaises(Variable.EnvError, control.declare, 'loc', 'scalar',
                          True)

        control.declare('myVar2', 'scalar', False)
        self.assertRaises(Variable.EnvError, control.declare, 'myVar2', 'list',
                          False)
        self.assertRaises(Variable.EnvError, control.declare, 'myVar2', 'list',
                          True)
        self.assertRaises(Variable.EnvError, control.declare, 'myVar2',
                          'scalar', True)

        control.declare('loc2', 'scalar', True)
        self.assertRaises(Variable.EnvError, control.declare, 'loc2', 'list',
                          False)
        self.assertRaises(Variable.EnvError, control.declare, 'loc2', 'list',
                          True)
        self.assertRaises(Variable.EnvError, control.declare, 'loc2', 'scalar',
                          False)
...
<env:set variable="INCLUDED_FILE">OK</env:set>
            ...
            self.assertEqual(
                str(control['INCLUDED_FILE_PATH']), os.path.join(
                    tmp, 'subdir'))
...
<env:default variable="var2">test2</env:default>
    ...
    def testRecursiveListExpansion(self):
        # see https://gitlab.cern.ch/gaudi/xenv/issues/2
        from contextlib import contextmanager
        @contextmanager
        def default_processors():
            try:
                bkup = Variable.processors
                Variable.processors = saved_processors
                yield
            finally:
                Variable.processors = bkup

        with default_processors():
            control = Control.Environment()

            control.set('A_PATH', '/usr/bin:/bin')
            assert str(control['A_PATH'])

            control.set('B_PATH', '${A_PATH}')
            assert str(control['B_PATH'])

            assert str(control['A_PATH']) == str(control['B_PATH'])
