...
def pytest_exception_interact(node, call, report):
    if node.config.option.influxdb_pytest:
        if report.failed:
            ...
            if test_result_dto:
                ...
                exception_list = []
                for line in stack_trace.chain[0][0].reprentries[0].lines:
                    exception_list.append(line.replace('"', "'").replace('\n', '\\n'))
                stack_trace.chain[0][0].reprentries[0].lines = exception_list
...
@pytest.fixture()
def shared_directory(request, pytestconfig):
    if pytestconfig.getoption('--influxdb-pytest'):
