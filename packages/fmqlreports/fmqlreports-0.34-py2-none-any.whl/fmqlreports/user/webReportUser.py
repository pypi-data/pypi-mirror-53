...
"""
    ...
    mu += muDateTimeByWeekDay(type3_081) + "\n\n"
...
Besides the _DoD User_, there are <span class='yellowIt'>{:,}</span> active __Non-Proxy Machine Users__ with <span class='yellowIt'>{}</span> signons. These users appear in most VistAs under fabricated social security numbers ...\n\n""".format(
    ...
    tbl = MarkdownTable(["Name [IEN]", "Entered", "SSN", "SMOs", "SignOns", "Period", "Remote Station Id(s)", "Remote IEN(s)", "IPs", "Unexpected"])
...
""".format(
    ...
    byWeekDay = nameWeekdays(comboST["date_time"]["byWeekDay"])
    tbl = MarkdownTable(["Day", "Count %"])
    for wd in byWeekDay:
        tbl.addRow([
            wd,
            reportAbsAndPercent(byWeekDay[wd], totalRemoteSignOns)
        ])
    mu += tbl.md() + "\n\n"
    ...
    cols = ["Name [IEN]", "Entered", "Remote Id(s)", "SignOns", "Days", "Period", "Remote Apps", "Options"]
    useUnexpected = False
    ...
    for i, userRef in enumerate(sorted(activeRemoteUserRefs, key=lambda x: st3_081ByUserRef[x]["_total"], reverse=True), 1):
        ...
        row = [
            ...
            muBVC(nameWeekdays(st["date_time"]["byWeekDay"])),
            muSignOnPeriod(st),
            remoteAppMU,
            smosMU
        ]
...
There are <span class='yellowIt'>{}</span> active Local Users with <span class='yellowIt'>{}</span> signons.\n\n""".format(reportAbsAndPercent(len(activeLocalUserRefs), totalActiveUsers), reportAbsAndPercent(totalLocalSignOns, totalSignOns))
    ...
    cols = ["Name [IEN]", "Entered", "Title", "SignOns", "Sign On Period", "PMO", "SMOs", "Keys"]
    useUnexpected = False
...
# TODO: add in average per week day by counting wednesdays over period etc
def muDateTimeByWeekDay(st):   
    byWeekDay = nameWeekdays(st["date_time"]["byWeekDay"])
    tbl = MarkdownTable(["Day", "Count %"])
    for wd in byWeekDay:
        tbl.addRow([
            wd,
            reportAbsAndPercent(byWeekDay[wd], st["_total"])
        ])
    return tbl.md()
    
def nameWeekdays(intWeekdayCount):
    weekday = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "__Saturday__", "__Sunday__"]
    nameWeekdayCount = OrderedDict() # keep day order
    for id in sorted(intWeekdayCount, key=lambda x: int(x)):
        nameWeekdayCount[weekday[int(id)]] = intWeekdayCount[id]
    return nameWeekdayCount
