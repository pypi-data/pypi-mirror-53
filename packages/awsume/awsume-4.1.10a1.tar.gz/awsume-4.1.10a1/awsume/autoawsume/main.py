...
def main():
    ...
    while True:
        ...
        for profile_name, auto_profile in auto_profiles.items():
            logger.info('Looking at profile [{}]: {}'.format(profile_name, json.dumps(auto_profile)))
        ...
        logger.debug('Collected expirations: {}'.format(json.dumps(expirations)))
...
def configure_logger():
    ...
    log_dir = str(Path('~/.awsume/logs/'))
    log_file = str(Path('~/.awsume/logs/autoawsume-{}'.format(datetime.now().strftime('%Y-%m-%d_%H:%M:%S'))))
...
def refresh_profile(auto_profile):
    logger.debug('Refreshing profile {}'.format(json.dumps(auto_profile)))
    try:
