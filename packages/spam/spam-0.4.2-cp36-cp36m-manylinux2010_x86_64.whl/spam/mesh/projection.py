...
def projectField(mesh, fields, thresholds=[0.0], outputFile=None, nSkip=1, writeConnectivity=None, vtkMesh=False):
    ...
    ----------
        ...
        outputFile: string, default=None
            When not None, it writes a text file called `outputFile` the list of node and the list of elements
                    which format is:
    ...
    """
    # init projmorpho
    pr = projmorpho("default" if writeConnectivity is None else writeConnectivity, thresholds)
    ...
    # mtk.project_fields( meshFile, outputFile, thresholds, fieldFiles, nSkip )
