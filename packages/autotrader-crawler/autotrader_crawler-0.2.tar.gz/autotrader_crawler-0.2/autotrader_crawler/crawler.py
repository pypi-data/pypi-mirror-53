...
def pull_page(page_number, postcode):
    for attempt in range(5):
        try:
            with contextlib.closing(
                    urllib.request.urlopen(_index_page_url(page_number, postcode))) as autotrader_response:
...
def _index_page_url(page_number, postcode):
