...
class DechunkedInput(io.RawIOBase):
    ...
    def __init__(self, rfile):
        print(" In constructor for DechunkedInput! ")
        self._rfile = rfile
        print("type of rfile is {}".format(rfile.__class__))
        print(" rfile is {}".format(rfile))
        self._done = False
        self._len = 0
    ...
    def read_chunk_len(self):
        print("In read_chunk_len")
        try:
            line = self._rfile.readline().decode("latin1")
            print("line is {}".format(line))
            _len = int(line.strip(), 16)
        except ValueError:
            raise IOError("Invalid chunk headerrr")
        if _len < 0:
