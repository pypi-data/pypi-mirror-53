...
def scatter(sim, method='lu', 
    ...
    if method.lower() not in ["numpyinv", "scipyinv", "lu", "superlu", 
        ...
        raise ValueError('Error: Unknown solving method. Must be one of' +
                         ...
                         ' "cg", "pycg", "dyson", "cuda", "skcuda", "cupy"].')
    ...
    ----------
      ...
      method : string, default: "lu"
        inversion method. One of ["lu", "numpyinv", "scipyinv", "pinv2", "superlu", "dyson"]
         ...
         - "superlu" often has a <N^3 efficieny but is very expensive in memory (`scipy.sparse.linalg.splu`)
         - "dyson" uses a dyson sequence. Only solver that does not depend on external libraries
         - "cuda" uses a CUDA-capable GPU for matrix inversion. 
...
#    if len(np.unique(dp_orient, axis=0)) > 1:
    ...
    for i_wl, wavelength in enumerate(wavelengths):
        ...
        ## --- construct matrix
        if method.lower() in ["lu", "numpyinv", "scipyinv", "superlu", "pinv2", "cuda"]:
            as_csc = True if method.lower() == 'superlu' else False
            CAM0 = get_side_by_side(sim, wavelength=wavelength, invertible=True, 
                                    times_alpha=True, as_csc=as_csc)
        else:
...
def get_general_propagator(CAM0, method='lu', sim=None, wavelength=None, 
    ...
    ## --- CUDA based inversion on GPU viua `skcuda` - EXPERIMENTAL
...
#        K = Ainv_gpu.get()
        ...
        import cupy.linalg
        ## move array to cuda device, cuda inversion
        Agpu = cp.array(np.ascontiguousarray(CAM0))  # move to device memory
...
def get_SBS_numba(sim, wavelength, invertible=True, times_alpha=True, 
    ...
    t_sbs(geo, wavelength, cnorm, step, eps1, eps2, eps3, spacing, alpha, M)
...
def get_SBS_cuda(sim, wavelength, invertible=True, times_alpha=True, 
                 as_csc=True, threadsperblock=(16,16)):
    # =========================================================================
    # create CUDA propagator and kernel
    # =========================================================================
    import math
    from numba import cuda
    from pyGDM2.propagators import G_cuda


    ## --- cuda version of propagator
    G_cuda = cuda.jit(device=True)(G_cuda)
    
    ## --- cuda kernel
    @cuda.jit()
    def t_sbs_kernel_cuda(geo, lamda, cnorm, step, eps1, eps2, eps3, spacing, alpha, M):
        """
        Code for kernel.
        """
        ...
        if i < M.shape[0]/3 and j < M.shape[1]/3:
            ...
            R2 = geo[j]
            aj = alpha[j]
            xx, yy, zz, xy, xz, yx, yz, zx, zy = G_cuda(R1, R2, lamda, 
    ...
    t_sbs_kernel_cuda[blockspergrid, threadsperblock](geo, wavelength, 
                                     cnorm, step, eps1, eps2, eps3, spacing, alpha, M)
    cuda.close()
