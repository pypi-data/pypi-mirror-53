...
def extinct(sim, field_index):
    ...
    warnings.warn("Info: The fortran code underlying 'extinct' will be replaced " +
          ...
          "The new implementation can be tested through: `linear.extinct_py`.")
...
def _calc_extinct(wavelength, n_env, alpha_spectral, E0, E):
...
def extinct_py(sim, field_index):
    """Extinction, scattering and absorption cross-sections
    ...
    ## --- get polarizability at wavelength
    alpha_spectral = sim.struct.getPolarizabilityTensor(wavelength)
    E0 = sim.efield.field_generator(sim.struct, **field_params)
    E = sim.E[field_index][1]
    
    return _calc_extinct(wavelength, n_env, alpha_spectral, E0, E)
...
#==============================================================================
    ...
    ce0x.T[tetalist.T>np.pi/2] *= -1
    ce0y.T[tetalist.T>np.pi/2] *= -1
    ce0z.T[tetalist.T>np.pi/2] *= -1
...
def mutlipole_decomp_extinct(sim, field_index, r0=None, eps_dd=0.1):
    ...
    ** ------- FUNCTION STILL UNDER TESTING ------- **
    ...
    p, m, q, mq = mutlipole_decomp(sim, field_index, r0=r0)
