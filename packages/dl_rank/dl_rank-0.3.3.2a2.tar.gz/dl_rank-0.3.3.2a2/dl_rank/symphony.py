...
def distributeInfer(conf, date, ps, emrName, logpath=emr_home_path, use_TFoS=True, dirs2issue=None):
    _args_useSpark = True
    if dirs2issue is None:
        dirs2issue = []
    if __name__=='__main__':
    ...
    with open(emr_home_path+'/next.sh', 'w') as f:
        f.writelines(command)
    with open(emr_home_path + '/command.txt', 'w') as f:
        f.writelines(command)
    return command
