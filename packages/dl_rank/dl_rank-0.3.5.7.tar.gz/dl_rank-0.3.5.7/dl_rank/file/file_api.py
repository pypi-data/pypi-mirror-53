...
class hdfsFile(object):
    ...
    @staticmethod
    def CopyLocal(srcPath, dstPath, overwrite=False):
    ...
    def Rename(srcPath, dstPath):
        if not hdfsFile.client.exists(srcPath):
            return False
        _ = os.system('hadoop fs -mv {src} {dst}'.format(src=srcPath, dst=dstPath))
...
class s3File(object):
    ...
    def CopyLocal(srcPath, dstPath, overwrite=False):
        ...
        _ = os.system(
            'aws s3 cp {srcPath} {dstPath}'.format(srcPath=srcPath, dstPath=dstPath))

    @staticmethod
    def CopyHdfs(srcPath, dstPath, overwrite=False):
        if overwrite and s3File.Exists(dstPath):
            s3File.Remove(dstPath)
        _ = os.system(
            's3-dist-cp --src={srcPath} --dest={dstPath}'.format(srcPath=srcPath, dstPath=dstPath))
