...
class StreamingFeaturizerTests(unittest.TestCase):
    ...
    def test_featurize_with_nans(self):
        ...
        try:
            ...
            featurized_X = streaming_featurizer.learn_transformations().transform(train_df)
    ...
    def test_categoricals_with_two_categories(self):
        ...
        try:
            ...
            test_transformed = featurization_transformer.transform(test_df)
    ...
    def test_drop_hashes(self):
        ...
        n = 10000
        train_array = np.random.rand(n)
        test_array = np.repeat([0], n)
        train_df = pd.DataFrame({'col1': train_array, 'label': test_array})
        train_df['col1'] = train_df['col1'].astype(str)
        path = dataframe_as_csv_file(train_df)

        try:
            X_train = dprep.read_csv(path=path)
            streaming_featurizer = StreamingFeaturizer(X_train, 'label')
            feat = streaming_featurizer.learn_transformations().transform(train_df)
    ...
    def test_text_transform(self):
        ...
        try:
            ...
            test_transformed = featurization_transformer.transform(test_reviews)
            # 83 because we don't have label column here
    ...
    def test_multiple_text_transforms(self):
        ...
        try:
            ...
            self.assertTrue(len(featurization_transformer.pipeline.steps) == 6,
                            "Expecting 5 separate NGram transformers for 5 input text columns, and"
                            "1 drop transform removing original columns")
        finally:
            os.remove(path)
    ...
    def test_flatten_similar_transforms(self):
        ...
        try:
            ...
            self.assertTrue(len(featurization_transformer.pipeline.steps) == 2,
                            "Expecting just 2 transformers for the given input data (1 one hot"
                            "transform and then 1 drop transform removing original columns)")
        finally:
            os.remove(path)
