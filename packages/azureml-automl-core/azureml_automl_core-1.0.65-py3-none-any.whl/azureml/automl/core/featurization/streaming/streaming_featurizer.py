...
from azureml.automl.core.featurizer.transformer import get_ngram_len, CategoricalFeaturizers, GenericFeaturizers, \
    TextFeaturizers
...
class StreamingFeaturizer:
    ...
    def __init__(self,
        ...
        # List of columns that are dropped from featurization and subsequently model training
    ...
    def get_transformed_vector_column_names(self) -> List[str]:
        return list(self._features_metadata_helper.alias_raw_feature_name_transformation_mapping.keys())

    def _update_metadata_using_transformed_column_names(
            self,
            estimator: NimbusMLStreamingEstimator) -> None:
        # get engineered feature column names by doing a dummy
        # transform on a single row of data
        transformed_column_names = estimator.transform(
            self._training_data.head(1)).columns.tolist()
        transformed_column_names.remove(self._label_column)
    ...
    def _get_transformations(self,
        ...
        columns_to_drop = list(set(self._training_columns) - set(self.get_transformed_vector_column_names()))
    ...
    def _get_transforms_for_column_purpose(self,
        ...
        if column_purpose == _FeatureType.Numeric:
            aliases_to_raw_col_names = self._get_new_col_aliases_to_raw_col_names(
                columns_to_transform, _SupportedTransformersInternal.NimbusMLImputer,
            ...
            self._training_columns.extend(columns_to_transform)
            featurizer_names = [type(missing_value_transform).__name__]

        elif column_purpose == _FeatureType.Categorical:
            aliases_to_raw_col_names = self._get_new_col_aliases_to_raw_col_names(
                columns_to_transform, _SupportedTransformersInternal.NimbusMLOneHotEncoder,
            ...
            aliases_to_raw_col_names = self._get_new_col_aliases_to_raw_col_names(
                columns_to_transform, _SupportedTransformersInternal.NimbusMLOneHotHashEncoder,
    ...
    def _get_text_transforms(self, column_name_with_stats: List[Tuple[str, RawFeatureStats]]) ->\
        ...
        for raw_column_name, raw_stats in column_name_with_stats:
            ...
            alias_to_raw_col_name = self._get_new_col_aliases_to_raw_col_names(
                [raw_column_name], _SupportedTransformersInternal.NimbusMLNGramFeaturizer,
