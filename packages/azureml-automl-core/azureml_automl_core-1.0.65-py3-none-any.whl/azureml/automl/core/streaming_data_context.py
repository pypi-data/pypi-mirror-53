...
class StreamingTransformedDataContext(BaseDataContext):
    ...
    def __init__(self,
        ...
        if not training_columns:
            self.training_columns = list(set(self.training_data.head(1).columns.values) - {self.label_column_name})
            if self.weight_column_name is not None:
                self.training_columns = list(set(self.training_columns) - set([self.weight_column_name]))
        else:
            self.training_columns = training_columns
        self.dropped_columns = dropped_columns
        self._featurization_transformer = None  # type: Optional[StreamingFeaturizationTransformer]
        self._transformed_vector_column_names = None  # type: Optional[List[str]]
    ...
    def set_featurization_transformer(
            self,
            featurization_transformer: StreamingFeaturizationTransformer) -> None:
    ...
    def get_transformed_vector_column_names(self) -> Optional[List[str]]:
        return self._transformed_vector_column_names

    def set_transformed_vector_column_names(
            self,
            transformed_vector_column_names: List[str]) -> None:
        self._transformed_vector_column_names = transformed_vector_column_names
