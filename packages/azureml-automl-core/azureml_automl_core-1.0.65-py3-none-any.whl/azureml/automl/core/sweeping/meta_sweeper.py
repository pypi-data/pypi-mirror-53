...
from ..abstract_jasmine_client import AbstractJasmineClient
...
class MetaSweeper:
    ...
    def __init__(self,
                 ...
                 jasmine_client: Optional[AbstractJasmineClient] = None,
        ...
        :param jasmine_client: Client for JOS communication.
        :paran parent_run_id: Parent run id.
        """
        ...
        self._cfg = {}  # type: Dict[str,Any]

        if self.jasmine_client is not None \
                and self.parent_run_id is not None \
                and isinstance(self.parent_run_id, str)\
                and isinstance(jasmine_client, AbstractJasmineClient):
            is_gpu = False
            try:
                import torch
                ...
                pass
            self._cfg = self.jasmine_client.get_feature_sweeping_config(parent_run_id=self.parent_run_id,
                                                                        task_type=task,
                                                                        is_gpu=is_gpu)
        else:
            self._logger.warning("Unable to fetch feature_sweeping_config from service. Either parent_run_id\
                or jasmine_client is not provided. Defaulting to blob store config.")
    ...
    def _build_featurizers(self, feature_config: Dict[str, Any],
                           logger: logging.Logger, is_cross_validation: bool = False) -> Pipeline:
        ...
        for c in feature_steps:
            if is_cross_validation:
                if c['id'] == 'bilstm_text_dnn' or c['id'] == 'pretrained_text_dnn':
                    return None
