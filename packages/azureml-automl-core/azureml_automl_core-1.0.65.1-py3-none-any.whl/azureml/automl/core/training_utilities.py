...
from automl.client.core.common.streaming_dataset import StreamingDataset, DatasetMetadataKeys
...
def _check_datatypes(training_data: dprep.Dataflow, validation_data: dprep.Dataflow) -> None:
    if training_data is not None and validation_data is not None:
        train_dtypes = training_data.dtypes.items()
        valid_dtypes = validation_data.dtypes.items()
...
def _get_auto_parameters_maybe(automl_settings: AutoMLBaseSettings,
    ...
    lags = automl_settings.lags[constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN] \
        if automl_settings.lags is not None else [0]  # type: Union[str, List[int]]
...
def init_streaming_dataset(
    ...
    training_columns = transformed_data_context.training_columns
    dropped_columns = transformed_data_context.dropped_columns
    transformed_vector_column_names = transformed_data_context.get_transformed_vector_column_names()

    dataset_metadata = {DatasetMetadataKeys.training_columns: training_columns,
                        ...
                        DatasetMetadataKeys.dropped_columns: dropped_columns,
                        DatasetMetadataKeys.raw_data_snapshot: transformed_data_context.raw_data_snapshot,
                        DatasetMetadataKeys.transformed_vector_column_names: transformed_vector_column_names}
