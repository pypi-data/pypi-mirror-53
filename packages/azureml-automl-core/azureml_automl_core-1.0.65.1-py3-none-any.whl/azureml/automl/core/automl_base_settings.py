        ...
        if is_timeseries:
            ...
            if isinstance(lags, int) or isinstance(lags, list):
                if isinstance(lags, int):
                    target_lags = [lags]  # type: Optional[Union[List[int], str]]
                else:
                    # Get unique values.
                    target_lags = list(set(lags))
                if target_lags == []:
                    target_lags = None
                elif target_lags is not None:  # This cindition is required for mypy.
            ...
            if target_lags is not None:
                self.lags =\
                    {constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN:
                     target_lags}  # type: Optional[Dict[str, Union[str, List[int]]]]
            else:
                self.lags = None
        ...
        if self.blacklist_algos is not None:
            ...
            for algo in self.blacklist_algos:
                if self.task_type == constants.Tasks.CLASSIFICATION:
                    ...
                    if self.is_timeseries:
                        new_blacklist.append(
                            ModelNameMappings.CustomerFacingModelToClassNameModelMapForecasting.get(algo))
                    else:
                        new_blacklist.append(
