...
def update_plate():
    global guide_shape
    if len(guide_path_pts) > 1:  # 判断拾取点列是否有效
        cs = vmi.CS4x4(axis2=-cup_axis)
        cs.orthogonalize([0, 1])

        # 构造初始投影面
        lmax = vmi.pdOBB_Pts(aceta_prop.data())['size'][0]
        pts = [pt - lmax * cs.axis(2) for pt in guide_path_pts]  # 相对路径点上升足够距离
        top_pd = vmi.pdTriangulate_Polyline(pts)  # 初步三角网格化多边形内部
        top_pd = vmi.pdRemesh(top_pd)  # 网格重划分，使其致密均匀

        # 投影构造下端面
        bottom_pd = vmi.pdRayCast_Pd(top_pd, cs.axis(2), aceta_prop.data(), guide_path_pts[0])  # 投影面上的所有网格点投影到模型表面
        ...
        # 等距构造上端面
        # top_pts = [pt + 4 * cup_axis for pt in bottom_pts]
        ...
        guide_shape = vmi.ccSolid(vmi.ccSew([top_face, profile, bottom_face]))  # 缝合构造实体

        guide_plate_prop.clone(guide_shape)  # 更新导板显示
    else:
        guide_plate_prop.clone()  # 拾取路径无效时清空导板显示
    guide_view.updateInTime()  # 刷新视图


def update_plate_hole(pt, radius):
    global guide_shape
    b = guide_plate_prop.data().GetBounds()
    b = np.array([b[1] - b[0], b[3] - b[2], b[5] - b[4]])
    b = np.linalg.norm(b)

    d = vmi.pdRayCast_Pt(pt + b * cup_axis, -cup_axis, guide_plate_prop.data())
    if d is None:
        return

    hole_center = pt + (b - d) * cup_axis

    # 构造引导孔外圆柱
    hole_outer_radius = radius + 2  # 半径
    hole_outer_height = 20  # 高度
    ...
    # 构造引导孔内圆柱
    hole_inner_radius = radius  # 半径
    hole_depth = b + hole_outer_height
    hole_inner_center = hole_center + hole_outer_height * cup_axis  # 圆孔定位到圆台上端面中心
    ...
    # 融合匹配板和引导孔构造导板
    guide_shape = vmi.ccBoolean_Union([guide_shape, hole_outer_solid])  # 布尔加匹配板和圆台
    guide_shape = vmi.ccBoolean_Difference([guide_shape, hole_inner_solid])  # 再布尔减圆孔
    guide_plate_prop.clone(guide_shape)  # 更新导板显示
