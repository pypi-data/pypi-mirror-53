...
def LeftButtonPressMove(**kwargs):
    if kwargs['picked'] is voi_view:
        ...
        if not voi_volume.visible() and voi_slice.visible():
            pt = voi_view.mouseOnPropCell()
            cline_centers[i] = vmi.rayPlane(voi_slice.slicePlaneOrigin(), voi_slice.slicePlaneNormal(),
...
if __name__ == '__main__':
    ...
    # 编辑视图
    edit_view = vmi.View()
    edit_slice = vmi.ImageSlice(edit_view)
    edit_slice.mouse['NoButton']['Wheel'] = edit_slice.mousepass
    edit_slice.bind(voi_volume)
    edit_slice.windowSoft()

    # 测量视图

    # 视图布局
    views = [original_view, voi_view, edit_view]
    ...
    layout.addWidget(edit_view, 0, 2, 3, 1)
