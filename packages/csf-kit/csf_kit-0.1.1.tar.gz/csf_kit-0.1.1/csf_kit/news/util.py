...
def unzip_news_files(zipfile_dir_path, unzip_to_dir_path='./unzipped_files'):
    ...
    print('Successfully Unzip news file to {}'.format(unzip_to_dir_path))


def extract_news_info_from_json(news_json, filed=None):
    """
    extract basic news information data from the one piece of news data
    :param news_json:
    :param filed: str or list. The filed to be extracted.
                  it could be any of ['newsId','newsTitle','newsTs','newsUrl','newsSource','newsSummary']
    :return:
    """
...
def align_trade_date(df_raw, date_col, cut_hour=15, cut_minute=0, trade_calendar=None):
    ...
    if not trade_calendar:
        dts = pd.read_csv('../../data/trade_calendar_zh.csv', index_col=0)
...
def filter_by_listed_days(fac_df, days=20):
    ...
    list_dts = pd.read_csv('../../data/stock_list_dates.csv', index_col=0,
