...
def extract_senti_data(file_path):
...
def raw_senti_data_process(df_raw, cut_hour=15, cut_minute=0):
    ...
    df_raw['senti_score_origin'] = df_raw['senti_type'] * df_raw['senti_weight'] * 100
    df_raw['senti_score'] = df_raw['senti_score_origin'] * df_raw['relevance']
    df_raw['code'] = [''.join([i[0:6], '.XSHE']) if i[-5:-3] == 'SZ' else ''.join([i[0:6], '.XSHG']) for i in
                      df_raw['code']]
    df_aligned = align_trade_date(df_raw, cut_hour, cut_minute)
...
if __name__ == '__main__':
    ...
    df1 = extract_senti_data('/Users/song/PycharmProjects/csf_kit/news/sample_news_file/20190701-20190801.txt')

    df = align_trade_date(df1, date_col='news_time')
