...
class MetricMean(Metric):
    def __init__(self, device=None, name='mean'):
        ...
        if device is not None:
            self.total = torch.zeros([], dtype=torch.float32).cuda(device)
            self.count = torch.zeros([], dtype=torch.float32).cuda(device)
        else:
            self.total = torch.zeros([], dtype=torch.float32)
            self.count = torch.zeros([], dtype=torch.float32)
...
class MetricAccuracy(Metric):
    ...
    def update(self, labels, predictions, weights=None):
        ...
        predictions = torch.squeeze(predictions).argmax(
...
class MetricConfusionMatrix(Metric):
    ...
    def __init__(self, num_thresholds, device=None, name='confusion_matrix'):
        ...
        if device is not None:
            self.thresholds = torch.FloatTensor(thresholds).cuda(device)
            self.tp = torch.zeros(
                [num_thresholds], dtype=torch.float32).cuda(device)
            self.fn = torch.zeros(
                [num_thresholds], dtype=torch.float32).cuda(device)
            self.tn = torch.zeros(
                [num_thresholds], dtype=torch.float32).cuda(device)
            ...
            self.thresholds = torch.FloatTensor(thresholds)
            self.tp = torch.zeros([num_thresholds], dtype=torch.float32)
            self.fn = torch.zeros([num_thresholds], dtype=torch.float32)
            self.tn = torch.zeros([num_thresholds], dtype=torch.float32)
            self.fp = torch.zeros([num_thresholds], dtype=torch.float32)
...
class MetricSpearmanCorrelation(Metric):
    def __init__(self, rank_method='average', device=None, name='spearman_corr'):
        ...
        if device is not None:
            self.vx = torch.zeros([1], dtype=torch.float32).cuda(device=device)
            self.vy = torch.zeros([1], dtype=torch.float32).cuda(device=device)
            self.vw = torch.ones([1], dtype=torch.float32).cuda(device=device)
        else:
            self.vx = torch.zeros([1], dtype=torch.float32)
            self.vy = torch.zeros([1], dtype=torch.float32)
            self.vw = torch.ones([1], dtype=torch.float32)
...
class MetricComoment(Metric):
    ...
    def __init__(self, device=None, name='comoment'):
        ...
        if device is not None:
            self.count = torch.zeros([], dtype=torch.float32).cuda(device)
            self.mean_x = torch.zeros([], dtype=torch.float32).cuda(device)
            self.mean_y = torch.zeros([], dtype=torch.float32).cuda(device)
            self.comoment = torch.zeros([], dtype=torch.float32).cuda(device)
        else:
            self.count = torch.zeros([], dtype=torch.float32)
            self.mean_x = torch.zeros([], dtype=torch.float32)
            self.mean_y = torch.zeros([], dtype=torch.float32)
            self.comoment = torch.zeros([], dtype=torch.float32)
