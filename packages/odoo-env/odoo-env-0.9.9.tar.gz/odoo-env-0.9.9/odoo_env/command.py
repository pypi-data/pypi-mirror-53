...
class CreateOdooConfig(Command):
    """ En este objeto usamos:
        args para pasar el contenido del config
        command para pasar el directorio donde salvar el config
    """
    def check_args(self):
    ...
    def execute(self):
        ...
        for param in self._args:
            conf = conf.replace(param['key'], str(param['value']))
