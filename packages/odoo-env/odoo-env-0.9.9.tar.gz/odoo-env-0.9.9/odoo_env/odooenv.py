...
try:
    ...
    from command import Command, MakedirCommand, ExtractSourcesCommand, \
        ...
        PullImage, PullRepo, CreateOdooConfig
...
class OdooEnv(object):
    ...
    def write_config(self, client_name, testing=False):
        ...
        self._client = Client(self, client_name)
        if self._client.numeric_ver < 11:
            from messages import Msg
            Msg.err('Can not write config for Versions < 11.0')

        # archivo config a crear
        command = '%sconfig/odoo.conf' % self.client.base_dir

        # directorio donde estan los sources
        sources = '%ssources' % self.client.base_dir

        repos = []
        for repo in os.listdir(sources):
            repos.append(IN_CUSTOM_ADDONS + '/' + repo)

        if testing:
            repos = [IN_CUSTOM_ADDONS + '/adhoc-financial-tools',
                     IN_CUSTOM_ADDONS + '/adhoc-account-payment',
                     IN_CUSTOM_ADDONS + '/oca-web']

        args = []
        args.append({'key': '$addons_path$', 'value': ','.join(repos)})
        ...
        workers = 0 if self._options['debug'] else 3
        args.append({'key': '$workers$', 'value': workers})
        ...
        # mode.
        max_cron_threads = 1
        args.append({'key': '$max_cron_threads$', 'value': max_cron_threads})

        # Number of requests a worker will process before being recycled and
        # restarted. Defaults to 8196
        limit_request = 8196
        args.append({'key': '$limit_request$', 'value': limit_request})

        # Maximum allowed virtual memory per worker. If the limit is exceeded,
        # the worker is killed and recycled at the end of the current request.
        # Defaults to 2048MB
        limit_memory_soft = 2048000000
        args.append({'key': '$limit_memory_soft$', 'value': limit_memory_soft})

        # Hard limit on virtual memory, any worker exceeding the limit will be
        # immediately killed without waiting for the end of the current request
        # processing. Defaults to 2560MiB.
        limit_memory_hard = 2560000000
        args.append({'key': '$limit_memory_hard$', 'value': limit_memory_hard})
        ...
        # to 60.
        limit_time_cpu = 60
        args.append({'key': '$limit_time_cpu$', 'value': limit_time_cpu})
        ...
        limit_time_real = 120
        args.append({'key': '$limit_time_real$', 'value': limit_time_real})

        cmd = CreateOdooConfig(
            self,
            command=command,
            args=args,
            usr_msg=msg
    ...
    def run_client(self, client_name):
        ...
        # si tenemos nginx no hay que exponer los puertos
        if not self.nginx:
