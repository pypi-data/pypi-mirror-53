...
class AdminListView(
    ...
    def get_model_list_fields(self):
        ...
        result = []
        for field in Field.get_fields(self.request.user, self.model, has_domain=False):
            if field.listable:
    ...
    def get_relation_fields(self):
        user = self.request.user
        fields = [field for field in Field.get_fields(user, self.model, has_domain=True) if
