...
class CharsetNormalizerMatches:
    ...
    def from_bytes(sequences, steps=10, chunk_size=512, threshold=0.20, cp_isolation=None, cp_exclusion=None, preemptive_behaviour=True, explain=False):
        ...
        logger.disable('charset_normalizer.probe_coherence')
    ...
    def best(self):
        ...
        logger.info(
            'We need to choose between {nb_suitable_match} match. Order By Chaos Then Coherence. '
            'Remaining-List = [{remaining_enc}]',
            nb_suitable_match=len(self),
            remaining_enc=', '.join([el.encoding for el in self._matches])
        )
        ...
        logger.info(
            'Lowest Chaos found is {lowest_chaos} %. Reduced list to {nb_suitable_match} match. '
            'Remaining-List = [{remaining_enc}]',
            lowest_chaos=sorted_matches[0].percent_chaos,
            nb_suitable_match=nb_lowest_ratio+1,
            remaining_enc=', '.join([el.encoding for el in sorted_matches[:nb_lowest_ratio+1]])
        )
