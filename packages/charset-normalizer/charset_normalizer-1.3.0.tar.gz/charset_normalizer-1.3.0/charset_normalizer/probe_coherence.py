    ...
    def _probe(self):
        ...
        for language, letters in ProbeCoherence.FREQUENCIES.items():
            ...
            if ratio_unavailable_letters < COHERENCE_MAXIMUM_UNAVAILABLE_LETTER:
                ...
                logger.info('{} can be present, first prerequisite passed. {} % available letters.',
                            language,
                            round((1. - ratio_unavailable_letters) * 100, ndigits=3))
                ...
                if not_respected_rank_coeff < COHERENCE_MAXIMUM_NOT_RESPECTED_RANK and n_tested_verified_on >= COHERENCE_MIN_LETTER_NEEDED:
                    ...
                    logger.info(
                        '{} has been identified in text variant. Respected letter appearance is up to {} %.',
                        language,
                        round((1.-not_respected_rank_coeff)*100, ndigits=3)
                    )
                    ...
                    logger.warning(
                        '{} has been rejected in text variant. Respected letter appearance is up to {} %.',
                        language,
                        round((1. - not_respected_rank_coeff) * 100, ndigits=3)
                    )
