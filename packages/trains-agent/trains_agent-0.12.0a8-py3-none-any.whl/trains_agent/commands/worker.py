...
class Worker(ServiceCommandSection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._task_status_change_message = "Changed by {} {}".format(
            PROGRAM_NAME, self.worker_id
        )
    ...
    def daemon(self, queues, log_level, foreground=False, docker=False, **kwargs):
        ...
        retry_session = HTTPSession()
        retry = Retry(total=10 ** 5, backoff_factor=1)
        for scheme in ("http", "https"):
            retry_session.mount("{}://".format(scheme), HTTPAdapter(max_retries=retry))

        if not queues:
            default_queue = self._session.send_api(queues_api.GetDefaultRequest())
            queues = [default_queue.id]
            print('Using default queue "{0.name}" ({0.id})'.format(default_queue))
        else:
