...
class LoadEDSdata(qtw.QDialog):
    ...
    def set_data(self):
        ...
        if dset_path != "":
            save_dataset(dset_path, exp, stack, composite, pts)
...
def load_dataset():
    ...
    stack, composite, points = read_dataset(full_dset_path)
    exp = stack.parent
    if composite is None:
        results.append(gluedata_from_stack(exp.label+"maps", stack))
    else:
        results.append(gluedata_from_stack_and_composite(exp.label+"maps", stack, composite)) #, coords)
