...
namespace necsim
{
    ...
    shared_ptr<CommunityParameters> CommunitiesArray::addNew(long double speciation_rate, long double time,
                                                             bool fragment, unsigned long metacommunity_reference,
                                                             const ProtractedSpeciationParameters &protracted_params)
    {
    ...
    bool MetacommunityParameters::isMetacommunityOption() const
    {
        if(metacommunity_size > 0 && speciation_rate > 0)
        {
            if(option != "simulated" && option != "analytical")
            {
                throw FatalException(
                        "Must use simulation or analytical solutions when supplying a metacommunity size and "
                        "speciation rate.");
            }
            return true;
        }
        if(option != "simulated" && option != "analytical" && option != "none")
        {
            ...
            {
                throw FatalException(
                        "External reference cannot be 0 when reading metacommunity from external database.");
            }
            return true;
        }
        return false;
    ...
    unsigned long MetacommunitiesArray::addNew(const unsigned long &metacommunity_size,
                                               const long double &speciation_rate, const string &option,
                                               const unsigned long &external_reference)
    {
        unsigned long max_reference = 1;
