...
class NotRunnableError(exceptions.StandardError):
...
def run_experiment(experiment, metabolites=[], local_cpu=None):
    ...
    *outside* of the simulations loop (e.g. if the user's code failed to
    ...
    if not main_exception:
        ...
        bob = _build_sim_descs(experiment, metabolites)

        results = pool.imap_unordered(_run_simulation,
                                      _build_sim_descs(experiment, metabolites),
                                      1)
...
def _build_sim_descs(experiment, metabolites):
    ...
    """
    # We guarantee that we won't pass Unicode strings to the pulse seq
    # code, so every string has to be UTF-8 encoded. Rationale here:
    ...
    observe_isotope = experiment.isotope.encode("utf-8")
    ...
    for pulse_design in experiment.pulse_sequence.pulse_projects:
        ...
        pulse.name = pulse.name.encode("utf-8")
        pulses.append(pulse)
    ...
    for dims in experiment.all_indices:
        for metabolite in metabolites:
            ...
            sim_desc.dims = [metabolite.name.encode("utf-8")] + list(dims)
            sim_desc.met_iso = [spin.isotope.encode("utf-8")
                                        for spin
                                        in metabolite.spins]
...
def _scrub_traceback_entry(code_name, entry):
    ...
    """
    # The filenames in the traceback will point to either Simulation/Vespa
...
def _run_simulation(sim_desc):
    ...
    # Python type.
    f = lambda an_object: isinstance(an_object, (float, int, long))
    ...
    result = dict(zip(("ppms", "areas", "phases"), result))
    result["started"] = started
    result["completed"] = util_time.now()
    # The metab name was handed to this module as Unicode, but _build_sim_descs()
    # converted it to utf-8 before passing it to the user's code. It's 
    # probably still utf-8, and if so I need to convert it back to Unicode.
    metabolite_name = dims[0]
    if isinstance(metabolite_name, str):
        metabolite_name = metabolite_name.decode("utf-8")
    #else:
        # It's already unicode. This would only happen if the user code 
        # converted it, which is wildly unlikely.
    result["metabolite"] = metabolite_name
    result["dims"] = dims[1:]
