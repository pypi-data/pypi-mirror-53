...
#       real world, by appending "y" above four times to itself
    ...
    yout,  rw,  res  = _test_old(x,y,n,f,nsteps,delta,ys,rw,res)
    yout2, rw2, res2 = _test_new(x,y,n,f,nsteps,delta,ys,rw,res)
    
    print "Max abs difference - yout = " + str(max(abs(yout - yout2)))
    print "Max abs difference - rw   = " + str(max(abs(np.array(rw) - np.array(rw2))))
    print "Max abs difference - res  = " + str(max(abs(res - res2)))
