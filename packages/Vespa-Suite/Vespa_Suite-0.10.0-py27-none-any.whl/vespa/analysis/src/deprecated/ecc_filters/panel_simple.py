...
import util_ecc
import shiftopt
import auto_gui.ecc_simple as ecc_simple
from vespa.common.constants import Deflate
import vespa.common.util.xml_ as util_xml


class PanelSimple(ecc_simple.EccSimpleUI):

    def __init__(self, parent, tab, attributes):
        ecc_simple.EccSimpleUI.__init__(self, parent)
            ...
            for k in range(dims[3]):
                for j in range(dims[2]):
                    for i in range(dims[1]):
                        ...
                        dat = shiftopt.optimize_b0_shift(dat, block.sw)
                        raw_data[k,j,i,:] = dat[0]
