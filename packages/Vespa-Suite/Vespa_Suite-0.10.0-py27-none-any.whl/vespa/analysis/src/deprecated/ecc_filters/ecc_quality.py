...
#----------------------------------------------------------
# Quality Eddy Current Correction
...
def _create_panel(parent, tab, attributes):
    ...
    import panel_quality
    
    return panel_quality.PanelQuality(parent, tab, attributes)



class EccQuality(ecc_filter.EccFilter):
    
    XML_VERSION = "1.0.0"

    ID = "c1bb1a09-637f-4cc2-8304-a19cc83ccf96"
...
class _FunctEccQuality(functor.Functor):
    """ 
    ...
    def algorithm(self, chain):
        ...
        if self.ecc_dataset: 
            ...
            if len(indx[0]) > 0:
                ...
                replace = np.complex64(thresh * np.exp(1j * phase))
                ecc[indx[0]] = replace
