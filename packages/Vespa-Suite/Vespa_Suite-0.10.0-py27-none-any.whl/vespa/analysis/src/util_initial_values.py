...
# import vespa.analysis.src.util_file as util_file as ufile       # used for debug
...
from vespa.analysis.src.constants import FitMacromoleculeMethodInitVal
...
#    nmet    = len(set.prior_list)
    ...
    for i in range(nmet):
        ...
        if tabbr in ['cho','gpc+pcho','pcho+gpc','pcho','gpc']:
            ...
            ccr, prot = _get_metinfo( 'cr', chain, defconc=7.0, defprot=3.0)
    ...
    if set.macromol_model == FitMacromoleculeMethod.SINGLE_BASIS_DATASET:
        ...
        if set.macromol_single_basis_dataset_initval == FitMacromoleculeMethodInitVal.MANUAL:
            ...
            if chain.basis_mmol is not None:
                ...
                ppm_start = int(util_ppm.ppm2pts(ppm_start, dataset, acq=True))
                ppm_end   = int(util_ppm.ppm2pts(ppm_end  , dataset, acq=True))
