...
#             self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        if not kernel.hide_file1:
            val = {'variable':u'file1', 'type':u'(File)', 'value':u''}
            new_params.append(TransformParameter(val))

        if not kernel.hide_file2:
            val = {'variable':u'file2', 'type':u'(File)', 'value':u''}
            new_params.append(TransformParameter(val))

        if kernel.transform_kernel_controls:
            for item in kernel.transform_kernel_controls:
                if item.type == u'Double':
                    val = {'variable':item.variable, 'type':u'(Double)', 'value':float(item.default)}
                if item.type == u'Long':
                    val = {'variable':item.variable, 'type':u'(Long)',   'value':int(float(item.default))}
                if item.type == u'String':
                    val = {'variable':item.variable, 'type':u'(String)', 'value':item.default}
                if item.type == u'Choice':
                    val = {'variable':item.variable, 'type':u'(Choice)', 'value':0} # first item in list
                if item.type == u'Output':
                    val = {'variable':item.variable, 'type':u'(Output)', 'value':item.default}
    ...
    def parameters_to_dict(self):
        ...
        for item in self.parameters:
            ...
            elif item.type == u'(Long)':
                d[item.variable] = int(float(item.value))
            elif item.type == u'(String)':
                d[item.variable] = item.value
            elif item.type == u'(Choice)':
                d[item.variable] = int(float(item.value))
            elif item.type == u'(File)':
                d[item.variable] = item.value
            elif item.type == u'(Output)':
                d[item.variable] = item.value
