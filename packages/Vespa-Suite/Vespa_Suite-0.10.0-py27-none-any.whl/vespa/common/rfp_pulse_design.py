...
class PulseDesign(object):
    ...
    def __init__(self, attributes=None):
        ...
        self.calc_resolution      = constants.DEFAULT_CALC_RESOLUTION       # 5000 - num_points for Bloch and rfpulse profile calcs.
        self.pulse_bandwidth_type = constants.DEFAULT_PULSE_BANDWIDTH_TYPE  #'half_height', 'minimum' or 'maximum'
        self.gyromagnetic_nuclei  = constants.DEFAULT_GYROMAGNETIC_NUCLEI   # 1H
        ...
        self.transforms  = []  # This replaces provenance,
    ...
    def deflate(self, flavor=Deflate.ETREE):
        if flavor == Deflate.ETREE:
            # ppe = pulse design element
            ppe = ElementTree.Element("pulse_design", { "id" : self.id, "version" : self.XML_VERSION})

            for attribute in ("name", "created", "creator", "comment", ):
                util_xml.TextSubElement(ppe, attribute, getattr(self, attribute))

            util_xml.TextSubElement(ppe, "calc_resolution", str(self.calc_resolution))
            util_xml.TextSubElement(ppe, "pulse_bandwidth_type", str(self.pulse_bandwidth_type))
            util_xml.TextSubElement(ppe, "gyromagnetic_nuclei", str(self.gyromagnetic_nuclei))

            ppe.append(self.machine_specs.deflate(flavor))

            for transform in self.transforms:
                ppe.append(transform.deflate(flavor))
...
def _convert_project_to_design(db, rfpulse_project):
    ...
    # some constants
    kernel_id    = u'3cb7a9e2-bb23-4934-ae17-96c01b8d93c7'     # the Import From RFPuls transform
