...
class PlotPanel(wx.Panel):
    ...
    def __init__(self, parent, naxes=2,
        ...
        self.uses_collections = uses_collections
        self.unlink = unlink
...
#             if links:
#                 self.zoom.append(ZoomSpan(self, links,
#                                       useblit=True,
#                                       do_zoom_select_event=do_zoom_select_event,
#                                       do_zoom_motion_event=do_zoom_motion_event,
#                                       rectprops=props_zoom))
...
#             if links:
#                 self.zoom.append(ZoomBox(self, links,
#                                      drawtype='box',
#                                      useblit=True,
#                                      button=1,
#                                      do_zoom_select_event=do_zoom_select_event,
...
#             if links:
#                 self.refs.append(CursorSpan(self, links,
#                                         useblit=True,
#                                         do_refs_select_event=do_refs_select_event,
#                                         do_refs_motion_event=do_refs_motion_event,
#                                         rectprops=props_cursor))
...
#             if links:
#                 self.middle.append(MiddleEvents(self, links,
#                                             do_middle_select_event=do_middle_select_event,
#                                             do_middle_motion_event=do_middle_motion_event,
#                                             do_middle_press_event=do_middle_press_event))
#             for axes in unlinks:
...
#         unlinks = []
#         links   = []
#         if self.unlink is False:
#             links = self.all_axes
#         elif self.unlink is True:
#             unlinks = self.all_axes
#         elif self.unlink is []:
#             links = self.all_axes
#         elif type(self.unlink) is list:
#             all_indices = range(len(self.all_axes))
#             # check that all indices are within N axes
#             for i in self.unlink:
#                 if i not in all_indices:
#                     raise ValueError("Unlink index outside available indices.")  
#             # create unlinks and links lists
#             links = list(self.all_axes)
#             for i in self.unlink:
#                 unlinks.append(self.all_axes[i])
#                 links.remove(self.all_axes[i])
#         else:
#             raise ValueError("Unlink index value not True, False or a List.")
#         
#         return links, unlinks
    ...
    def get_values(self, event):
        """
        This function was made a method of the plot_panel object so users can
        overwrite it if necessary. This was the case for the alternative view
        object, plot_panel_transform_contour, used for B1 inhomogeneity plots.
        
        Default functionality, this method polls the axes that the mouse is 
        within to return a list of data values at the x location of the cursor.
        ...
        if event.inaxes:
            if self.uses_collections:
                ...
                # This code was set up to gather values from the Simulation
                #  1D plots that are stored in a Line Collection. It figures
                #  out the offset of the cursor and reaturns the path value
                #  for the x-position of that line plot.
