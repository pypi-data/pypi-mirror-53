...
def run_transform(transform, previous=None):
    ...
    *outside* of the simulations loop (e.g. if the user's code failed to
    ...
    try:
        ...
        algorithm_module = _create_temporary_module(module_name, 
                                                    "transform_",
                                                    algorithm_code)
        module = imp.load_source(algorithm_module.__name__, 
    ...
    if not main_exception:
        ...
        trans_desc = _build_trans_descs(transform, previous)
...
def _build_trans_descs(transform, previous):
    """
    ...
    """
#FIXME bjs - make the statement below true sometime soon
    
    # We guarantee that we won't pass Unicode strings to the transform
    ...
    if previous is not None:
        previous_rf         = np.copy(previous.result.rf_waveform)
        previous_rf_xaxis   = np.copy(previous.result.rf_xaxis)
        previous_gradient   = np.copy(previous.result.gradient)
        previous_grad_xaxis = np.copy(previous.result.grad_xaxis)
    else:
        previous_rf         = None
...
def _scrub_traceback_entry(code_name, entry):
    ...
    """
    # The filenames in the traceback will point to either RFPulse/Vespa
...
def _run_transform(trans_desc, _algorithm_function):
    ...
    result = dict(zip(("rf_waveform", "rf_xaxis", "gradient", "grad_xaxis"), result))
