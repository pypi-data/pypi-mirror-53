...
# Our modules
import auto_gui.machine_settings as machine_settings_module
...
class DialogMachineSettings(machine_settings_module.MyDialog):

    def __init__(self, parent, db, machine_settings=None, read_only=False,
                                                          is_template=False):
        
        # This dialog can be invoked in four different states. They are --
        # 1. Edit machine settings
        # 2. New machine settings template
        # 3. Edit machine settings template
        # 4. View machine settings template
        
        # Notably absent from the list are "New machine settings" and "View
        # machine settings". This dialog could be used for those purposes,
        ...
        machine_settings_module.MyDialog.__init__(self, parent)
        ...
        self.templates = db.fetch_machine_settings_templates()
        ...
        if machine_settings:
            ...
            # We're creating a new machine settings template
            self.read_only = False
            # Create a new machine_settings object based on the default
            self.machine_settings = copy.copy(self.default_template)
            self.machine_settings.is_default = False
            self.machine_settings.id = 0
            self.machine_settings.name = ""
    ...
    def on_template_selected(self, event):
        ...
        self.machine_settings = rfp_machine_settings.settings_from_template(template)
        
        self.populate_controls()

    
    def on_ok(self, event):
        if self.validate_machine_settings():
    ...
    def initialize_controls(self):
        ...
        if self.is_template:
            ...
            # Editing machine settings -- case 1 (see __init__ comment)
    ...
    def populate_controls(self):
        # populates controls with the data from self.machine_settings.
        if self.is_template:
            self.TextName.ChangeValue(self.machine_settings.name)
        self.TextFieldStrength.ChangeValue(str(self.machine_settings.field_strength))
        self.TextMaxB1Field.ChangeValue(str(self.machine_settings.max_b1_field))
        self.TextZeroPadding.ChangeValue(str(self.machine_settings.zero_padding))
        self.TextMinDwellTime.ChangeValue(str(self.machine_settings.min_dwell_time))
        self.TextDwellTimeIncrement.ChangeValue(str(self.machine_settings.dwell_time_increment))
        self.TextGradientRasterTime.ChangeValue(str(self.machine_settings.gradient_raster_time))
        self.TextGradientSlewRate.ChangeValue(str(self.machine_settings.gradient_slew_rate))
        self.TextGradientMaximum.ChangeValue(str(self.machine_settings.gradient_maximum))
    ...
    def validate_machine_settings(self):
        ...
        if not msg and self.is_template and not self.machine_settings.id:
            # I still have yet to check whether or not the name is unique.
            # I save it until last because it is a database hit.
            names = [template.name for template in self.templates]
            if name in names:
                msg = "A machine settings template with this name already exists.\n"  \
        ...
        if msg:
            ...
            # All is well!
            if self.is_template:
                self.machine_settings.name                 = name
            self.machine_settings.machine_type         = machine_type
            self.machine_settings.field_strength       = float(field_strength)
            self.machine_settings.max_b1_field         = float(max_b1_field)
            self.machine_settings.zero_padding         = int(zero_padding)
            self.machine_settings.min_dwell_time       = float(min_dwell_time)
            self.machine_settings.dwell_time_increment = float(dwell_time_increment)
            self.machine_settings.gradient_raster_time = float(gradient_raster_time)
            self.machine_settings.gradient_slew_rate   = float(gradient_slew_rate)
            self.machine_settings.gradient_maximum     = float(gradient_maximum)
