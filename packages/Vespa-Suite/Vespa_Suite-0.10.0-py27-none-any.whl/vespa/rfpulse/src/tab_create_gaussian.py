...
# Our modules
import auto_gui.panel_gauss as panel_gauss
...
class _PanelParameters(panel_gauss.PanelGaussian):
    def __init__(self, parent, inner_notebook):
        
        panel_gauss.PanelGaussian.__init__(self, parent)
    ...
    def initialize_controls(self):      
        ...
        self.ComboBoxFilterType.AppendItems( \
                                [common_constants.FilterType.NO_SELECTION['display'],
                                 common_constants.FilterType.COSINE['display'],
                                 common_constants.FilterType.HAMMING['display']])      
 
        
class TabCreateGaussian(tab_transform_base.TabTransformBase):
    ...
    def get_cooked_gui_data(self):
        ...
        for key in ("tip_angle", "duration", "bandwidth", "filter_application", ):
            d[key] = float(d[key])

        d["time_points"] = int(d["time_steps"])
            
        d["filter_type"] = common_constants.FilterType.get_type_for_value(d["filter_type"], 'display')
    ...
    def get_raw_gui_data(self):
        ...
        http://scion.duhs.duke.edu/vespa/rfpulse/wiki/CommonTabFeatures
        """
        # Get values from the CreateGaussian panel
