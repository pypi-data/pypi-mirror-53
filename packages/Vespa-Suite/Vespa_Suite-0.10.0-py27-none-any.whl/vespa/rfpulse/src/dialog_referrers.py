...
# Our modules
import auto_gui.referrers as gui_referrers
...
class DialogReferrers(gui_referrers.MyDialog):
    def __init__(self, parent, pulse_project):
        ...
        self.SetTitle("Pulse Sequences Using %s" % pulse_project.name)

        names = [referrer[1] for referrer in pulse_project.referrers]
    ...
    def on_copy(self, event):
        s = "Vespa %s:\n" % self.GetTitle()
        s += "\n".join([referrer[1] for referrer in self.pulse_project.referrers])
