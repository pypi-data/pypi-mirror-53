...
# Our modules
import auto_gui.third_party_export as third_party_export
...
def _write_lines(filename, lines):
    ...
    try:
        ...
        common_dialogs.message(msg, "RFPulse - Third Party Export")
...
class DialogThirdPartyExport(third_party_export.MyDialog):
    ...
    def __init__(self, parent, pulse_project_id, pulse, bandwidth, tip_angle):
        ...
        self.pulse_project_id = pulse_project_id
        self.pulse = pulse
    ...
    def on_browse(self, event): 
        ...
        if self.format == constants.ThirdPartyExportFormat.IDEA: 
            fname = fname +'.pta'
            filetype_filter = "RFPulse Third Party Export Filename (*.pta)|*.pta"
        elif self.format == constants.ThirdPartyExportFormat.VISION:
            fname = fname +'.pta'
            filetype_filter = "RFPulse Third Party Export Filename (*.pta)|*.pta"
        elif self.format == constants.ThirdPartyExportFormat.ASCII_MAGN_PHASE:
            fname = fname +'.txt'
            filetype_filter = "RFPulse Third Party Export Filename (*.txt)|*.txt"
        elif self.format == constants.ThirdPartyExportFormat.ANNOTATED_ASCII_MAGN_PHASE:
            fname = fname +'.txt'
            filetype_filter = "RFPulse Third Party Export Filename (*.txt)|*.txt"
        if self.format == constants.ThirdPartyExportFormat.IDEA_C_HEADER: 
            fname = fname +'.h'
            filetype_filter = "RFPulse Third Party Export Filename (*.h)|*.h"
    ...
    def populate_controls(self):
        ...
        comment = 'Vespa-RFPulse export: Project Name = %s '      \
                  ', UUID = %s, Duration = %s [ms] '            \
                  ', Bandwidth = %s [kHz], and Tip Angle = %s [deg]'
        comment %= (self.pulse.name, self.pulse_project_id, self.pulse.duration, self.bandwidth, self.tip_angle)
    ...
    def do_idea_export(self, d):
        waveform  = np.array(self.pulse.waveform)
    ...
    def do_ascii_magn_phase_export(self, include_annotation=False):
        ...
        if include_annotation:
            all_lines.append('# Exported from Vespa-RFPulse')
            ...
            all_lines.append('# Project UUID: ' + self.pulse_project_id)
