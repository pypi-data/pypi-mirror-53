...
class EventLog(Base):
    ...
    log_flag = db.Column(db.String(64), nullable=False)
    log_message = db.Column(db.JSON)
    log_timestamp = db.Column(db.TIMESTAMP, default=datetime.datetime.utcnow, nullable=False)
    log_type = db.Column(db.String(64), nullable=False)

    device_skiply_id = db.Column(db.String(255), nullable=False)
...
def add_log(device_id, log_type, log_flag, log_message):
    log = LogStep(device_id, log_type, log_flag, log_message)
    db_session.add(log)
    db_session.commit()