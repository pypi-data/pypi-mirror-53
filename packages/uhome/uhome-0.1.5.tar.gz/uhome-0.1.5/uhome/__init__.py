"""
Uhome is python wrapper for communication with Uponor Smatrix Wave PLUS Smart Home Gateway R-167 aka U@home.
...
class Uhome(object):
    ...
    def update(self):
        ...
        # update module keys
        self.__update_keys(self.uhome_module_keys)
        # update controller keys
        for uc in self.uhome_controllers:
            self.__update_keys(uc.uhome_controller_keys)
        # update thermostat keys
        for t in self.uhome_thermostats:
            self.__update_keys(t.uhome_thermostat_keys)
        ...
        for uc in self.uhome_controllers:
            ...
            for i in range(0, 12):
                ...
                data = '{"jsonrpc": "2.0", "id": 8, "method": "read", "params": {"objects": [{"id": "' + \
                       str(UHOME_THERMOSTAT_KEYS['room_setpoint']['addr'] + (500 * uc.index) + (40 * i)) + \
                       '", "properties": {"85": {}}}]}}'
                response_data = self.__do_rest_call(data)
                try:
                    rs = response_data['result']['objects'][0]['properties']['85']['value']
                    if 0 < rs < 50:
    ...
    def init_controllers(self):
        ...
        for i in range(0, 4):
            ...
            data = '{"jsonrpc": "2.0", "id": 8, "method": "read", "params": {"objects": [{"id": "' + \
                   str(UHOME_CONTROLLER_KEYS['controller_sv_version']['addr']+(500 * i)) + \
                   '", "properties": {"85": {}}}]}}'
            response_data = self.__do_rest_call(data)
            try:
                sw = response_data['result']['objects'][0]['properties']['85']['value']
                if sw != '0.00':
