...
from passit import passit
from unittest.mock import MagicMock


class passitTest(unittest.TestCase):
    """ Test suite for the passit module """

    def setUp(self):
        self.key = "ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
        self.mock = MagicMock()
        self.mock[0] = passit._shuffle_key(self.key)
    ...
    def test_default_key_generation_is_one_key(self):
        self.assertEqual(len(passit.generate_random_keys()), 1)

    def test_default_key_generation_key_length_is_16(self):
        self.assertEqual(len(passit.generate_random_keys()[0]), 16)

    def test_generate_max_100_keys(self):
        self.assertEqual(len(passit.generate_random_keys(100)), 100)

    def test_error_returns_correct_message_if_keys_greater_than_100(self):
        self.assertEqual(passit.generate_random_keys(
    ...
    def test_all_too_large_errors_can_concatenate(self):
        self.assertEqual(passit.generate_random_keys(101, 257),
