...
def get_api_data(dataitem,item,parameters=None):
    ...
    try:
        ...
        logger.error("Unable to obtain data from Norgate Data - perhaps NDU is not running?")
        sys.exit()
    return apiresponse
...
def validate_api_response(r,symbol):
    ...
    if (not (r.status == 200 or r.status == 204)) :
        logger.error(inspect.currentframe().f_back.f_code.co_name + ": Error in receiving Norgate Data - check paramters are correctly formatted.  Status code is: " + str(r.status))
        sys.exit()
