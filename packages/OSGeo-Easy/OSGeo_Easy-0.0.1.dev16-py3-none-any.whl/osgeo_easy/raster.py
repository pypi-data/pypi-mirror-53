...
def read_and_fill(raster_ref: Union[str, gdal.Dataset], value: Union[np.int, np.float32, np.float64]) -> gdal.Dataset:
    """
    Reads a raster file and se all of its values to a single value. Used to obtain the grid.
    :param raster_ref:
    :param value:
    :return: New single-value gdal.Dataset filled with the single value given
    """

    # read file and get a filled matrix
    base_ds = read(raster_ref) if type(raster_ref) is str else raster_ref
