...
class Cache(object):
    ...
    def build_key(self, key_base, **kwargs):
        logger.info('Cache.build_key() entered')
        primer_cls = self.primer(key_base)
        logger.info('Cache.build_key() found primer for {}'.format(key_base))
        if not primer_cls.parameters:
            logger.info('Cache.build_key() - returning: {}'.format(key_base))
            return key_base
        parts = [key_base]
        logger.info('iterating over {} parameters'.format(len(primer_cls.parameters)))
        for p in primer_cls.parameters:
            if not issubclass(p, BaseParameter):
                raise UnknownParameterType(p)
            parts.append(p.to_str(kwargs[p.name]))
        logger.info('Cache.build_key() - returning: {}'.format(KEYSEP.join(parts)))
