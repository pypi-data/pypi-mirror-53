    ...
    def _query_osm(self, request):
        ...
        if not self.__cmd:
            ...
            with open(os.devnull, 'w') as devnull:
                process = subprocess.run('{cmd}'.format(cmd=self.__cmd), input=encoded_request, stdout=subprocess.PIPE,
                                         stderr=devnull)
...
def weighted_position(particles, weights):
    ...
    Args:
        particles (np.ndarray): A list of positions
        weights (np.ndarray):   A list of weights
    ...
    if particles.shape[0] == 2:
        ...
        return np.dot(particles.T, weights)
