...
def tilemapbase_project_numpy(longitude, latitude):
    """Project the longitude / latitude coords to the unit square using numpy.
    ...
    Returns:
         Coordinates `(x,y)` in the "Web Mercator" projection, normalised to be in the range [0,1].
    """
...
def draw_tile(lat, lon, ax=None, style=tilemapbase.tiles.Stamen_Toner):
    """
    ...
    plotter = tilemapbase.Plotter(extent, style, zoom=12)
...
def tilemap_project(particles, zone_number, zone_letter):
    ...
    return tilemapbase_project_numpy(latitude=lat, longitude=lon)
...
def mark_locations(utmlocations, descriptions, sc_kwargs, ax, xoffset=0.01):
    """
    Mark locations in a plot.
    Args:
        utmlocations (list[UTMLocation]):   A list of UTMLocation to mark in the plot
    ...
    assert len(utmlocations) == len(descriptions)
    assert len(utmlocations) == len(sc_kwargs)

    right_edge = xoffset

    color_map = ListedColormap(color_palette(num_color=len(utmlocations))).colors

    for idx, (location, description, kwargs) in enumerate(zip(utmlocations, descriptions, sc_kwargs)):

        if 'c' not in kwargs:
            kwargs['c'] = color_map[idx]

        if 'ax' in kwargs:
            kwargs['ax'] = ax

        mark_location(location, ax=ax, marker='o', color=kwargs['c'], **kwargs)

        # show the legend for the PF location
        ax.add_patch(Circle((right_edge, 0.96 - (0.04 * idx)), 0.01, fc=kwargs['c'], alpha=kwargs.get('alpha', 0.8),
