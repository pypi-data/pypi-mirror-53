...
class StoreMeta:
    ...
    def __setitem__(self, key, data):
        ...
        if elem is None:
            ...
            copied_data = copy(elem.data)
            if isinstance(copied_data, dict) or \
               (isinstance(key, int) and isinstance(copied_data, list)):
                elem.data[key] = data
                elem.update = datetime.utcnow()

                self.data = copied_data #elem.data
                self.update = elem.update.strftime("%Y-%m-%dT%H:%M:%S")
            else:
                raise Exception('data not dict!')
...
class Store(object):
    ...
    def __getattribute__(self, key):
        ...
        if elem:
            self.validate(copy(elem.data), meta=elem.meta )
    ...
    def __getitem__(self, key):
        ...
        for elem in elems:
            self.validate(copy(elem.data), meta=elem.meta, extra=elem.key )
    ...
    def query_meta(self, key, for_update=False):
        ...
        for elem in elems:
            self.validate(copy(elem.data), extra=elem.key, meta=elem.meta)
        return StoreMetas(elems, store=self.store)
