...
def load_vectors_from_azure(vectors):
    ...
    for p in parquet_files:
        ...
        project = vector_key.split('_')[0]
        customer_name = vector_key.split('_')[1]
        customer_type = p.split(customer_name)[0].split('/')[-2]
        try:
            status = int(db.get_status_project('status_projects', customer_name, customer_type, 'similars', project)[0])
        except Exception as e:
            status = 0
        ...
        if status == 2:
            ...
            if vector_data is not None:
                vectors[(p.split('/part')[0]).split('/')[-1].split('.parquet')[0]] = vector_data
                logger.info('Parquet %s  loaded successfully' % (p.split('/part')[0]).split('/')[-1].split('.parquet')[0])
            else:
                logger.info(
                    'Parquet %s not loaded' % (p.split('/part')[0]).split('/')[-1].split('.parquet')[0])
