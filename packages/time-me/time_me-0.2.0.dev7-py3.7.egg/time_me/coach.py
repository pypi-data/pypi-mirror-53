...
class Coach:
    ...
    class ResultComparison(Sequence[Result]):
        ...
        def bar(self, autoshow=True):
            ...
            best = self[0]
            if best > 0:
                plt.text(0, best / 2, f'{float(best):.1} sec', ha='center', wrap=True)

                for i, not_best in enumerate(self[1:], 1):
                    plt.text(i, not_best / 2, f'{float(not_best):.1} sec, {not_best / best:.1f}x {best.runner.__name__}', ha='center', wrap=True)
