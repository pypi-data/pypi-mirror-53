...
class ProductCounter(object):
    ...
    def __init__(self, retailer, date, ttl, execution_id=None, store='all', host='localhost', port=6379):
        self._retailer = retailer
        self._date = date
        self._execution_id = execution_id
        self._host = host
        self._port = port
        self._db = redis.Redis(host=host, port=port, db=self.database_name(date))
        self._name_template = '{}:{}:{}:{}:{}:{}:{}'
        self._ttl = ttl
        self._store = store
