...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_estimators='auto', perc=100, alpha=0.05,
                 two_step=True, max_iter=100, random_state=777, verbose=0):
        self.estimator = lgb.LGBMClassifier(boosting_type='rf', num_leaves=256, max_depth=5, learning_rate=0.01,
                                            class_weight='balanced', min_child_samples=1,
                                            subsample_freq=1, subsample=0.7, colsample_bytree=1,
                                            random_state=777, n_jobs=4, importance_type='gain')
    ...
    def _get_imp(self, X, y):
        try:
            n_ft = X.shape[1]
            self.estimator.set_params(colsample_bytree=np.sqrt(n_ft) / n_ft)
