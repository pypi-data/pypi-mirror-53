...
class SharedTopicPubSubPullSensor(PubSubPullSensor):
    ...
    def poke(self, context):
        ...
        if not self._messages:
            ...
            # message the sensor is waiting for. As soon as one does then stop looping 
            # and return from the function.
            for m in self._messages:
                if m.get('ackId') and m.get('message') and base62.b64decode(m['message']['data']) == self.trigger_msg:
