...
def authenticate(
        user_g_key='user',
        sign_in_endpoint=None,
        sign_out_endpoint=None,
        mfa_required=None,
        mfa_authorize_endpoint=None,
        mfa_authorized_attr='mfa_authorized',
        mfa_enable_endpoint=None,
    ...
    def authenticate(state):
        ...
        # Build endpoints for the link
        _mfa_authorize_endpoint = mfa_authorize_endpoint \
                or state.manage_config.get_endpoint('mfa_auth')

        _mfa_enable_endpoint = mfa_enable_endpoint \
                or state.manage_config.get_endpoint('mfa_enable')

        _sign_in_endpoint = sign_in_endpoint \
                or state.manage_config.get_endpoint('sign_in')

        _sign_out_endpoint = sign_out_endpoint \
                or state.manage_config.get_endpoint('sign_out')
        ...
        if not state.manage_user:
            ...
            redirect_url = flask.url_for(
                ...
                next=secure.safe_redirect_url(
                    full_path,
                    [flask.url_for(_sign_out_endpoint)]
                )
            )
        ...
        if _mfa_required:
            if not getattr(state.manage_user, mfa_enabled_attr, None):
                return flask.redirect(flask.url_for(_mfa_enable_endpoint))
...
def mfa_authenticate_scoped_session(
    ...
    def mfa_authenticate_scoped_session(state):
        ...
        # Build endpoints for the link
        _authorize_endpoint = authorize_endpoint \
                or state.manage_config.get_endpoint('mfa_auth')
...
def mfa_end_scoped_session(get_cache=None):
    """
