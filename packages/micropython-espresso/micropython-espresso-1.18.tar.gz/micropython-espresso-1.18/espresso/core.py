...
if _flash_dir:
    _espresso_conf_file = '/flash/espresso.conf'
else:
...
def espresso_repl(t):
    ...
    if script:
        ...
        # duplicate the stdout and create new one in BytesIO
        espresso_stdout = BytesIO()

        try:
            uos.dupterm(espresso_stdout)
        except:
            print('No "uos.dupterm" in this MicroPython implementation,'
                  'no way to get stdout and sdterr.')
