...
class Schedule:
    ...
    def datetime_info(self):
        ...
        x = games['gameDate']
        date_today = dt.today().date().strftime('%B %d, %Y')
        date_tomorrow = \
            (dt.today().date() + timedelta(days=1)).strftime('%B %d, %Y')
        # If date is today, display "Today"
        # If date is tomorrow, display "Tomorrow"
        # Else, display actual date
        date_option = {
            date_today: "Today",
        ...
        next_game_date = date_option.get(default_date, default_date)
        next_game_time = \
        ...
        if response.status_code == requests.codes.ok:
            if data['totalItems'] == 1:
                ...
                if plays != []:
                    ...
                    shot_type = plays[-1]['result']['secondaryType']
