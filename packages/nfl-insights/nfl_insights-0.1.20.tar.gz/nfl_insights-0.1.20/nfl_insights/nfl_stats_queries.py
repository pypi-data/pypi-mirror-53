...
class NFLStatsQueries():
    ...
    def pbp_get_composed_stat(self, compstat, object, playDimention='all', gameDimention='game', filter='True'):
        ...
        numeratorDF[compstat] = df[numeratorDef['AggField']]/df[denominatorDef['AggField']+'_dn']*compstatDef['StatRatio']
        numeratorDF.sort_values(by=compstat, ascending=False, inplace=True)
        numeratorDF['rank'] = numeratorDF[compstat].rank(method='dense', ascending=False)
        return numeratorDF
...
def test():
    ...
    #df = generator.pbp_get_stat('passAttempts', 'offenseTeam', 'all', 'ownTerritory',aggfunc='sum', filter="df.season=='2018-regular'")
    df = generator.pbp_get_composed_stat('completionRate', 'passingPlayer')
