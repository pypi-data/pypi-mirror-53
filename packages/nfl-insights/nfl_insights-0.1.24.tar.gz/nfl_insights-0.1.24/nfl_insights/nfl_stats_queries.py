...
class NFLStatsQueries():
    ...
    def _save_trace_df(self, traceDF, initialColumns, spreadId, sheetName):
        if not spreadId:
        ...
        traceDF[finalColumns].to_csv('{}.csv'.format(sheetName))
        url = self.bqu.upload_file_to_gcp('sport-uploads', '{}.csv'.format(sheetName), 'NFL/temp/{}.csv'.format(sheetName))
        print('trace can be found in this url:', url)

    def pbp_get_stat(self, statName, object, playDimention='all', gameDimention='game', aggfunc=None, playType=None, filter='True', trace=False, traceSpreadId=None, traceSheetName=None):
        ...
        if trace:
            self._save_trace_df(filteredDF, finalDF.columns, traceSpreadId, traceSheetName)
        #
...
def test():
    ...
    df = generator.pbp_get_stat('yardsRushed', 'rushingPlayer', 'all', 'game',aggfunc='sum', filter="df.season=='2019-regular'", trace=True)
