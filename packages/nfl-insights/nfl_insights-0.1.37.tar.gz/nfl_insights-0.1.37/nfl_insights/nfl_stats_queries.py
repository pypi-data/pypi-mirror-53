...
class NFLStatsQueries():
    ...
    def pbp_get_stat(self, statName, object, playDimention='all', gameDimention='game', aggfunc=None, playType=None, filter='True', trace=False, playerId = None, teamId=None):
        ...
        if statObject=='team':
            finalDF.columns=['teamId', 'teamName', 'statValue', 'count']
        else:
            finalDF.columns=['playerId', 'firstName', 'lastName', 'position', 'teamId', 'teamName', 'statValue', 'count']
...
def test():
    ...
    df = generator.pbp_get_stat('rushTD', 'rushingPlayer', 'all', 'game', filter="df.season=='2019-regular'", trace=False, playerId=12606)
    logger.info('Columns={}, Shape={}\n{}'.format(df.columns, df.shape, df))
