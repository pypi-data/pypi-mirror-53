...
class NFLStatsQueries():
    ...
    def _generate_pbp_query(self):
        exceptionList = [
            'description', 'blockingPlayer', 'penalty', 'recoveringTeam', 'fumblingTeam',
    ...
    def pbp_get_stat(self, statName, object, playDimention='all', gameDimention='game', aggfunc=None, playType=None, filter='True', trace=False, playerId = None, teamId=None):
        ...
        groupingeval = "filteredDF.groupby({groupby}, as_index=False).agg({}'{aggField}': '{aggFunc}'{}).sort_values(by='{aggField}', ascending=False)"
        ...
        if statObject=='team':
            finalDF.columns=['teamId', 'teamName', 'statValue']
        else:
            finalDF.columns=['playerId', 'firstName', 'lastName', 'position', 'teamId', 'teamName', 'statValue']
...
def test():
    ...
    df = generator.pbp_get_stat('yardsRushed', 'offenseTeam', 'all', 'game', filter="df.season=='2019-regular'", trace=False, teamId=56)
    logger.info('Columns={}, Shape={}\n\n{}\n\n'.format(df.columns, df.shape, df))
    df = generator.pbp_get_stat('rushes', 'rushingPlayer', 'all', 'game', filter="df.season=='2019-regular'", trace=False, playerId=12807)
    logger.info('Columns={}, Shape={}\n{}'.format(df.columns, df.shape, df))
...
if __name__ == '__main__':
    contendo_logging_setup(default_level=logging.ERROR)
    test()
