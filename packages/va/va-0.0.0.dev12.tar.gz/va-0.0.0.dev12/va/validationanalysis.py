...
class ValidationAnalysis:
    ...
    def density_distribution(self):
        ...
        try:
            ...
            datadict = {
                'density_distribution': {'y': np.round(newhist, 4).tolist(), 'x': np.round(bin_edges[:-1], 4).tolist()}}
        except:
        ...
        if self.models is None:
            ...
            for key, value in combinresult.items():
                try:
                    ...
                    datadict[str(counter)] = {'name': key, 'level': [round(elem, 4) for elem in levels.tolist()],
                                              'all_atom': [round(elem, 4) for elem in result[0]],
                                              'backbone': [round(elem, 4) for elem in result[1]]}

                    # import matplotlib.pyplot as plt
                    # plt.plot([round(elem, 4) for elem in levels.tolist()], [round(elem, 4) for elem in result[0]])
                    # plt.plot([round(elem, 4) for elem in levels.tolist()], [round(elem, 4) for elem in result[1]])
                ...
                try:
                    for contour, keysvalues in allcontoursdict.items():
                        ...
                        contourkey = str(round(float(contour), 4))
    ...
    def volumecontour(self):
        ...
        try:
            ...
            datadict = {
                'volume_estimate': {'volume': np.round(tmpresult, 4).tolist(), 'level': np.round(bins, 4).tolist()}}
        except:
            ...
            try:
                ...
                for i in range(len(indiaxis)):
                    if i != len(indiaxis) - 1:
                        ...
                        psum = log10(psmap.fullMap[tuple(indices.T)].sum() / len(indices))
                        aps.append(psum)
                datadict = {'rotationally_averaged_power_spectrum': {'y': np.round(aps, 4).tolist(),
                                                                     'x': np.round(indiaxis[:-1], 4).tolist()}}
                import matplotlib.pyplot as plt
                plt.plot(np.round(indiaxis[:-1], 4).tolist(), np.round(aps, 4).tolist())
                plt.savefig('test')
