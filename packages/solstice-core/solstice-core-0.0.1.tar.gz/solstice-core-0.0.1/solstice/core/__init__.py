...
"""
Initialization module for solstice-core
"""
...
def init(do_reload=False, import_libs=True):
    ...
    class SolsticeCore(importer.Importer, object):
        def __init__(self):
            super(SolsticeCore, self).__init__(module_name='solstice.core')
    ...
    if import_libs:
        ...
        import artellapipe.utils
        artellapipe.utils.init(do_reload=do_reload)
        import artellapipe.gui
        artellapipe.gui.init(do_reload=do_reload)
        import artellapipe.core
        artellapipe.core.init(do_reload=do_reload)
    ...
    artellapipe.core.set_project(project.Solstice)
...
def get_logging_level():
    ...
    if os.environ.get('SOLSTICE_CORE_LOG_LEVEL', None):
        return os.environ.get('SOLSTICE_CORE_LOG_LEVEL')

    return os.environ.get('SOLSTICE_CORE_LOG_LEVEL', 'WARNING')
