...
# @cython.boundscheck(False)
# cdef IdsScores sorted_multi_sum_scores_must_internal(list ids_list, list scores_list, assume_sorted=False):
#     # this method does not use the divide and conquer because it is not parallelizable
#     cdef int l = len(ids_list)



# @cython.boundscheck(False)
# def sorted_multi_interleave_scores(list ids_list, list scores_list, assume_sorted=False):
#     cdef IdsScores ids_scores = sorted_multi_interleave_scores_internal(ids_list, scores_list, assume_sorted=assume_sorted)
#     return (np.array(ids_scores.ids, dtype=np.int32), np.array(ids_scores.scores, dtype=np.float32))
