...
def validateCompoundQuery(query, mapping):
...
def validateBoost(query, mapping):
    assert type(query) is dict
    assert len(query) == 2
    ...
    assert len(q) == 1
    root_op = [key for key in q][0]

    assert root_op in query_validators

    query_validators[root_op](q[root_op], mapping)

    return True

def validateScoredTerm(query, mapping):
    assert type(query) is dict
    assert len(query) == 2
    ...
    assert field_type == 'scored_keyword'

    assert type(value) is str

    return True

def validateInterleave(query, mapping):
    assert type(query) is dict
    assert 'queries' in query
    for key in query['queries']:
        validateQuery(query['queries'][key], mapping)
    return True
...
query_validators = {
    ...
    'filter': validateCompoundQuery,
    'must': validateCompoundQuery,
    'should': validateCompoundQuery,
    'must_not': validateCompoundQuery,
    'term': validateTerm,
    'terms': validateTerms,
    'match': validateMatch,
    'normalized_match': validateMatch,
    'range': validateRange,
    'fieldBoost': validateFieldBoost,
    'field_boost': validateFieldBoost,
    'boost': validateBoost,
    'scored_term': validateScoredTerm,
    'interleave': validateInterleave,
}
