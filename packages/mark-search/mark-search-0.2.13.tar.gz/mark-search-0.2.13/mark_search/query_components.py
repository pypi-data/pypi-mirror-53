...
from .query_result import IdsQR, IdsScoresQR, IdsScoreQR, EmptyQR
from mark_search.combine_scores import sorted_multi_sum_scores_must, sorted_intersect1d, \
    sorted_multi_sum_scores, sorted_multi_union1d#, sorted_multi_interleave_scores
...
# class Interleave(QueryComponent):
#     def __init__(self, context):
#         super().__init__('interleave', context, None)
...
#         ids_list = [e.getIds() for e in candidate_list]
#         scores_list = [e.getScores() for e in candidate_list]
#         ids, scores = sorted_multi_interleave_scores(ids_list, scores_list, assume_sorted=True)
#         return IdsScoresQR(ids, scores)
...
class Terms(QueryComponent):
    ...
    def _score(self, field, terms):
        ...
        if type(terms) is str:
            ...
            tokens = terms
        if len(tokens):
            term_queries = [Term(self.context)._score(field, term, score=1) for term in tokens]
...
class Match(QueryComponent):
    ...
    def _score(self, field, terms, normalize=False):
        tokens = []
        if type(terms) is str:
            tokens = self.context.index.analyze(field, terms, return_text=False)
        elif type(terms) is list:
        ...
        if len(tokens):
            ...
            if normalize:
                ...
                for tqr in term_queries:
                    if tqr.scores.size:
                        max_possible_score += tqr.scores[0]
                if max_possible_score > 0:
                    or_query_result.scores /= max_possible_score
            return or_query_result
        else:
            return EmptyQR()
