...
from .query_validator import validateQuery

from .postings_prefetch import prefetchPostings
...
def termsHandler(value, qc):
    field = [key for key in value][0]
    terms = value[field]
    
    if qc.setting == 'filter':
        return Terms(qc)._filter(field, terms)
    elif qc.setting == 'score':
        return Terms(qc)._score(field, terms)
    else:
        assert False
...
def normalizedMatchHandler(value, qc):
    field = [key for key in value][0]
    terms = value[field]
    ...
    if qc.setting == 'filter':
        ...
        return Match(qc)._score(field, terms, normalize=True)
    else:
        assert False
...
def fieldBoostHandler(value, qc):
    ...
    if qc.setting == 'filter':
        ...
        if boost != 1:
            # TODO: maybe in index.getPostings, we make a copy for the query
            b = IdsScoresQR(a.ids, np.array(a.scores, dtype=np.float32) * boost)
            return b
        else:
            return a
    else:
...
def boostHandler(value, qc):
    ...
    if qc.setting == 'filter':
       ...
       query_type = [key for key in query][0]
       qr = query_part_handlers[query_type](query[query_type], qc)
       qr.scores *= np.multiply(qr.getScores(), boost)
       return qr
    else:
        assert False


def scoredTermHandler(value, qc):
    field = value['field']
    val = value['value']

    return  qc.index.getPostings(field, val)

def interleaveHandler(value, qc):
    queries = value['queries']
    n_from_each = value.get('n_from_each', 100)

    sub_query_results = []

    for query_name in queries:
        results = doQuery(queries[query_name],
            index=qc.index,
            limit=n_from_each,
            offset=0,
            min_score=0,
        ...
        sub_query_results.append((results.get('docs', []), results.get('scores', [])))
    
    ids = []
    scores = []
    curr_score = n_from_each * len(sub_query_results)

    for i in range(n_from_each):
        for j in range(len(sub_query_results)):
            if i < len(sub_query_results[j][0]):
                ids.append(sub_query_results[j][0][i])
    ...
    #     ids = np.array(results.get('ids', []), dtype=np.int32)
    #     scores = np.array(results.get('scores', []), dtype=np.float32)

    #     sort_mask = np.argsort(ids)

    #     sub_query_results.append(IdsScoresQR(np.array(ids[sort_mask]), np.array(scores[sort_mask])))
    
    # return Interleave(qc)._score(sub_query_results)
...
query_part_handlers = {
    ...
    'terms': termsHandler,
    'match': matchHandler,
    'normalized_match': normalizedMatchHandler,
    'range': rangeHandler,
    'fieldBoost': fieldBoostHandler,
    'field_boost': fieldBoostHandler,
    'boost': boostHandler,
    'scored_term': scoredTermHandler,
    'interleave': interleaveHandler
}
...
def doQuery(query,
    ...
    validateQuery(query, index.mapping)

    validation_done_time = time.time()

    has_interleave = query.get('interleave') is not None

    if prefetch and not has_interleave:
        prefetchPostings(query, index)

    prefetch_done_time = time.time()
    ...
    if return_documents:
        response['hits'] = index.getDocuments(doc_ids) #[index.getDocument(i) for i in doc_ids]

    response_done_time = time.time()

    response['validation_took'] = validation_done_time - start_time
    response['prefetch_took'] = prefetch_done_time - validation_done_time
    response['query_took'] = query_done_time - prefetch_done_time
    response['response_took_total'] = response_done_time - start_time
