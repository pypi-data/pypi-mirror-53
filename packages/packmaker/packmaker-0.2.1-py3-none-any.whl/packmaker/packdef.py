...
class PackDefinition (object):
    ...
    def load_one(self, filename):
        ...
        # check for env variable substitution
        subst = PackDefinition.SUBST_REX.search(packtext)
        while subst:
            var = subst.group(1)
            try:
                ...
                if substval is None:
                    raise Exception('Undefined environment variable in pack definition file: {}'.format(var))
            packtext = re.sub(r'\${%s}' % subst.group(0)[2:-1], substval, packtext)
            subst = PackDefinition.SUBST_REX.search(packtext)
