...
class CredentialStorePlugin(PluginBase):
    ...
    def get_private_key_list(self, cookie, session_cookie, **kwargs):
        ...
        return self.__fetch_credentials(self.do_get_private_key_list, 'private_keys')

    def get_password_list(self, cookie, session_cookie, **kwargs):
        self._setup_self(cookie, session_cookie, **kwargs)
        return self.__fetch_credentials(self.do_get_password_list, 'passwords')
    ...
    def session_ended(self, cookie, session_cookie, **kwargs):
        ...
        if not self._get_private('check_in_called', False) and self.account:
            self.logger.info(
                "Executing check-in as it was missed/failed in authentication_completed account={} asset={}".
                format(self.account, self.asset)
            )
            self.do_check_in_credential()
            self._set_private('check_in_called', True)
