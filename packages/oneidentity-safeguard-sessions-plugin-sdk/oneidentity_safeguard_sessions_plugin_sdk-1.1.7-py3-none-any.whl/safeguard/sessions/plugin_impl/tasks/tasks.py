...
from safeguard.sessions.plugin.data.aa_sample import scenarios
...
@task(
    help={'config_file': 'Name of config file',
          'target_username': 'Username on the server',
          'gateway_username': 'Username on the gateway'},
    pre=[check_manifest, check_entry_point],
)
def run_scenario(c, config_file=None, target_username=None, gateway_username=None):
    config = ''
    if config_file:
    ...
    plugin_module = load_source('plugintotest', manifest.entry_point)
    AAPluginTester.run_rdp_authenticate(plugin_module.Plugin, config, target_username, gateway_username)
...
@task(
    help=dedent("""
        ...
        $ pytest --backend-service=use
    """),
    pre=[ensure_conftest_py],
)
def run_interactive_tests(c):
    c.run("pytest --backend-service=use")


@task(
    help=dedent("""
        Run tests in interactive mode, save modified test data. Equivalent to
        $ pytest --backend-service=record
    """),
    pre=[ensure_conftest_py],
)
def run_recording_tests(c):
    c.run("pytest --backend-service=record")
