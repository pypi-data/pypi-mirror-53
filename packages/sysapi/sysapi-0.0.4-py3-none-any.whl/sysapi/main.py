...
from sysapi.backend import do_get_user, do_get_users, do_get_snapshots, do_get_snapshot, do_create_user
...
@app.get("/")
def read_root():
    return {"service": "sysapi", "version": VERSION}
...
def get_snapshot(snapshot: str = Path(...,min_length=1, regex="^@[a-zA-Z0-9-_']+$")) :
    if snapshot in do_get_snapshots():
        return SnapshotModel(name=snapshot)
    else:
        raise HTTPException(status_code=404, detail="Snapshot unknown")

@app.get("/v1/at/{snapshot}/{username}/{path:path}", response_model=FileModel)
...
@app.get("/v1/at/{snapshot}/{username}/{path:path}/", response_model=DirectoryModel)
def get_dir_at_path(
    path: str = Path(None, regex='^.*[^/]$'),
    snapshot: str = Path(...,min_length=1, regex="^@[a-zA-Z0-9-_']+$"), 
    username: str = Path(...,min_length=1, regex="^[a-zA-Z'][a-zA-Z0-9-_']*$")
...
@app.get("/v1/before/{snapshot}/{username}/{path:path}", response_model=FileModel)
...
@app.get("/v1/before/{snapshot}/{username}/{path:path}/", response_model=DirectoryModel)
...
@app.get("/v1/past/{snapshot}/{username}/{path:path}", response_model=FileModel)
