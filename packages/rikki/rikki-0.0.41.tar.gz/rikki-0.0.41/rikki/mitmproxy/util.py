        ...
        if request:
            ...
            if request.params and not contains_params(flow.request.query, request.params):
                if flow in result:
                    result.remove(flow)
                continue
            if request.content and not compare_content(request.content, flow.request.content.decode("utf8")):
                if flow in result:
                    result.remove(flow)
...
def compare_content(left: str, right: str) -> bool:
    """
