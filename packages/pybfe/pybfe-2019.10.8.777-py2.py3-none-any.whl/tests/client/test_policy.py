...
from pybfe.client.policy._policy import (
    _get_policy_details, load_policy, _update_policy_status, write_policy
)
from pybfe.client.policy._test import (
    _get_test_details, update_test_status, write_test
)
from pybfe.datamodel.policy import (
    Assert, CiProperties, Policy, Question, STATUS_ERROR, STATUS_FAIL, STATUS_PASS, Test)
...
def test_test_write():
    ...
    t = Test('t1')
    q1 = Question(name='q1')
    a1 = Assert(name='a1', status=STATUS_PASS, message='message1',
                questions=[q1])
...
def test_policy_write():
    ...
    policies = json.loads(
        session.get_snapshot_object_text('policies', snapshot='ss1'))
...
def test_policy_load():
    ...
    p1_pid1_t1_a1 = Assert(name='a1',
                           status=STATUS_PASS,
                           message='message',
                           questions=[p1_pid1_t1_a1_q1])
    p1_pid1_t1_q = Question(name='q')
    p1_pid1_t1 = Test(name='t1', asserts=[p1_pid1_t1_a1],
                      questions=[p1_pid1_t1_q])
    p1_pid1_ci_prop1 = CiProperties(url="url1")
    p1_pid1 = Policy(name='p1', id_='pid1', ci_props=p1_pid1_ci_prop1, tests=[p1_pid1_t1],
    ...
    object_text_dict = {
        'ss1': {
            ...
            'policies/p1': json.dumps(
                [p1_pid1.get_summary(), p1_pid2.get_summary()]),
        }
    }
    ...
    assert load_policy(session, 'p1',
                       'pid2', 'ss1').get_summary() == p1_pid2.get_summary()
