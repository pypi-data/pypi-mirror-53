...
from pybfe.datamodel.policy import (CiProperties, STATUS_ERROR, STATUS_FAIL,
                                    STATUS_PASS)
...
def session_with_no_assert_metadata():
    return MockSession(assert_name=None, test_name='test',
                       policy_name='policy', policy_id='pid',
                       policy_ci_props=CiProperties(url="url"))
...
@pytest.mark.parametrize('assert_func, params, helper, name', [
...
def test_record_result(asserts_with_metadata):
    ...
    with patch.object(asserts_with_metadata.session,
        ...
        asserts_with_metadata._record_result(result='result',
                                             status='status',
                                             message='message',
                                             snapshot='ss1')
...
def test_record_result_no_assert_metadata(asserts_with_no_assert_metadata):
    ...
    with patch.object(asserts_with_no_assert_metadata.session,
                      '_add_assertion') as mock_add:
        asserts_with_no_assert_metadata._record_result(result='result',
                                                       status='status',
                                                       message='message',
                                                       snapshot='ss1')
        mock_add.assert_called_with(policy_name='policy', policy_id='pid',
