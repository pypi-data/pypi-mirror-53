...
import json
from typing import (
    Any, Dict, List, TYPE_CHECKING, Text
)
...
def write_test(test, policy, session, snapshot):
    ...
    test_summary_resource = 'policies/{}/{}/{}'.format(policy.name,
                                                       policy.id,
                                                       test.name)
    data = _get_test_details(test)
    session.put_snapshot_object(test_summary_resource,
                                json.dumps(data), snapshot=snapshot)
