...
import json
from typing import (
    Any, Dict, List, Optional, TYPE_CHECKING, Text
)

from requests import HTTPError

from pybfe.client.policy._common import aggregate_status
from pybfe.client.policy._test import (
    update_test_status, write_test
)
...
def _get_latest_run(session, name, snapshot):
    # type: (Session, Text, Optional[Text]) -> Optional[Dict]
    """Get the latest run for a given policy name."""
    try:
        policies = json.loads(
            session.get_snapshot_object_text(
...
def _load_policy_summary(session, policy_name, snapshot):
    # type: (Session, Text, Optional[Text]) -> Optional[List[Dict]]
    ...
    try:
        return json.loads(
            session.get_snapshot_object_text(policy_history_rsc,
                                             snapshot=snapshot))
    except HTTPError as e:
...
def load_policy(session, policy_name, policy_id=None, snapshot=None):
    ...
    policy_history = _load_policy_summary(session, policy_name,
                                          snapshot)
    ...
    for policy in policy_history:
        if policy.get('name') == policy_name and policy.get(
                'id') == policy_id:
            ci_props = CiProperties.from_dict(policy.get("ci_props", {}))
            timestamp = policy.get('timestamp')
            break

    policy_details_rsc = 'policies/{}/{}'.format(policy_name,
                                                 policy_id)
    ...
    try:
        test_dicts = json.loads(
            session.get_snapshot_object_text(policy_details_rsc,
                                             snapshot=snapshot))
    except HTTPError as e:
    ...
    for test_dict in test_dicts:
        test_details_rsc = '{}/{}'.format(policy_details_rsc,
                                          test_dict.get('name'))

        # Fetch list of assertions for each test from service
        test_details_dict = json.loads(
            session.get_snapshot_object_text(test_details_rsc,
                                             snapshot=snapshot))
        asserts = [
            _to_assertion(a) for a in test_details_dict.get('assertions')
        ]
    ...
    p = Policy(policy_name, policy_id, ci_props=ci_props, tests=tests, timestamp=timestamp)
...
def _to_question(question_dict):
    # type: (Dict[Text, Any]) -> Question
    """Convert question dictionary into a question object."""
    return Question(name=question_dict.get('name'))
...
def _update_policies_summary(policy, session, snapshot):
    ...
    try:
        latest_policy_summaries = json.loads(
            session.get_snapshot_object_text(policies_resource,
                                             snapshot=snapshot))
        ...
        if e.response.status_code == 404:
            ...
            raise e
    session.put_snapshot_object(policies_resource,
                                json.dumps(latest_policy_summaries),
                                snapshot=snapshot)
...
def _update_policy_history(policy, session, snapshot):
    ...
    try:
        policy_history = json.loads(
            session.get_snapshot_object_text(policy_history_resource,
                                             snapshot=snapshot))
        ...
        for i, prev_run in enumerate(policy_history):
            if prev_run.get('name') == policy.name and prev_run.get(
                    'id') == policy.id:
        ...
        if e.response.status_code == 404:
            ...
            raise e
    session.put_snapshot_object(policy_history_resource,
                                json.dumps(policy_history), snapshot=snapshot)


def _write_policy_details(policy, session, snapshot):
    # type: (Policy, Session, Text) -> None
    """Add policy details (Test summaries) for this policy to the remote service."""
    specific_policy_resource = 'policies/{}/{}'.format(policy.name,
                                                       policy.id)
    session.put_snapshot_object(specific_policy_resource,
