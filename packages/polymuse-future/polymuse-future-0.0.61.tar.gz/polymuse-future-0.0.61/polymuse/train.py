...
def train(dataset_path, maxx = 1, graph = False):
    ...
    data_gen = data_generator.NoteDataGenerator(0, f, 32, 32) #init the data generator, this makes possible to train on large dataset which cannot fit into memory

    rnn_h.build_sFlat_model(data_gen, epochs=50, dev = True, cell_count= 512) # makes a models and train on Note data, sFlat representaion of midi, and then to octave encoding used for training 
    if graph : 
...
def train_gpu(dataset_path, max = 1):
    pass
