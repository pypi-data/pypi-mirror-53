...
def _datapath(_path_wd):
    return str(_path_wd.mkdir('dataset'))
...
def _dataset(_config, _datapath):
    ...
    np.savez_compressed(os.path.join(_datapath, 'x_test'), x_test)
    np.savez_compressed(os.path.join(_datapath, 'y_test'), y_test)
    np.savez_compressed(os.path.join(_datapath, 'x_norm'), x_test)

    _config.set('paths', 'dataset_path', _datapath)
