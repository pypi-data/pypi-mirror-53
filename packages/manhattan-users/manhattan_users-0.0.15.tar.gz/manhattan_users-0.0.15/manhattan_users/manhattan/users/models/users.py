...
class BaseUser(ComparableFrame):
    ...
    def password(self, value):
        ...
        # Set the password
        iterations = flask.current_app.config['USER_PASSWORD_HASH_ITERATIONS']
    ...
    def generate_mfa_otp_provisioning_uri(self):
        ...
        return pyotp.TOTP(self.mfa_otp_secret).provisioning_uri(
            self.email,
            issuer_name=current_app.config['USER_MFA_ISSUER']
        )
    ...
    def sign_in(self):
        ...
        if last_accessed:
            ...
            if age > flask.current_app.config['USER_SESSION_LIFESPAN']:
    ...
    def from_session(cls, projection=None):
        ...
        if not cls.get_session_cls().verify_access(user._id):
            return None
    ...
    def is_locked_out(cls, email):
        ...
        cache = config['USER_FAILED_SIGN_IN_CACHE']
        return (cache.get(cls.get_sign_in_attempt_key(email)) or 0) \
                >= config['USER_MAX_FAILED_SIGN_IN_ATTEMPTS']
